class CfgPatches
{
	class rhsusf_c_troops
	{
		units[]=
		{
			"rhsusf_army_ucp_rifleman_1stcav",
			"rhsusf_army_ucp_rifleman_10th",
			"rhsusf_army_ucp_rifleman",
			"rhsusf_army_ucp_rifleman_m4",
			"rhsusf_army_ucp_rifleman_m16",
			"rhsusf_army_ucp_rifleman_m590",
			"rhsusf_army_ucp_riflemanl",
			"rhsusf_army_ucp_riflemanat",
			"rhsusf_army_ucp_grenadier",
			"rhsusf_army_ucp_autorifleman",
			"rhsusf_army_ucp_autoriflemana",
			"rhsusf_army_ucp_machinegunner",
			"rhsusf_army_ucp_machinegunnera",
			"rhsusf_army_ucp_marksman",
			"rhsusf_army_ucp_officer",
			"rhsusf_army_ucp_squadleader",
			"rhsusf_army_ucp_teamleader",
			"rhsusf_army_ucp_jfo",
			"rhsusf_army_ucp_fso",
			"rhsusf_army_ucp_javelin",
			"rhsusf_army_ucp_javelin_assistant",
			"rhsusf_army_ucp_maaws",
			"rhsusf_army_ucp_aa",
			"rhsusf_army_ucp_uav",
			"rhsusf_army_ucp_medic",
			"rhsusf_army_ucp_engineer",
			"rhsusf_army_ucp_explosives",
			"rhsusf_army_ucp_sniper",
			"rhsusf_army_ucp_sniper_m24sws",
			"rhsusf_army_ucp_sniper_m107",
			"rhsusf_army_ucp_crewman",
			"rhsusf_army_ucp_combatcrewman",
			"rhsusf_army_ucp_driver",
			"rhsusf_army_ucp_driver_armored",
			"rhsusf_army_ucp_helipilot",
			"rhsusf_army_ucp_ah64_pilot",
			"rhsusf_army_ucp_helicrew",
			"rhsusf_army_ucp_rifleman_82nd",
			"rhsusf_army_ucp_rifleman_101st",
			"rhsusf_army_ucp_arb_rifleman",
			"rhsusf_army_ucp_arb_riflemanl",
			"rhsusf_army_ucp_rifleman_arb_m16",
			"rhsusf_army_ucp_arb_rifleman_m590",
			"rhsusf_army_ucp_arb_riflemanat",
			"rhsusf_army_ucp_arb_maaws",
			"rhsusf_army_ucp_arb_grenadier",
			"rhsusf_army_ucp_arb_autorifleman",
			"rhsusf_army_ucp_arb_autoriflemana",
			"rhsusf_army_ucp_arb_machinegunner",
			"rhsusf_army_ucp_arb_machinegunnera",
			"rhsusf_army_ucp_arb_marksman",
			"rhsusf_army_ucp_arb_squadleader",
			"rhsusf_army_ucp_arb_teamleader",
			"rhsusf_army_ucp_arb_medic",
			"rhsusf_army_ucp_arb_engineer",
			"rhsusf_army_ucp_arb_sniper_m107",
			"rhsusf_army_ocp_rifleman_1stcav",
			"rhsusf_army_ocp_rifleman_10th",
			"rhsusf_army_ocp_rifleman",
			"rhsusf_army_ocp_rifleman_m4",
			"rhsusf_army_ocp_rifleman_m16",
			"rhsusf_army_ocp_rifleman_m590",
			"rhsusf_army_ocp_riflemanl",
			"rhsusf_army_ocp_riflemanat",
			"rhsusf_army_ocp_grenadier",
			"rhsusf_army_ocp_autorifleman",
			"rhsusf_army_ocp_autoriflemana",
			"rhsusf_army_ocp_machinegunner",
			"rhsusf_army_ocp_machinegunnera",
			"rhsusf_army_ocp_marksman",
			"rhsusf_army_ocp_officer",
			"rhsusf_army_ocp_squadleader",
			"rhsusf_army_ocp_teamleader",
			"rhsusf_army_ocp_jfo",
			"rhsusf_army_ocp_fso",
			"rhsusf_army_ocp_javelin",
			"rhsusf_army_ocp_javelin_assistant",
			"rhsusf_army_ocp_maaws",
			"rhsusf_army_ocp_aa",
			"rhsusf_army_ocp_uav",
			"rhsusf_army_ocp_medic",
			"rhsusf_army_ocp_engineer",
			"rhsusf_army_ocp_explosives",
			"rhsusf_army_ocp_sniper",
			"rhsusf_army_ocp_sniper_m24sws",
			"rhsusf_army_ocp_sniper_m107",
			"rhsusf_army_ocp_crewman",
			"rhsusf_army_ocp_combatcrewman",
			"rhsusf_army_ocp_driver",
			"rhsusf_army_ocp_driver_armored",
			"rhsusf_army_ocp_helipilot",
			"rhsusf_army_ocp_ah64_pilot",
			"rhsusf_army_ocp_helicrew",
			"rhsusf_army_ocp_rifleman_82nd",
			"rhsusf_army_ocp_rifleman_101st",
			"rhsusf_army_ocp_arb_rifleman",
			"rhsusf_army_ocp_arb_riflemanl",
			"rhsusf_army_ocp_rifleman_arb_m16",
			"rhsusf_army_ocp_arb_rifleman_m590",
			"rhsusf_army_ocp_arb_riflemanat",
			"rhsusf_army_ocp_arb_maaws",
			"rhsusf_army_ocp_arb_grenadier",
			"rhsusf_army_ocp_arb_autorifleman",
			"rhsusf_army_ocp_arb_autoriflemana",
			"rhsusf_army_ocp_arb_machinegunner",
			"rhsusf_army_ocp_arb_machinegunnera",
			"rhsusf_army_ocp_arb_marksman",
			"rhsusf_army_ocp_arb_squadleader",
			"rhsusf_army_ocp_arb_teamleader",
			"rhsusf_army_ocp_arb_medic",
			"rhsusf_army_ocp_arb_engineer",
			"rhsusf_army_ocp_arb_sniper_m107",
			"rhsusf_airforce_pilot",
			"rhsusf_airforce_jetpilot",
			"rhsusf_navy_sarc_d_fast",
			"rhsusf_navy_sarc_w_fast",
			"rhsusf_navy_sarc_d",
			"rhsusf_navy_sarc_w",
			"rhsusf_navy_marpat_d_medic",
			"rhsusf_navy_marpat_wd_medic",
			"rhsusf_socom_swcc_officer",
			"rhsusf_socom_swcc_crewman",
			"rhsusf_socom_marsoc_marksman",
			"rhsusf_socom_marsoc_sniper_m107",
			"rhsusf_socom_marsoc_sniper",
			"rhsusf_socom_marsoc_spotter",
			"rhsusf_socom_marsoc_sarc",
			"rhsusf_socom_marsoc_cso_mechanic",
			"rhsusf_socom_marsoc_cso_eod",
			"rhsusf_socom_marsoc_jfo",
			"rhsusf_socom_marsoc_jtac",
			"rhsusf_socom_marsoc_elementleader",
			"rhsusf_socom_marsoc_teamchief",
			"rhsusf_socom_marsoc_teamleader",
			"rhsusf_socom_marsoc_cso_grenadier",
			"rhsusf_socom_marsoc_cso_breacher",
			"rhsusf_socom_marsoc_cso_light",
			"rhsusf_socom_marsoc_cso_cqb",
			"rhsusf_socom_marsoc_cso",
			"rhsusf_socom_marsoc_cso_mk17",
			"rhsusf_socom_marsoc_cso_mk17_light",
			"rhsusf_infantry_socom_armysf_rifleman",
			"rhsusf_usmc_marpat_d_rifleman",
			"rhsusf_usmc_marpat_d_rifleman_m4",
			"rhsusf_usmc_marpat_d_rifleman_light",
			"rhsusf_usmc_marpat_d_riflemanat",
			"rhsusf_usmc_marpat_d_rifleman_law",
			"rhsusf_usmc_marpat_d_rifleman_m590",
			"rhsusf_usmc_marpat_d_grenadier",
			"rhsusf_usmc_marpat_d_grenadier_m32",
			"rhsusf_usmc_marpat_d_autorifleman",
			"rhsusf_usmc_marpat_d_autorifleman_m249",
			"rhsusf_usmc_marpat_d_autorifleman_m249_ass",
			"rhsusf_usmc_marpat_d_machinegunner",
			"rhsusf_usmc_marpat_d_machinegunner_ass",
			"rhsusf_usmc_marpat_d_officer",
			"rhsusf_usmc_marpat_d_squadleader",
			"rhsusf_usmc_marpat_d_teamleader",
			"rhsusf_usmc_marpat_d_jfo",
			"rhsusf_usmc_marpat_d_fso",
			"rhsusf_usmc_marpat_d_smaw",
			"rhsusf_usmc_marpat_d_javelin",
			"rhsusf_usmc_marpat_d_javelin_assistant",
			"rhsusf_usmc_marpat_d_stinger",
			"rhsusf_usmc_marpat_d_engineer",
			"rhsusf_usmc_marpat_d_explosives",
			"rhsusf_usmc_marpat_d_uav",
			"rhsusf_usmc_marpat_d_marksman",
			"rhsusf_usmc_marpat_d_spotter",
			"rhsusf_usmc_marpat_d_sniper",
			"rhsusf_usmc_marpat_d_sniper_m110",
			"rhsusf_usmc_marpat_d_sniper_M107",
			"rhsusf_usmc_marpat_d_crewman",
			"rhsusf_usmc_marpat_d_combatcrewman",
			"rhsusf_usmc_marpat_d_driver",
			"rhsusf_usmc_marpat_d_gunner",
			"rhsusf_usmc_marpat_d_helipilot",
			"rhsusf_usmc_marpat_d_helicrew",
			"rhsusf_usmc_recon_marpat_d_teamleader_lite",
			"rhsusf_usmc_recon_marpat_d_marksman_lite",
			"rhsusf_usmc_recon_marpat_d_machinegunner_m249_lite",
			"rhsusf_usmc_recon_marpat_d_autorifleman_lite",
			"rhsusf_usmc_recon_marpat_d_rifleman_at_lite",
			"rhsusf_usmc_recon_marpat_d_rifleman_lite",
			"rhsusf_usmc_recon_marpat_d_teamleader_fast",
			"rhsusf_usmc_recon_marpat_d_marksman_fast",
			"rhsusf_usmc_recon_marpat_d_machinegunner_m249_fast",
			"rhsusf_usmc_recon_marpat_d_autorifleman_fast",
			"rhsusf_usmc_recon_marpat_d_rifleman_at_fast",
			"rhsusf_usmc_recon_marpat_d_rifleman_fast",
			"rhsusf_usmc_recon_marpat_d_teamleader",
			"rhsusf_usmc_recon_marpat_d_officer",
			"rhsusf_usmc_recon_marpat_d_sniper_M107",
			"rhsusf_usmc_recon_marpat_d_marksman",
			"rhsusf_usmc_recon_marpat_d_machinegunner_m249",
			"rhsusf_usmc_recon_marpat_d_machinegunner",
			"rhsusf_usmc_recon_marpat_d_grenadier_m32",
			"rhsusf_usmc_recon_marpat_d_autorifleman",
			"rhsusf_usmc_recon_marpat_d_rifleman_at",
			"rhsusf_usmc_recon_marpat_d_rifleman",
			"rhsusf_usmc_lar_marpat_d_machinegunner",
			"rhsusf_usmc_lar_marpat_d_combatcrewman",
			"rhsusf_usmc_lar_marpat_d_crewman",
			"rhsusf_usmc_lar_marpat_d_marksman",
			"rhsusf_usmc_lar_marpat_d_teamleader",
			"rhsusf_usmc_lar_marpat_d_squadleader",
			"rhsusf_usmc_lar_marpat_d_autorifleman",
			"rhsusf_usmc_lar_marpat_d_grenadier_m32",
			"rhsusf_usmc_lar_marpat_d_riflemanat",
			"rhsusf_usmc_lar_marpat_d_rifleman_light",
			"rhsusf_usmc_lar_marpat_d_rifleman",
			"rhsusf_usmc_marpat_wd_rifleman",
			"rhsusf_usmc_marpat_wd_rifleman_m4",
			"rhsusf_usmc_marpat_wd_rifleman_light",
			"rhsusf_usmc_marpat_wd_riflemanat",
			"rhsusf_usmc_marpat_wd_rifleman_law",
			"rhsusf_usmc_marpat_wd_rifleman_m590",
			"rhsusf_usmc_marpat_wd_grenadier",
			"rhsusf_usmc_marpat_wd_grenadier_m32",
			"rhsusf_usmc_marpat_wd_autorifleman",
			"rhsusf_usmc_marpat_wd_autorifleman_m249",
			"rhsusf_usmc_marpat_wd_autorifleman_m249_ass",
			"rhsusf_usmc_marpat_wd_machinegunner",
			"rhsusf_usmc_marpat_wd_machinegunner_ass",
			"rhsusf_usmc_marpat_wd_officer",
			"rhsusf_usmc_marpat_wd_squadleader",
			"rhsusf_usmc_marpat_wd_teamleader",
			"rhsusf_usmc_marpat_wd_jfo",
			"rhsusf_usmc_marpat_wd_fso",
			"rhsusf_usmc_marpat_wd_smaw",
			"rhsusf_usmc_marpat_wd_javelin",
			"rhsusf_usmc_marpat_wd_javelin_assistant",
			"rhsusf_usmc_marpat_wd_stinger",
			"rhsusf_usmc_marpat_wd_engineer",
			"rhsusf_usmc_marpat_wd_explosives",
			"rhsusf_usmc_marpat_wd_uav",
			"rhsusf_usmc_marpat_wd_marksman",
			"rhsusf_usmc_marpat_wd_spotter",
			"rhsusf_usmc_marpat_wd_sniper",
			"rhsusf_usmc_marpat_wd_sniper_m110",
			"rhsusf_usmc_marpat_wd_sniper_M107",
			"rhsusf_usmc_marpat_wd_crewman",
			"rhsusf_usmc_marpat_wd_combatcrewman",
			"rhsusf_usmc_marpat_wd_driver",
			"rhsusf_usmc_marpat_wd_gunner",
			"rhsusf_usmc_marpat_wd_helipilot",
			"rhsusf_usmc_marpat_wd_helicrew",
			"rhsusf_usmc_recon_marpat_wd_teamleader_lite",
			"rhsusf_usmc_recon_marpat_wd_marksman_lite",
			"rhsusf_usmc_recon_marpat_wd_machinegunner_m249_lite",
			"rhsusf_usmc_recon_marpat_wd_autorifleman_lite",
			"rhsusf_usmc_recon_marpat_wd_rifleman_at_lite",
			"rhsusf_usmc_recon_marpat_wd_rifleman_lite",
			"rhsusf_usmc_recon_marpat_wd_teamleader_fast",
			"rhsusf_usmc_recon_marpat_wd_marksman_fast",
			"rhsusf_usmc_recon_marpat_wd_machinegunner_m249_fast",
			"rhsusf_usmc_recon_marpat_wd_autorifleman_fast",
			"rhsusf_usmc_recon_marpat_wd_rifleman_at_fast",
			"rhsusf_usmc_recon_marpat_wd_rifleman_fast",
			"rhsusf_usmc_recon_marpat_wd_teamleader",
			"rhsusf_usmc_recon_marpat_wd_officer",
			"rhsusf_usmc_recon_marpat_wd_sniper_M107",
			"rhsusf_usmc_recon_marpat_wd_marksman",
			"rhsusf_usmc_recon_marpat_wd_machinegunner_m249",
			"rhsusf_usmc_recon_marpat_wd_machinegunner",
			"rhsusf_usmc_recon_marpat_wd_autorifleman",
			"rhsusf_usmc_recon_marpat_wd_grenadier_m32",
			"rhsusf_usmc_recon_marpat_wd_rifleman_at",
			"rhsusf_usmc_recon_marpat_wd_rifleman",
			"rhsusf_usmc_lar_marpat_wd_machinegunner",
			"rhsusf_usmc_lar_marpat_wd_combatcrewman",
			"rhsusf_usmc_lar_marpat_wd_crewman",
			"rhsusf_usmc_lar_marpat_wd_marksman",
			"rhsusf_usmc_lar_marpat_wd_teamleader",
			"rhsusf_usmc_lar_marpat_wd_squadleader",
			"rhsusf_usmc_lar_marpat_wd_autorifleman",
			"rhsusf_usmc_lar_marpat_wd_grenadier_m32",
			"rhsusf_usmc_lar_marpat_wd_riflemanat",
			"rhsusf_usmc_lar_marpat_wd_rifleman_light",
			"rhsusf_usmc_lar_marpat_wd_rifleman",
			"Item_rhs_uniform_cu_ocp",
			"Item_rhs_uniform_cu_ucp",
			"Item_rhs_uniform_FROG01_d",
			"Item_rhs_uniform_FROG01_wd",
			"Item_rhs_uniform_g3_mc",
			"Item_rhs_uniform_g3_m81",
			"Item_rhs_uniform_g3_aor2",
			"Item_rhs_uniform_g3_tan",
			"Item_rhs_uniform_g3_rgr",
			"Item_rhs_uniform_g3_blk",
			"Item_rhs_uniform_cu_ocp_82nd",
			"Item_rhs_uniform_cu_ocp_101st",
			"Item_rhs_uniform_cu_ocp_10th",
			"Item_rhs_uniform_cu_ucp_82nd",
			"Item_rhs_uniform_cu_ucp_101st",
			"Item_rhs_uniform_cu_ucp_10th",
			"Item_rhs_uniform_cu_ocp_patchless",
			"Item_rhs_uniform_cu_ucp_patchless",
			"Item_rhs_uniform_acu_ucp",
			"Item_rhs_uniform_abu",
			"Item_rhs_uniform_bdu_erdl",
			"Item_rhs_xmas_antlers",
			"Item_rhs_Booniehat_ocp",
			"Item_rhs_Booniehat_ucp",
			"Item_rhs_Booniehat_marpatd",
			"Item_rhs_Booniehat_marpatwd",
			"Item_rhs_Booniehat_m81",
			"Item_rhs_booniehat2_marpatd",
			"Item_rhs_booniehat2_marpatwd",
			"Item_rhs_8point_marpatd",
			"Item_rhs_8point_marpatwd",
			"Item_RHS_jetpilot_usaf",
			"Item_rhsusf_patrolcap_ocp",
			"Item_rhsusf_patrolcap_ucp",
			"Item_rhsusf_ach_helmet_ocp",
			"Item_rhsusf_ach_helmet_ocp_norotos",
			"Item_rhsusf_ach_helmet_ucp",
			"Item_rhsusf_ach_helmet_ucp_norotos",
			"Item_rhsusf_ach_helmet_M81",
			"Item_rhsusf_ach_helmet_camo_ocp",
			"Item_rhsusf_ach_helmet_headset_ocp",
			"Item_rhsusf_ach_helmet_headset_ucp",
			"Item_rhsusf_ach_helmet_ESS_ocp",
			"Item_rhsusf_ach_helmet_ESS_ucp",
			"Item_rhsusf_ach_helmet_headset_ess_ocp",
			"Item_rhsusf_ach_helmet_headset_ess_ucp",
			"Item_rhsusf_ach_bare",
			"Item_rhsusf_ach_bare_ess",
			"Item_rhsusf_ach_bare_headset",
			"Item_rhsusf_ach_bare_headset_ess",
			"Item_rhsusf_ach_bare_tan",
			"Item_rhsusf_ach_bare_tan_ess",
			"Item_rhsusf_ach_bare_tan_headset",
			"Item_rhsusf_ach_bare_tan_headset_ess",
			"Item_rhsusf_ach_bare_wood",
			"Item_rhsusf_ach_bare_wood_ess",
			"Item_rhsusf_ach_bare_wood_headset",
			"Item_rhsusf_ach_bare_wood_headset_ess",
			"Item_rhsusf_ach_bare_des",
			"Item_rhsusf_ach_bare_des_ess",
			"Item_rhsusf_ach_bare_des_headset",
			"Item_rhsusf_ach_bare_des_headset_ess",
			"Item_rhsusf_ach_bare_semi",
			"Item_rhsusf_ach_bare_semi_ess",
			"Item_rhsusf_ach_bare_semi_headset",
			"Item_rhsusf_ach_bare_semi_headset_ess",
			"Item_rhsusf_opscore_01",
			"Item_rhsusf_opscore_fg",
			"Item_rhsusf_opscore_fg_pelt",
			"Item_rhsusf_opscore_fg_pelt_nsw",
			"Item_rhsusf_opscore_fg_pelt_cam",
			"Item_rhsusf_opscore_ut",
			"Item_rhsusf_opscore_ut_pelt",
			"Item_rhsusf_opscore_ut_pelt_cam",
			"Item_rhsusf_opscore_ut_pelt_nsw",
			"Item_rhsusf_opscore_ut_pelt_nsw_cam",
			"Item_rhsusf_opscore_bk",
			"Item_rhsusf_opscore_bk_pelt",
			"Item_rhsusf_opscore_mc",
			"Item_rhsusf_opscore_mc_pelt",
			"Item_rhsusf_opscore_mc_pelt_nsw",
			"Item_rhsusf_opscore_aor1",
			"Item_rhsusf_opscore_aor1_pelt",
			"Item_rhsusf_opscore_aor1_pelt_nsw",
			"Item_rhsusf_opscore_aor2",
			"Item_rhsusf_opscore_aor2_pelt",
			"Item_rhsusf_opscore_aor2_pelt_nsw",
			"Item_rhsusf_opscore_paint",
			"Item_rhsusf_opscore_paint_pelt",
			"Item_rhsusf_opscore_paint_pelt_nsw",
			"Item_rhsusf_opscore_paint_pelt_nsw_cam",
			"Item_rhsusf_opscore_cover",
			"Item_rhsusf_opscore_mc_cover",
			"Item_rhsusf_opscore_mc_cover_pelt",
			"Item_rhsusf_opscore_mc_cover_pelt_nsw",
			"Item_rhsusf_opscore_mc_cover_pelt_cam",
			"Item_rhsusf_opscore_rg_cover",
			"Item_rhsusf_opscore_rg_cover_pelt",
			"Item_rhsusf_opscore_coy_cover",
			"Item_rhsusf_opscore_coy_cover_pelt",
			"Item_rhsusf_opscore_mar_01",
			"Item_rhsusf_opscore_mar_ut",
			"Item_rhsusf_opscore_mar_ut_pelt",
			"Item_rhsusf_opscore_mar_fg",
			"Item_rhsusf_opscore_mar_fg_pelt",
			"Item_rhsusf_opscore_01_tan",
			"Item_rhsusf_opscore_02",
			"Item_rhsusf_opscore_02_tan",
			"Item_rhsusf_opscore_03_ocp",
			"Item_rhsusf_opscore_04_ocp",
			"Item_rhsusf_cvc_helmet",
			"Item_rhsusf_cvc_alt_helmet",
			"Item_rhsusf_cvc_green_helmet",
			"Item_rhsusf_cvc_green_alt_helmet",
			"Item_rhsusf_cvc_ess",
			"Item_rhsusf_cvc_green_ess",
			"Item_rhsusf_hgu56p",
			"Item_rhsusf_hgu56p_visor",
			"Item_rhsusf_hgu56p_mask",
			"Item_rhsusf_hgu56p_visor_mask",
			"Item_rhsusf_hgu56p_visor_mask_skull",
			"Item_rhsusf_hgu56p_mask_skull",
			"Item_rhsusf_hgu56p_visor_mask_mo",
			"Item_rhsusf_hgu56p_mask_mo",
			"Item_rhsusf_hgu56p_mask_smiley",
			"Item_rhsusf_hgu56p_visor_mask_smiley",
			"Item_rhsusf_hgu56p_black",
			"Item_rhsusf_hgu56p_visor_black",
			"Item_rhsusf_hgu56p_mask_black",
			"Item_rhsusf_hgu56p_visor_mask_black",
			"Item_rhsusf_hgu56p_visor_mask_black_skull",
			"Item_rhsusf_hgu56p_mask_black_skull",
			"Item_rhsusf_hgu56p_visor_mask_Empire_black",
			"Item_rhsusf_hgu56p_green",
			"Item_rhsusf_hgu56p_visor_green",
			"Item_rhsusf_hgu56p_mask_green",
			"Item_rhsusf_hgu56p_visor_mask_green",
			"Item_rhsusf_hgu56p_mask_green_mo",
			"Item_rhsusf_hgu56p_visor_mask_green_mo",
			"Item_rhsusf_hgu56p_tan",
			"Item_rhsusf_hgu56p_visor_tan",
			"Item_rhsusf_hgu56p_mask_tan",
			"Item_rhsusf_hgu56p_visor_mask_tan",
			"Item_rhsusf_hgu56p_pink",
			"Item_rhsusf_hgu56p_visor_pink",
			"Item_rhsusf_hgu56p_mask_pink",
			"Item_rhsusf_hgu56p_visor_mask_pink",
			"Item_rhsusf_hgu56p_usa",
			"Item_rhsusf_hgu56p_visor_usa",
			"Item_rhsusf_hgu56p_saf",
			"Item_rhsusf_hgu56p_visor_saf",
			"Item_rhsusf_hgu56p_mask_saf",
			"Item_rhsusf_hgu56p_visor_mask_saf",
			"Item_rhsusf_ihadss",
			"Item_rhsusf_ANPVS_14",
			"Item_rhsusf_ANPVS_15",
			"Item_rhsusf_Rhino",
			"Item_rhsusf_anvis_nvg_bc_caps",
			"Item_rhsusf_ANVIS",
			"Item_rhsusf_mich_helmet_marpatwd",
			"Item_rhsusf_mich_helmet_marpatwd_headset",
			"Item_rhsusf_mich_helmet_marpatd",
			"Item_rhsusf_mich_helmet_marpatd_headset",
			"Item_rhsusf_mich_helmet_marpatwd_alt",
			"Item_rhsusf_mich_helmet_marpatwd_alt_headset",
			"Item_rhsusf_mich_helmet_marpatd_alt",
			"Item_rhsusf_mich_helmet_marpatd_alt_headset",
			"Item_rhsusf_mich_helmet_marpatwd_norotos",
			"Item_rhsusf_mich_helmet_marpatwd_norotos_headset",
			"Item_rhsusf_mich_helmet_marpatd_norotos",
			"Item_rhsusf_mich_helmet_marpatd_norotos_headset",
			"Item_rhsusf_mich_helmet_marpatwd_norotos_arc",
			"Item_rhsusf_mich_helmet_marpatwd_norotos_arc_headset",
			"Item_rhsusf_mich_helmet_marpatd_norotos_arc",
			"Item_rhsusf_mich_helmet_marpatd_norotos_arc_headset",
			"Item_rhsusf_mich_bare",
			"Item_rhsusf_mich_bare_headset",
			"Item_rhsusf_mich_bare_alt",
			"Item_rhsusf_mich_bare_norotos",
			"Item_rhsusf_mich_bare_norotos_headset",
			"Item_rhsusf_mich_bare_norotos_alt",
			"Item_rhsusf_mich_bare_norotos_alt_headset",
			"Item_rhsusf_mich_bare_norotos_arc",
			"Item_rhsusf_mich_bare_norotos_arc_headset",
			"Item_rhsusf_mich_bare_norotos_arc_alt",
			"Item_rhsusf_mich_bare_norotos_arc_alt_headset",
			"Item_rhsusf_mich_bare_tan",
			"Item_rhsusf_mich_bare_tan_headset",
			"Item_rhsusf_mich_bare_alt_tan",
			"Item_rhsusf_mich_bare_norotos_tan",
			"Item_rhsusf_mich_bare_norotos_tan_headset",
			"Item_rhsusf_mich_bare_norotos_alt_tan",
			"Item_rhsusf_mich_bare_norotos_alt_tan_headset",
			"Item_rhsusf_mich_bare_norotos_arc_tan",
			"Item_rhsusf_mich_bare_norotos_arc_tan_headset",
			"Item_rhsusf_mich_bare_norotos_arc_alt_tan",
			"Item_rhsusf_mich_bare_norotos_arc_alt_tan_headset",
			"Item_rhsusf_mich_bare_semi",
			"Item_rhsusf_mich_bare_semi_headset",
			"Item_rhsusf_mich_bare_alt_semi",
			"Item_rhsusf_mich_bare_norotos_semi",
			"Item_rhsusf_mich_bare_norotos_semi_headset",
			"Item_rhsusf_mich_bare_norotos_alt_semi",
			"Item_rhsusf_mich_bare_norotos_alt_semi_headset",
			"Item_rhsusf_mich_bare_norotos_arc_semi",
			"Item_rhsusf_mich_bare_norotos_arc_semi_headset",
			"Item_rhsusf_mich_bare_norotos_arc_alt_semi",
			"Item_rhsusf_mich_bare_norotos_arc_alt_semi_headset",
			"Item_rhsusf_ach_helmet_ocp",
			"Item_rhsusf_lwh_helmet_marpatd",
			"Item_rhsusf_lwh_helmet_marpatd_ess",
			"Item_rhsusf_lwh_helmet_marpatd_headset",
			"Item_rhsusf_lwh_helmet_marpatwd",
			"Item_rhsusf_lwh_helmet_marpatwd_ess",
			"Item_rhsusf_lwh_helmet_marpatwd_blk_ess",
			"Item_rhsusf_lwh_helmet_marpatwd_headset",
			"Item_rhsusf_lwh_helmet_marpatwd_headset_blk",
			"Item_rhsusf_lwh_helmet_M1942",
			"Item_rhsusf_Bowman",
			"Item_rhsusf_bowman_cap",
			"Item_rhsusf_protech_helmet",
			"Item_rhsusf_protech_helmet_ess",
			"Item_rhsusf_protech_helmet_rhino",
			"Item_rhsusf_protech_helmet_rhino_ess",
			"Item_rhsusf_iotv_ocp",
			"Item_rhsusf_iotv_ocp_Grenadier",
			"Item_rhsusf_iotv_ocp_Medic",
			"Item_rhsusf_iotv_ocp_Repair",
			"Item_rhsusf_iotv_ocp_Rifleman",
			"Item_rhsusf_iotv_ocp_SAW",
			"Item_rhsusf_iotv_ocp_Squadleader",
			"Item_rhsusf_iotv_ocp_Teamleader",
			"Item_rhsusf_iotv_ucp",
			"Item_rhsusf_iotv_ucp_Grenadier",
			"Item_rhsusf_iotv_ucp_Medic",
			"Item_rhsusf_iotv_ucp_Repair",
			"Item_rhsusf_iotv_ucp_Rifleman",
			"Item_rhsusf_iotv_ucp_SAW",
			"Item_rhsusf_iotv_ucp_Squadleader",
			"Item_rhsusf_iotv_ucp_Teamleader",
			"Item_rhsusf_spcs_ocp_squadleader",
			"Item_rhsusf_spcs_ocp_teamleader",
			"Item_rhsusf_spcs_ocp_teamleader_alt",
			"Item_rhsusf_spcs_ocp_saw",
			"Item_rhsusf_spcs_ocp_grenadier",
			"Item_rhsusf_spcs_ocp_rifleman",
			"Item_rhsusf_spcs_ocp_rifleman_alt",
			"Item_rhsusf_spcs_ocp_medic",
			"Item_rhsusf_spcs_ocp_crewman",
			"Item_rhsusf_spcs_ocp_machinegunner",
			"Item_rhsusf_spcs_ocp_sniper",
			"Item_rhsusf_spcs_ucp",
			"Item_rhsusf_spcs_ucp_squadleader",
			"Item_rhsusf_spcs_ucp_teamleader",
			"Item_rhsusf_spcs_ucp_teamleader_alt",
			"Item_rhsusf_spcs_ucp_saw",
			"Item_rhsusf_spcs_ucp_grenadier",
			"Item_rhsusf_spcs_ucp_rifleman",
			"Item_rhsusf_spcs_ucp_rifleman_alt",
			"Item_rhsusf_spcs_ucp_medic",
			"Item_rhsusf_spcs_ucp_crewman",
			"Item_rhsusf_spcs_ucp_machinegunner",
			"Item_rhsusf_spcs_ucp_sniper",
			"Item_rhsusf_mbav",
			"Item_rhsusf_mbav_light",
			"Item_rhsusf_mbav_rifleman",
			"Item_rhsusf_mbav_mg",
			"Item_rhsusf_mbav_grenadier",
			"Item_rhsusf_mbav_medic",
			"Item_rhsusf_plateframe_sapi",
			"Item_rhsusf_plateframe_light",
			"Item_rhsusf_plateframe_rifleman",
			"Item_rhsusf_plateframe_teamleader",
			"Item_rhsusf_plateframe_grenadier",
			"Item_rhsusf_plateframe_machinegunner",
			"Item_rhsusf_plateframe_marksman",
			"Item_rhsusf_plateframe_medic",
			"Item_rhsusf_acc_compm4",
			"Item_rhsusf_acc_T1_high",
			"Item_rhsusf_acc_T1_low",
			"Item_rhsusf_acc_g33_t1",
			"Item_rhsusf_acc_EOTECH",
			"Item_rhsusf_acc_eotech_552",
			"Item_rhsusf_acc_eotech_552_d",
			"Item_rhsusf_acc_eotech_552_wd",
			"Item_rhsusf_acc_g33_xps3",
			"Item_rhsusf_acc_g33_xps3_tan",
			"Item_rhsusf_acc_M2A1",
			"Item_rhsusf_acc_RM05",
			"Item_rhsusf_acc_mrds",
			"Item_rhsusf_acc_mrds_c",
			"Item_rhsusf_acc_LEUPOLDMK4",
			"Item_rhsusf_acc_LEUPOLDMK4_d",
			"Item_rhsusf_acc_LEUPOLDMK4_wd",
			"Item_rhsusf_acc_LEUPOLDMK4_2",
			"Item_rhsusf_acc_LEUPOLDMK4_2_MRDS",
			"Item_rhsusf_acc_LEUPOLDMK4_2_d",
			"Item_rhsusf_acc_M8541",
			"Item_rhsusf_acc_M8541_mrds",
			"Item_rhsusf_acc_M8541_low",
			"Item_rhsusf_acc_M8541_low_d",
			"Item_rhsusf_acc_M8541_low_wd",
			"Item_rhsusf_acc_premier",
			"Item_rhsusf_acc_premier_mrds",
			"Item_rhsusf_acc_premier_low",
			"Item_rhsusf_acc_ELCAN",
			"Item_rhsusf_acc_ELCAN_ard",
			"Item_rhsusf_acc_ACOG",
			"Item_rhsusf_acc_ACOG2",
			"Item_rhsusf_acc_ACOG3",
			"Item_rhsusf_acc_ACOG_RMR",
			"Item_rhsusf_acc_ACOG_d",
			"Item_rhsusf_acc_ACOG_wd",
			"Item_rhsusf_acc_ACOG_USMC",
			"Item_rhsusf_acc_ACOG2_USMC",
			"Item_rhsusf_acc_ACOG3_USMC",
			"Item_rhsusf_acc_ACOG_MDO",
			"Item_rhsusf_acc_anpas13gv1",
			"Item_rhsusf_acc_M2010S",
			"Item_rhsusf_acc_M2010S_d",
			"Item_rhsusf_acc_M2010S_sa",
			"Item_rhsusf_acc_M2010S_wd",
			"Item_rhsusf_acc_SR25S",
			"Item_rhsusf_acc_SR25S_wd",
			"Item_rhsusf_acc_SR25S_d",
			"Item_rhsusf_acc_m24_silencer_black",
			"Item_rhsusf_acc_m24_silencer_d",
			"Item_rhsusf_acc_m24_silencer_wd",
			"Item_rhsusf_acc_m24_muzzlehider_black",
			"Item_rhsusf_acc_m24_muzzlehider_d",
			"Item_rhsusf_acc_m24_muzzlehider_wd",
			"Item_rhsusf_acc_rotex5_grey",
			"Item_rhsusf_acc_rotex5_tan",
			"Item_rhsusf_acc_nt4_black",
			"Item_rhsusf_acc_nt4_tan",
			"Item_rhsusf_acc_SF3P556",
			"Item_rhsusf_acc_SFMB556",
			"Item_rhsusf_acc_omega9k",
			"Item_rhsusf_acc_M952V",
			"Item_rhsusf_acc_wmx",
			"Item_rhsusf_acc_wmx_bk",
			"Item_rhsusf_acc_anpeq15side",
			"Item_rhsusf_acc_anpeq15_top",
			"Item_rhsusf_acc_anpeq15",
			"Item_rhsusf_acc_anpeq15_light",
			"Item_rhsusf_acc_anpeq15_wmx",
			"Item_rhsusf_acc_anpeq15_wmx_light",
			"Item_rhsusf_acc_anpeq15A",
			"Item_rhsusf_acc_anpeq16a",
			"Item_rhsusf_acc_anpeq16a_top",
			"Item_rhsusf_acc_harris_bipod",
			"Item_rhsusf_acc_harris_swivel",
			"Item_rhsusf_acc_saw_bipod",
			"Item_rhsusf_acc_saw_lw_bipod",
			"Item_rhsusf_acc_grip1",
			"Item_rhsusf_acc_grip2",
			"Item_rhsusf_acc_grip2_tan",
			"Item_rhsusf_acc_grip2_wd",
			"Item_rhsusf_acc_grip3",
			"Item_rhsusf_acc_grip3_tan",
			"Item_rhsusf_acc_grip4",
			"Item_rhsusf_acc_grip4_bipod",
			"Item_rhsusf_acc_kac_grip",
			"Item_rhsusf_acc_kac_grip_saw_bipod",
			"Item_rhsusf_acc_rvg_blk",
			"Item_rhsusf_acc_rvg_de",
			"Item_rhsusf_acc_tacsac_blk",
			"Item_rhsusf_acc_tacsac_tan",
			"Item_rhsusf_acc_tacsac_blue",
			"Item_rhsusf_acc_tdstubby_blk",
			"Item_rhsusf_acc_tdstubby_tan"
		};
		weapons[]=
		{
			"rhs_uniform_g3_mc",
			"rhs_uniform_g3_m81",
			"rhs_uniform_g3_aor2",
			"rhs_uniform_g3_tan",
			"rhs_uniform_g3_rgr",
			"rhs_uniform_g3_blk",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_cu_ucp",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_cu_ocp_1stcav",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_cu_ucp_1stcav",
			"rhs_uniform_cu_ucp",
			"rhs_uniform_cu_ocp_82nd",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_cu_ucp_82nd",
			"rhs_uniform_cu_ucp",
			"rhs_uniform_cu_ocp_101st",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_cu_ucp_101st",
			"rhs_uniform_cu_ucp",
			"rhs_uniform_cu_ocp_10th",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_cu_ucp_10th",
			"rhs_uniform_cu_ucp",
			"rhs_uniform_cu_ucp_patchless",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_cu_ocp_patchless",
			"rhs_uniform_cu_ucp",
			"rhs_uniform_FROG01_d",
			"rhs_uniform_cu_ocp",
			"rhs_uniform_FROG01_wd",
			"rhs_uniform_FROG01_d",
			"rhs_uniform_FROG01_m81",
			"rhs_uniform_acu_ucp",
			"rhs_uniform_acu_ucpd",
			"rhs_uniform_acu_oefcp",
			"rhs_uniform_acu_ocp",
			"rhs_uniform_abu",
			"rhs_uniform_bdu_erdl",
			"rhs_xmas_antlers",
			"rhs_Booniehat_ocp",
			"rhs_Booniehat_ucp",
			"rhs_Booniehat_marpatd",
			"rhs_Booniehat_marpatwd",
			"rhs_Booniehat_m81",
			"rhs_booniehat2_marpatd",
			"rhs_booniehat2_marpatwd",
			"rhs_8point_marpatd",
			"rhs_8point_marpatwd",
			"RHS_jetpilot_usaf",
			"rhsusf_patrolcap_ocp",
			"rhsusf_patrolcap_ucp",
			"rhsusf_ach_helmet_ocp",
			"rhsusf_ach_helmet_ocp_alt",
			"rhsusf_ach_helmet_ocp_norotos",
			"rhsusf_ach_helmet_ucp",
			"rhsusf_ach_helmet_ucp_alt",
			"rhsusf_ach_helmet_ucp_norotos",
			"rhsusf_ach_helmet_M81",
			"rhsusf_ach_helmet_camo_ocp",
			"rhsusf_ach_helmet_headset_ocp",
			"rhsusf_ach_helmet_headset_ocp_alt",
			"rhsusf_ach_helmet_headset_ucp",
			"rhsusf_ach_helmet_headset_ucp_alt",
			"rhsusf_ach_helmet_ESS_ocp",
			"rhsusf_ach_helmet_ESS_ocp_alt",
			"rhsusf_ach_helmet_ESS_ucp",
			"rhsusf_ach_helmet_ESS_ucp_alt",
			"rhsusf_ach_helmet_headset_ess_ocp",
			"rhsusf_ach_helmet_headset_ess_ocp_alt",
			"rhsusf_ach_helmet_headset_ess_ucp",
			"rhsusf_ach_helmet_headset_ess_ucp_alt",
			"rhsusf_ach_bare",
			"rhsusf_ach_bare_ess",
			"rhsusf_ach_bare_headset",
			"rhsusf_ach_bare_headset_ess",
			"rhsusf_ach_bare_tan",
			"rhsusf_ach_bare_tan_ess",
			"rhsusf_ach_bare_tan_headset",
			"rhsusf_ach_bare_tan_headset_ess",
			"rhsusf_ach_bare_wood",
			"rhsusf_ach_bare_wood_ess",
			"rhsusf_ach_bare_wood_headset",
			"rhsusf_ach_bare_wood_headset_ess",
			"rhsusf_ach_bare_des",
			"rhsusf_ach_bare_des_ess",
			"rhsusf_ach_bare_des_headset",
			"rhsusf_ach_bare_des_headset_ess",
			"rhsusf_ach_bare_semi",
			"rhsusf_ach_bare_semi_ess",
			"rhsusf_ach_bare_semi_headset",
			"rhsusf_ach_bare_semi_headset_ess",
			"rhsusf_opscore_01",
			"rhsusf_opscore_fg",
			"rhsusf_opscore_fg_pelt",
			"rhsusf_opscore_fg_pelt_nsw",
			"rhsusf_opscore_fg_pelt_cam",
			"rhsusf_opscore_ut",
			"rhsusf_opscore_ut_pelt",
			"rhsusf_opscore_ut_pelt_cam",
			"rhsusf_opscore_ut_pelt_nsw",
			"rhsusf_opscore_ut_pelt_nsw_cam",
			"rhsusf_opscore_bk",
			"rhsusf_opscore_bk_pelt",
			"rhsusf_opscore_mc",
			"rhsusf_opscore_mc_pelt",
			"rhsusf_opscore_mc_pelt_nsw",
			"rhsusf_opscore_aor1",
			"rhsusf_opscore_aor1_pelt",
			"rhsusf_opscore_aor1_pelt_nsw",
			"rhsusf_opscore_aor2",
			"rhsusf_opscore_aor2_pelt",
			"rhsusf_opscore_aor2_pelt_nsw",
			"rhsusf_opscore_paint",
			"rhsusf_opscore_paint_pelt",
			"rhsusf_opscore_paint_pelt_nsw",
			"rhsusf_opscore_paint_pelt_nsw_cam",
			"rhsusf_opscore_cover",
			"rhsusf_opscore_mc_cover",
			"rhsusf_opscore_mc_cover_pelt",
			"rhsusf_opscore_mc_cover_pelt_nsw",
			"rhsusf_opscore_mc_cover_pelt_cam",
			"rhsusf_opscore_rg_cover",
			"rhsusf_opscore_rg_cover_pelt",
			"rhsusf_opscore_coy_cover",
			"rhsusf_opscore_coy_cover_pelt",
			"rhsusf_opscore_mar_01",
			"rhsusf_opscore_mar_ut",
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_opscore_mar_fg",
			"rhsusf_opscore_mar_fg_pelt",
			"rhsusf_opscore_01_tan",
			"rhsusf_opscore_02",
			"rhsusf_opscore_02_tan",
			"rhsusf_opscore_03_ocp",
			"rhsusf_opscore_04_ocp",
			"rhsusf_cvc_helmet",
			"rhsusf_cvc_alt_helmet",
			"rhsusf_cvc_green_helmet",
			"rhsusf_cvc_green_alt_helmet",
			"rhsusf_cvc_ess",
			"rhsusf_cvc_green_ess",
			"rhsusf_hgu56p",
			"rhsusf_hgu56p_visor",
			"rhsusf_hgu56p_mask",
			"rhsusf_hgu56p_visor_mask",
			"rhsusf_hgu56p_visor_mask_skull",
			"rhsusf_hgu56p_mask_skull",
			"rhsusf_hgu56p_visor_mask_mo",
			"rhsusf_hgu56p_mask_mo",
			"rhsusf_hgu56p_mask_smiley",
			"rhsusf_hgu56p_visor_mask_smiley",
			"rhsusf_hgu56p_black",
			"rhsusf_hgu56p_visor_black",
			"rhsusf_hgu56p_mask_black",
			"rhsusf_hgu56p_visor_mask_black",
			"rhsusf_hgu56p_visor_mask_black_skull",
			"rhsusf_hgu56p_mask_black_skull",
			"rhsusf_hgu56p_visor_mask_Empire_black",
			"rhsusf_hgu56p_green",
			"rhsusf_hgu56p_visor_green",
			"rhsusf_hgu56p_mask_green",
			"rhsusf_hgu56p_visor_mask_green",
			"rhsusf_hgu56p_mask_green_mo",
			"rhsusf_hgu56p_visor_mask_green_mo",
			"rhsusf_hgu56p_tan",
			"rhsusf_hgu56p_visor_tan",
			"rhsusf_hgu56p_mask_tan",
			"rhsusf_hgu56p_visor_mask_tan",
			"rhsusf_hgu56p_pink",
			"rhsusf_hgu56p_visor_pink",
			"rhsusf_hgu56p_mask_pink",
			"rhsusf_hgu56p_visor_mask_pink",
			"rhsusf_hgu56p_usa",
			"rhsusf_hgu56p_visor_usa",
			"rhsusf_hgu56p_saf",
			"rhsusf_hgu56p_visor_saf",
			"rhsusf_hgu56p_mask_saf",
			"rhsusf_hgu56p_visor_mask_saf",
			"rhsusf_ANPVS_14",
			"rhsusf_ANPVS_15",
			"rhsusf_Rhino",
			"rhsusf_anvis_nvg_bc_caps",
			"rhsusf_ANVIS",
			"rhsusf_mich_helmet_marpatwd",
			"rhsusf_mich_helmet_marpatwd_headset",
			"rhsusf_mich_helmet_marpatd",
			"rhsusf_mich_helmet_marpatd_headset",
			"rhsusf_mich_helmet_marpatwd_alt",
			"rhsusf_mich_helmet_marpatwd_alt_headset",
			"rhsusf_mich_helmet_marpatd_alt",
			"rhsusf_mich_helmet_marpatd_alt_headset",
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_mich_helmet_marpatwd_norotos_headset",
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_mich_helmet_marpatd_norotos_headset",
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_mich_helmet_marpatwd_norotos_arc_headset",
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_mich_helmet_marpatd_norotos_arc_headset",
			"rhsusf_mich_bare",
			"rhsusf_mich_bare_headset",
			"rhsusf_mich_bare_alt",
			"rhsusf_mich_bare_norotos",
			"rhsusf_mich_bare_norotos_headset",
			"rhsusf_mich_bare_norotos_alt",
			"rhsusf_mich_bare_norotos_alt_headset",
			"rhsusf_mich_bare_norotos_arc",
			"rhsusf_mich_bare_norotos_arc_headset",
			"rhsusf_mich_bare_norotos_arc_alt",
			"rhsusf_mich_bare_norotos_arc_alt_headset",
			"rhsusf_mich_bare_tan",
			"rhsusf_mich_bare_tan_headset",
			"rhsusf_mich_bare_alt_tan",
			"rhsusf_mich_bare_norotos_tan",
			"rhsusf_mich_bare_norotos_tan_headset",
			"rhsusf_mich_bare_norotos_alt_tan",
			"rhsusf_mich_bare_norotos_alt_tan_headset",
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_mich_bare_norotos_arc_tan_headset",
			"rhsusf_mich_bare_norotos_arc_alt_tan",
			"rhsusf_mich_bare_norotos_arc_alt_tan_headset",
			"rhsusf_mich_bare_semi",
			"rhsusf_mich_bare_semi_headset",
			"rhsusf_mich_bare_alt_semi",
			"rhsusf_mich_bare_norotos_semi",
			"rhsusf_mich_bare_norotos_semi_headset",
			"rhsusf_mich_bare_norotos_alt_semi",
			"rhsusf_mich_bare_norotos_alt_semi_headset",
			"rhsusf_mich_bare_norotos_arc_semi",
			"rhsusf_mich_bare_norotos_arc_semi_headset",
			"rhsusf_mich_bare_norotos_arc_alt_semi",
			"rhsusf_mich_bare_norotos_arc_alt_semi_headset",
			"rhsusf_ach_helmet_ocp",
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_lwh_helmet_marpatwd_blk_ess",
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_lwh_helmet_marpatwd_headset_blk",
			"rhsusf_lwh_helmet_M1942",
			"rhsusf_Bowman",
			"rhsusf_bowman_cap",
			"rhsusf_protech_helmet",
			"rhsusf_protech_helmet_ess",
			"rhsusf_protech_helmet_rhino",
			"rhsusf_protech_helmet_rhino_ess",
			"rhsusf_iotv_ocp",
			"rhsusf_iotv_ocp_Grenadier",
			"rhsusf_iotv_ocp_Medic",
			"rhsusf_iotv_ocp_Repair",
			"rhsusf_iotv_ocp_Rifleman",
			"rhsusf_iotv_ocp_SAW",
			"rhsusf_iotv_ocp_Squadleader",
			"rhsusf_iotv_ocp_Teamleader",
			"rhsusf_iotv_ucp",
			"rhsusf_iotv_ucp_Grenadier",
			"rhsusf_iotv_ucp_Medic",
			"rhsusf_iotv_ucp_Repair",
			"rhsusf_iotv_ucp_Rifleman",
			"rhsusf_iotv_ucp_SAW",
			"rhsusf_iotv_ucp_Squadleader",
			"rhsusf_iotv_ucp_Teamleader",
			"rhsusf_spcs_ocp_squadleader",
			"rhsusf_spcs_ocp_teamleader",
			"rhsusf_spcs_ocp_teamleader_alt",
			"rhsusf_spcs_ocp_saw",
			"rhsusf_spcs_ocp_grenadier",
			"rhsusf_spcs_ocp_rifleman",
			"rhsusf_spcs_ocp_rifleman_alt",
			"rhsusf_spcs_ocp_medic",
			"rhsusf_spcs_ocp_crewman",
			"rhsusf_spcs_ocp_machinegunner",
			"rhsusf_spcs_ocp_sniper",
			"rhsusf_spcs_ucp",
			"rhsusf_spcs_ucp_squadleader",
			"rhsusf_spcs_ucp_teamleader",
			"rhsusf_spcs_ucp_teamleader_alt",
			"rhsusf_spcs_ucp_saw",
			"rhsusf_spcs_ucp_grenadier",
			"rhsusf_spcs_ucp_rifleman",
			"rhsusf_spcs_ucp_rifleman_alt",
			"rhsusf_spcs_ucp_medic",
			"rhsusf_spcs_ucp_crewman",
			"rhsusf_spcs_ucp_machinegunner",
			"rhsusf_spcs_ucp_sniper",
			"rhsusf_mbav",
			"rhsusf_mbav_light",
			"rhsusf_mbav_rifleman",
			"rhsusf_mbav_mg",
			"rhsusf_mbav_grenadier",
			"rhsusf_mbav_medic",
			"rhsusf_plateframe_sapi"
		};
		name="RHSUSF Infantry & Equipment";
	};
};
class CfgFunctions
{
	class RHS
	{
		tag="RHS";
		class functions
		{
			class nvgHandler
			{
				file="\rhsusf\addons\rhsusf_c_troops\scripts\rhs_nvghandler.sqf";
				description="NVG equip";
			};
		};
	};
	class RHSUSF
	{
		tag="RHS";
		class functions
		{
			class nvgHandler
			{
				file="\rhsusf\addons\rhsusf_c_troops\scripts\rhs_nvghandler.sqf";
				description="NVG equip";
			};
		};
	};
};
class CfgGroups
{
	class West
	{
		class rhs_faction_usarmy_d
		{
			name="$STR_RHSUSF_FAC_USARMY_D";
			class rhs_group_nato_usarmy_d_infantry
			{
				name="$STR_RHS_GROUP_USARMY_infantry";
				aliveCategory="Infantry";
				class rhs_group_nato_usarmy_d_company_hq
				{
					name="$STR_RHSUSF_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_officer";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman_m4";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_platoon_hq
				{
					name="$STR_RHSUSF_GROUPS_PLATOON_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_officer";
						rank="LIEUTENANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_medic";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman_m4";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman_m4";
						rank="PRIVATE";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_weaponsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_AA
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_AT
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_medic";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_infantry_arb
			{
				name="$STR_RHS_GROUP_USARMY_infantry_arb";
				aliveCategory="Infantry";
				class rhs_group_nato_usarmy_d_company_hq
				{
					name="$STR_RHSUSF_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_officer";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_squadleader";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_rifleman";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_platoon_hq
				{
					name="$STR_RHSUSF_GROUPS_PLATOON_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_officer";
						rank="LIEUTENANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_squadleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_medic";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_squadleader";
						rank="SERGEANT";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_rifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_rifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_weaponsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_machinegunnera";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_machinegunnera";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_riflemanat";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_riflemanat";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_machinegunner";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_machinegunnera";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_AA
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_AT
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_arb_medic";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_RG33
			{
				name="$STR_RHS_GROUP_USARMY_RG33";
				aliveCategory="Motorized";
				class rhs_group_nato_usarmy_d_RG33_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_RG33_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_RG33_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_RG33_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_RG33_m2_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_RG33_m2_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_RG33_m2_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_RG33_m2_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_FMTV
			{
				name="$STR_RHS_GROUP_USARMY_FMTV";
				aliveCategory="Motorized";
				class rhs_group_nato_usarmy_d_FMTV_1078_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_FMTV_1078_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_FMTV_1078_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_FMTV_1078_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_FMTV_1083_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_FMTV_1083_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_FMTV_1083_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_FMTV_1083_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_d_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_HMMWV
			{
				name="$STR_RHS_GROUP_USARMY_HMMWV";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (GMG)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d_mk19";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (HMG)";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d_m2";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Anti-armor Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhs_group_nato_usarmy_d_M113
			{
				name="$STR_RHS_GROUP_USARMY_M113";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_d_M113_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113d_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_M113_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113d_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_M113_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113d_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_M113_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113d_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_bradley
			{
				name="$STR_RHS_GROUP_USARMY_Bradley";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_d_bradley_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_bradley_busk
			{
				name="$STR_RHS_GROUP_USARMY_Bradley_busk";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_d_bradley_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_bradleyA3
			{
				name="$STR_RHS_GROUP_USARMY_bradleyA3";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_d_bradleyA3_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_bradleyA3_busk
			{
				name="$STR_RHS_GROUP_USARMY_bradleyA3_busk";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_d_bradleyA3_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_bradleyA3_busk3
			{
				name="$STR_RHS_GROUP_USARMY_bradleyA3_busk3";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_d_bradleyA3_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_stryker
			{
				name="$STR_RHS_GROUP_USARMY_STRYKER";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_d_stryker_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_stryker_m1126_m2_d";
						rank="CORPORAL";
						position[]={0,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_autorifleman";
						rank="PRIVATE";
						position[]={-7,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ocp_grenadier";
						rank="PRIVATE";
						position[]={-9,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ocp_rifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ocp_javelin";
						rank="PRIVATE";
						position[]={-11,0,0};
					};
				};
				class rhs_group_nato_usarmy_d_stryker_wpnsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ocp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_stryker_m1126_m2_d";
						rank="CORPORAL";
						position[]={0,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={5,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunner";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ocp_machinegunnera";
						rank="PRIVATE";
						position[]={-7,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_d_m1a1
			{
				name="M1A1 Groups";
				aliveCategory="Armored";
				class RHS_M1A1AIM_Platoon
				{
					name="M1A1SA Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_Platoon_AA
				{
					name="M1A1SA Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_Section
				{
					name="M1A1SA Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aimd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_M1A1AIM_TUSK_Platoon
				{
					name="M1A1SA (TUSK I) Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_TUSK_Platoon_AA
				{
					name="M1A1SA (TUSK I) Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_TUSK_Section
				{
					name="M1A1SA (TUSK I) Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_d";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
			class rhs_group_nato_usarmy_d_M1A2
			{
				name="M1A2 Groups";
				aliveCategory="Armored";
				class RHS_M1A2SEP_Platoon
				{
					name="M1A2SEPv1 Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_Platoon_AA
				{
					name="M1A2SEPv1 Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_Section
				{
					name="M1A2SEPv1 Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1d_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_M1A2SEP_TUSK_Platoon
				{
					name="M1A2SEPv1 (TUSK I) Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_TUSK_Platoon_AA
				{
					name="M1A2SEPv1 (TUSK I) Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_TUSK_Section
				{
					name="M1A2SEPv1 (TUSK I) Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskid_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_M1A2SEP_d_TUSK2_Platoon
				{
					name="M1A2SEPv1 (TUSK II) Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_d_TUSK2_Platoon_AA
				{
					name="M1A2SEPv1 (TUSK II) Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_d_TUSK2_Section
				{
					name="M1A2SEPv1 (TUSK II) Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiid_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
			class rhs_group_nato_usarmy_d_M109
			{
				name="M109 Artillery";
				aliveCategory="Artillery";
				class RHS_M109_Platoon
				{
					name="Artillery M109 Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_art.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m109d_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m109d_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m109d_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m109d_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M109_Section
				{
					name="Artillery M109 Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_art.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m109d_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m109d_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
			};
		};
		class rhs_faction_usarmy_wd
		{
			name="$STR_RHSUSF_FAC_USARMY_WD";
			class rhs_group_nato_usarmy_wd_infantry
			{
				name="$STR_RHS_GROUP_USARMY_infantry";
				aliveCategory="Infantry";
				class rhs_group_nato_usarmy_wd_company_hq
				{
					name="$STR_RHSUSF_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_officer";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman_m4";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_platoon_hq
				{
					name="$STR_RHSUSF_GROUPS_PLATOON_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_officer";
						rank="LIEUTENANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_medic";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman_m4";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman_m4";
						rank="PRIVATE";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_weaponsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_AA
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_medic";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_heavy_AT
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_infantry_arb
			{
				name="$STR_RHS_GROUP_USARMY_infantry_arb";
				aliveCategory="Infantry";
				class rhs_group_nato_usarmy_wd_company_hq
				{
					name="$STR_RHSUSF_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_officer";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_squadleader";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_rifleman";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_platoon_hq
				{
					name="$STR_RHSUSF_GROUPS_PLATOON_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_officer";
						rank="LIEUTENANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_medic";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_squadleader";
						rank="SERGEANT";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_rifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_rifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_weaponsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_machinegunnera";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_machinegunnera";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_riflemanat";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_riflemanat";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_machinegunner";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_machinegunnera";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_AA
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_medic";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_infantry_team_heavy_AT
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_arb_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_RG33
			{
				name="$STR_RHS_GROUP_USARMY_RG33";
				aliveCategory="Motorized";
				class rhs_group_nato_usarmy_wd_RG33_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_RG33_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_RG33_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_RG33_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_RG33_m2_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_RG33_m2_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_RG33_m2_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_RG33_m2_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1232_M2_usarmy_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_FMTV
			{
				name="$STR_RHS_GROUP_USARMY_FMTV";
				aliveCategory="Motorized";
				class rhs_group_nato_usarmy_wd_FMTV_1078_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_FMTV_1078_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_FMTV_1078_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_FMTV_1078_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_1078";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1078A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_FMTV_1083_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_FMTV_1083_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_FMTV_1083_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_FMTV_1083_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_1083";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_M1083A1P2_wd_fmtv_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_HMMWV
			{
				name="$STR_RHS_GROUP_USARMY_HMMWV";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (GMG)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w_mk19";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (HMG)";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w_m2";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Anti-armor Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_M113
			{
				name="$STR_RHS_GROUP_USARMY_M113";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_wd_M113_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_M113_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_M113_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_M113_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m113_usarmy";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_bradley
			{
				name="$STR_RHS_GROUP_USARMY_Bradley";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_wd_bradley_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradley_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradley_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradley_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_bradley_busk
			{
				name="$STR_RHS_GROUP_USARMY_Bradley_busk";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_wd_bradley_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradley_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradley_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradley_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A2_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradley_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_bradleyA3
			{
				name="$STR_RHS_GROUP_USARMY_bradleyA3";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_wd_bradleyA3_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_bradleyA3_busk
			{
				name="$STR_RHS_GROUP_USARMY_bradleyA3_busk";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_wd_bradleyA3_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKI_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_bradleyA3_busk3
			{
				name="$STR_RHS_GROUP_USARMY_bradleyA3_Busk3";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_wd_bradleyA3_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_wd_bradleyA3_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M2A3_BUSKIII_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_marksman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
				class rhs_group_nato_usarmy_d_bradleyA3_aa
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={16,-14,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={-16,-16,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={27,-27,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-27,-27,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={31,-31,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-31,-31,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={37,-37,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_aa";
						rank="PRIVATE";
						position[]={-37,-37,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={40,-40,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={-40,-40,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_stryker
			{
				name="$STR_RHS_GROUP_USARMY_STRYKER";
				aliveCategory="Mechanized";
				class rhs_group_nato_usarmy_wd_stryker_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_stryker_m1126_m2_d";
						rank="CORPORAL";
						position[]={0,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_autorifleman";
						rank="PRIVATE";
						position[]={-7,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_army_ucp_grenadier";
						rank="PRIVATE";
						position[]={-9,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_army_ucp_rifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_army_ucp_javelin";
						rank="PRIVATE";
						position[]={-11,0,0};
					};
				};
				class rhs_group_nato_usarmy_wd_stryker_wpnsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_army_ucp_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_stryker_m1126_m2_d";
						rank="CORPORAL";
						position[]={0,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={5,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunner";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_army_ucp_machinegunnera";
						rank="PRIVATE";
						position[]={-7,0,0};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_m1a1
			{
				name="M1A1 Groups";
				aliveCategory="Armored";
				class RHS_M1A1AIM_wd_Platoon
				{
					name="M1A1SA Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_wd_Platoon_AA
				{
					name="M1A1SA Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_wd_Section
				{
					name="M1A1SA Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aimwd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_M1A1AIM_wd_TUSK_Platoon
				{
					name="M1A1SA (TUSK I) Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_wd_TUSK_Platoon_AA
				{
					name="M1A1SA (TUSK I) Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1AIM_wd_TUSK_Section
				{
					name="M1A1SA (TUSK I) Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1aim_tuski_wd";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_M1A2
			{
				name="M1A2 Groups";
				aliveCategory="Armored";
				class RHS_M1A2SEP_wd_Platoon
				{
					name="M1A2SEPv1 Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_wd_Platoon_AA
				{
					name="M1A2SEPv1 Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_wd_Section
				{
					name="M1A2SEPv1 Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1wd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_M1A2SEP_wd_TUSK_Platoon
				{
					name="M1A2SEPv1 (TUSK I) Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_wd_TUSK_Platoon_AA
				{
					name="M1A2SEPv1 (TUSK I) Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_wd_TUSK_Section
				{
					name="M1A2SEPv1 (TUSK I) Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiwd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_M1A2SEP_wd_TUSK2_Platoon
				{
					name="M1A2SEPv1 (TUSK II) Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_wd_TUSK2_Platoon_AA
				{
					name="M1A2SEPv1 (TUSK II) Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="RHS_M6_wd";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A2SEP_wd_TUSK2_Section
				{
					name="M1A2SEPv1 (TUSK II) Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a2sep1tuskiiwd_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
			class rhs_group_nato_usarmy_wd_M109
			{
				name="M109 Artillery";
				aliveCategory="Artillery";
				class RHS_M109_wd_Platoon
				{
					name="Artillery M109 Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_art.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m109_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m109_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m109_usarmy";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m109_usarmy";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M109_wd_Section
				{
					name="Artillery M109 Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_art.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m109_usarmy";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m109_usarmy";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
			};
		};
		class rhs_faction_usmc_d
		{
			name="$STR_RHSUSF_FAC_USMC_D";
			class rhs_group_nato_usmc_d_infantry
			{
				name="$STR_RHS_GROUP_USARMY_infantry";
				aliveCategory="Infantry";
				class rhs_group_nato_usmc_d_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="CORPORAL";
						position[]={17,0,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={19,0,0};
					};
					class Unit11
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={21,0,0};
					};
					class Unit12
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_marksman";
						rank="PRIVATE";
						position[]={23,0,0};
					};
				};
				class rhs_group_nato_usmc_d_infantry_weaponsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usmc_d_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="CORPORAL";
						position[]={11,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usmc_d_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_riflemanat";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_d_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_d_infantry_team_AA
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_stinger";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_d_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_navy_marpat_d_medic";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_d_infantry_team_heavy_AT
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_javelin";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_nato_usmc_recon_d_infantry
			{
				name="$STR_RHSUSF_VEHCLASS_INFANTRY_RECON";
				aliveCategory="Infantry";
				class rhs_group_nato_usmc_recon_d_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_marksman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_autorifleman";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman_at";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_machinegunner_m249";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_d_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_marksman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_autorifleman";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman_at";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_d_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_marksman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_autorifleman";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_navy_sarc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_machinegunner_m249";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_d_infantry_team_lite
				{
					name="$STR_RHSUSF_GROUPS_TEAM_LITE";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_teamleader_lite";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_marksman_lite";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_autorifleman_lite";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman_at_lite";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman_lite";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_machinegunner_m249_lite";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_d_infantry_team_fast
				{
					name="$STR_RHSUSF_GROUPS_TEAM_FAST";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_teamleader_fast";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_marksman_fast";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_autorifleman_fast";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman_at_fast";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_rifleman_fast";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_d_machinegunner_m249_fast";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
			};
			class rhs_group_nato_usmc_d_RG33
			{
				name="$STR_RHS_GROUP_USARMY_RG33";
				aliveCategory="Motorized";
				class rhs_group_nato_usmc_d_RG33_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_d_RG33_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_d_RG33_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_d_RG33_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_d_RG33_m2_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_d_RG33_m2_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_d_RG33_m2_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_d_RG33_m2_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_d";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
			class rhs_group_nato_usmc_d_HMMWV
			{
				name="$STR_RHS_GROUP_USARMY_HMMWV";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (GMG)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d_s_Mk19";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (HMG)";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d_s_m2";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Anti-armor Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d_s";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_javelin";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_javelin";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_d_s";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_rifleman";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_stinger";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_d_stinger";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhs_group_nato_usmc_d_m1a1
			{
				name="M1A1 Groups";
				aliveCategory="Armored";
				class RHS_M1A1AIM_d_Platoon
				{
					name="M1A1FEP Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1fep_d";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1fep_d";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1fep_d";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a1fep_d";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1FEP_d_Section
				{
					name="M1A1FEP Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1fep_d";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1fep_d";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
		};
		class rhs_faction_usmc_wd
		{
			name="$STR_RHSUSF_FAC_USMC_WD";
			class rhs_group_nato_usmc_wd_infantry
			{
				name="$STR_RHS_GROUP_USARMY_infantry";
				aliveCategory="Infantry";
				class rhs_group_nato_usmc_wd_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_riflemanat";
						rank="PRIVATE";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="CORPORAL";
						position[]={17,0,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={19,0,0};
					};
					class Unit11
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={21,0,0};
					};
					class Unit12
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_marksman";
						rank="PRIVATE";
						position[]={23,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_infantry_weaponsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="CORPORAL";
						position[]={11,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_riflemanat";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_infantry_team_AA
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_stinger";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_navy_marpat_wd_medic";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_infantry_team_heavy_AT
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman_m4";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_javelin";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_nato_usmc_recon_wd_infantry
			{
				name="$STR_RHSUSF_VEHCLASS_INFANTRY_RECON";
				aliveCategory="Infantry";
				class rhs_group_nato_usmc_recon_wd_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_marksman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_autorifleman";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman_at";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_machinegunner_m249";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_wd_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_marksman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_autorifleman";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman_at";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_wd_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_hq.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_marksman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_autorifleman";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_navy_sarc_w";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_machinegunner_m249";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_wd_infantry_team_lite
				{
					name="$STR_RHSUSF_GROUPS_TEAM_LITE";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_teamleader_lite";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_marksman_lite";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_autorifleman_lite";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman_at_lite";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman_lite";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_machinegunner_m249_lite";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
				class rhs_group_nato_usmc_recon_wd_infantry_team_fast
				{
					name="$STR_RHSUSF_GROUPS_TEAM_FAST";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_recon.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_teamleader_fast";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_marksman_fast";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_autorifleman_fast";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman_at_fast";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_rifleman_fast";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_recon_marpat_wd_machinegunner_m249_fast";
						rank="CORPORAL";
						position[]={9,0,0};
					};
				};
			};
			class rhs_group_nato_usmc_wd_HMMWV
			{
				name="$STR_RHS_GROUP_USARMY_HMMWV";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (GMG)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w_s_Mk19";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (HMG)";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w_s_m2";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_riflemanat";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Anti-armor Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w_s";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_javelin";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_javelin";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1025_w_s";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_rifleman";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_stinger";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_stinger";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhs_group_nato_usmc_wd_RG33
			{
				name="$STR_RHS_GROUP_USARMY_RG33";
				aliveCategory="Motorized";
				class rhs_group_nato_usmc_wd_RG33_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_RG33_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_RG33_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_RG33_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_army_ocp_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_RG33_m2_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_riflemanat";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_marksman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_RG33_m2_squad_2mg
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_2MG_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_RG33_m2_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_autorifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_nato_usmc_wd_RG33_m2_squad_mg_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_MG_SNIPER_RG33_M2";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_rg33_m2_usmc_wd";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_machinegunner_ass";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_sniper";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_usmc_marpat_wd_spotter";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
			class rhs_group_nato_usmc_wd_m1a1
			{
				name="M1A1 Groups";
				aliveCategory="Armored";
				class RHS_M1A1AIM_wd_Platoon
				{
					name="M1A1FEP Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1fep_wd";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1fep_wd";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_m1a1fep_wd";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_m1a1fep_wd";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
				};
				class RHS_M1A1FEP_wd_Section
				{
					name="M1A1FEP Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsusf_m1a1fep_wd";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_m1a1fep_wd";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
		};
		class rhs_faction_socom_marsoc
		{
			name="$STR_RHSUSF_FAC_SOCOM";
			class rhs_group_nato_marsoc_infantry
			{
				name="$STR_RHSUSF_VEHCLASS_MARSOC";
				aliveCategory="Infantry";
				class rhs_group_nato_marsoc_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_MSOT";
					side=1;
					rarityGroup=0.75;
					class Unit0
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_teamleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_teamchief";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_elementleader";
						rank="SERGEANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso_breacher";
						rank="CORPORAL";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso_mechanic";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_sarc";
						rank="CORPORAL";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_elementleader";
						rank="SERGEANT";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso";
						rank="CORPORAL";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso_breacher";
						rank="CORPORAL";
						position[]={17,0,0};
					};
					class Unit10
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso_mechanic";
						rank="CORPORAL";
						position[]={19,0,0};
					};
					class Unit11
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_sarc";
						rank="CORPORAL";
						position[]={21,0,0};
					};
				};
				class rhs_group_nato_marsoc_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_MSOT_ELEMENT";
					side=1;
					rarityGroup=0.75;
					class Unit0
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_elementleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso_breacher";
						rank="CORPORAL";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_cso_mechanic";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_sarc";
						rank="CORPORAL";
						position[]={7,0,0};
					};
				};
				class rhs_group_nato_marsoc_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_MSOT_SNIPER_ELEMENT";
					side=1;
					rarityGroup=0.75;
					class Unit0
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_sniper";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsusf_socom_marsoc_spotter";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
				};
			};
		};
	};
};
class UniformSlotInfo;
class CfgVehicles
{
	class B_AssaultPack_Base;
	class rhsusf_assault_eagleaiii_ucp: B_AssaultPack_Base
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\backpacks\data\eagleaiii_ucp_co.paa"
		};
		maximumLoad=240;
		mass=35;
	};
	class rhsusf_assault_eagleaiii_ocp: rhsusf_assault_eagleaiii_ucp
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\backpacks\data\eagleaii_ocp_co.paa"
		};
	};
	class rhsusf_assault_eagleaiii_ocp_engineer: rhsusf_assault_eagleaiii_ocp
	{
		maximumLoad=160;
		mass=20;
	};
	class rhsusf_assault_eagleaiii_ucp_engineer: rhsusf_assault_eagleaiii_ucp
	{
	};
	class rhsusf_assault_eagleaiii_ocp_medic: rhsusf_assault_eagleaiii_ocp
	{
	};
	class rhsusf_assault_eagleaiii_ucp_medic: rhsusf_assault_eagleaiii_ucp
	{
	};
	class rhsusf_assault_eagleaiii_ocp_demo: rhsusf_assault_eagleaiii_ocp
	{
	};
	class rhsusf_assault_eagleaiii_ucp_demo: rhsusf_assault_eagleaiii_ucp
	{
	};
	class rhsusf_assault_eagleaiii_ocp_ar: rhsusf_assault_eagleaiii_ocp
	{
	};
	class rhsusf_assault_eagleaiii_ucp_ar: rhsusf_assault_eagleaiii_ucp
	{
	};
	class rhsusf_assault_eagleaiii_ocp_mg: rhsusf_assault_eagleaiii_ocp
	{
	};
	class rhsusf_assault_eagleaiii_ucp_mg: rhsusf_assault_eagleaiii_ucp
	{
	};
	class rhsusf_assault_eagleaiii_ocp_at: rhsusf_assault_eagleaiii_ocp
	{
	};
	class rhsusf_assault_eagleaiii_ucp_at: rhsusf_assault_eagleaiii_ucp
	{
	};
	class rhsusf_assault_eagleaiii_ucp_maaws: rhsusf_assault_eagleaiii_ucp
	{
	};
	class rhsusf_assault_eagleaiii_ocp_maaws: rhsusf_assault_eagleaiii_ocp
	{
	};
	class rhsusf_assault_eagleaiii_coy: rhsusf_assault_eagleaiii_ucp
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\backpacks\data\eagleaii_coy_co.paa"
		};
	};
	class rhsusf_assault_eagleaiii_coy_engineer: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_demo: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_eod: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_m27: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_ar: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_mg: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_assaultman: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_at: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_assault_eagleaiii_coy_aa: rhsusf_assault_eagleaiii_coy
	{
	};
	class B_AssaultPack_rgr;
	class rhsusf_falconii: B_AssaultPack_rgr
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\backpacks\data\falconii_rgr_co.paa"
		};
	};
	class rhsusf_falconii_coy: B_AssaultPack_rgr
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\backpacks\data\falconii_coy_co.paa"
		};
	};
	class rhsusf_falconii_mc: B_AssaultPack_rgr
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\backpacks\data\falconii_mc_co.paa"
		};
	};
	class rhsusf_falconii_gr: rhsusf_falconii
	{
	};
	class rhsusf_falconii_coy_gr: rhsusf_falconii_coy
	{
	};
	class rhsusf_falconii_gr_m32: rhsusf_falconii_coy
	{
	};
	class rhsusf_falconii_breach: rhsusf_falconii
	{
	};
	class rhsusf_falconii_recon: rhsusf_falconii_coy
	{
	};
	class B_Carryall_cbr;
	class rhsusf_pack_slackman_m240: B_Carryall_cbr
	{
	};
	class rhsusf_pack_slackman_m249: B_Carryall_cbr
	{
	};
	class B_Parachute;
	class Steerable_Parachute_F;
	class rhsusf_eject_Parachute: Steerable_Parachute_F
	{
		side=0;
		maxgravity=-10;
		maxliftduration=4;
		maxliftthrust=-40;
		maxrotationx=0.1;
		maxrotationz=0.40000001;
		maxsensitivityhorizontal=0.050000001;
		maxsensitivityvertical=0.0099999998;
		mingravity=-2;
		minliftduration=4;
		minliftthrust=-40;
		minrotationx=-0.1;
		minrotationz=-0.40000001;
		normalgravity=-5;
		thrustaccel=0.001;
		thrustdeccel=0.001;
		thrustnormal=0.001;
		turnforcescale=9.9999997e-005;
	};
	class rhsusf_eject_Parachute_backpack: B_Parachute
	{
		ParachuteClass="rhsusf_eject_Parachute";
		maximumLoad=0;
		mass=130;
	};
	class Item_Base_F;
	class Item_rhs_uniform_cu_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_FROG01_d: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_FROG01_wd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_g3_mc: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_g3_m81: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_g3_aor2: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_g3_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_g3_rgr: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_g3_blk: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ocp_82nd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ocp_101st: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ocp_10th: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ucp_82nd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ucp_101st: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ucp_10th: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ocp_patchless: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_cu_ucp_patchless: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_acu_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_abu: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_uniform_bdu_erdl: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp_Grenadier: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp_Medic: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp_Repair: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp_Rifleman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp_SAW: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp_Squadleader: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ocp_teamleader: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp_Grenadier: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp_Medic: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp_Repair: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp_Rifleman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp_SAW: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp_Squadleader: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_iotv_ucp_teamleader: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_rifleman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_iar: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_corpsman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_crewman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_light: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_marksman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_sniper: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_mg: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_squadleader: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_spc_teamleader: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mbav: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mbav_light: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mbav_rifleman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mbav_mg: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mbav_grenadier: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mbav_medic: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_sapi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_light: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_rifleman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_teamleader: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_grenadier: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_machinegunner: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_marksman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_plateframe_medic: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_patrolcap_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_patrolcap_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_8point_marpatd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_8point_marpatwd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_m81: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_camo_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_headset_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_headset_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_ess_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_ess_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_headset_ess_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_headset_ess_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_ocp_norotos: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_helmet_ucp_norotos: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_headset_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_tan_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_tan_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_tan_headset_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_wood: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_wood_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_wood_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_wood_headset_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_des: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_des_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_des_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_des_headset_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_semi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_semi_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_semi_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ach_bare_semi_headset_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_ut: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_bk: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_mc_cover: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_rg_cover: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_coy_cover: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_mc_cover_pelt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_rg_cover_pelt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_coy_cover_pelt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_01: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_01_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_02: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_02_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_03_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_04_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_mar_ut: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_mar_ut_pelt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_mar_fg: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_opscore_mar_fg_pelt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_cvc_helmet: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_cvc_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_cvc_green_helmet: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_cvc_green_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_hgu56p: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_hgu56p_mask: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_ihadss: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_Booniehat_ocp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_Booniehat_ucp: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_Booniehat_marpatd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_Booniehat_marpatwd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_Booniehat_m81: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_booniehat2_marpatd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhs_booniehat2_marpatwd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_Bowman: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_Bowman_cap: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_lwh_helmet_marpatd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_lwh_helmet_marpatd_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_lwh_helmet_marpatd_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_lwh_helmet_marpatwd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_lwh_helmet_marpatwd_ess: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_lwh_helmet_marpatwd_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd_alt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd_alt_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd_alt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd_alt_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd_norotos: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd_norotos_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd_norotos: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd_norotos_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd_norotos_arc: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatwd_norotos_arc_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd_norotos_arc: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_helmet_marpatd_norotos_arc_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_alt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_alt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_alt_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_alt: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_alt_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_tan_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_alt_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_tan_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_alt_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_alt_tan_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_tan_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_alt_tan: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_alt_tan_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_semi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_semi_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_alt_semi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_semi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_semi_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_alt_semi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_alt_semi_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_semi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_semi_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_alt_semi: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_mich_bare_norotos_arc_alt_semi_headset: Item_Base_F
	{
		editorCategory="EdCat_Equipment";
	};
	class Item_rhsusf_acc_compm4: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_T1_high: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_T1_low: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_g33_t1: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_EOTECH: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_eotech_552: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_eotech_552_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_eotech_552_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_g33_xps3: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_g33_xps3_tan: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M2A1: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_RM05: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_mrds: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_mrds_c: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhs_weap_optic_smaw: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_LEUPOLDMK4: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_LEUPOLDMK4_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_LEUPOLDMK4_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_LEUPOLDMK4_2: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_LEUPOLDMK4_2_MRDS: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_LEUPOLDMK4_2_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M8541: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M8541_mrds: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M8541_low: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M8541_low_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M8541_low_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_premier: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_premier_mrds: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_premier_low: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ELCAN: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ELCAN_ard: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG2: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG3: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG_RMR: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG_USMC: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG2_USMC: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG3_USMC: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ACOG_MDO: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpas13gv1: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M2010S: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M2010S_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M2010S_sa: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M2010S_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_SR25S: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_SR25S_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_SR25S_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_aac_scarh_silencer: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_aac_762sd_silencer: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_aac_762sdn6_silencer: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_m24_silencer_black: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_m24_silencer_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_m24_silencer_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_m24_muzzlehider_black: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_m24_muzzlehider_d: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_m24_muzzlehider_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_rotex5_grey: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_rotex5_tan: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_nt4_black: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_nt4_tan: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_SF3P556: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_SFMB556: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_omega9k: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_M952V: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_wmx: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_wmx_bk: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq15side: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq15_top: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq15: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq15_light: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq15_wmx: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq15_wmx_light: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq15A: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq16a: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_anpeq16a_top: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_harris_bipod: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_harris_swivel: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_saw_bipod: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_saw_lw_bipod: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip1: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip2: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip2_tan: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip2_wd: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip3: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip3_tan: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip4: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_grip4_bipod: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_kac_grip: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_kac_grip_saw_bipod: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_rvg_blk: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_rvg_de: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_tacsac_blk: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_tacsac_tan: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_tacsac_blue: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_tdstubby_blk: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_tdstubby_tan: Item_Base_F
	{
		editorCategory="EdCat_WeaponAttachments";
	};
	class Item_rhsusf_acc_ELCAN_PIP: Item_rhsusf_acc_ELCAN
	{
	};
	class Item_rhsusf_acc_ACOG_PIP: Item_rhsusf_acc_ACOG
	{
	};
	class Land;
	class Man;
	class CAManBase: Man
	{
		class HitPoints;
	};
	class SoldierWB: CAManBase
	{
		class HitPoints: HitPoints
		{
			class HitFace;
			class HitNeck;
			class HitHead;
			class HitPelvis;
			class HitAbdomen;
			class HitDiaphragm;
			class HitChest;
			class HitBody;
			class HitArms;
			class HitHands;
			class HitLegs;
		};
		class EventHandlers;
	};
	class rhsusf_socom_uniform_base: SoldierWB
	{
		side=1;
		portrait="";
		icon="iconMan";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO",
			"rhsusf_g_army"
		};
		faceType="Man_A3";
		genericNames="NATOMen";
		cost=100000;
		accuracy=3;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		minFireTime=7;
		canCarryBackPack=1;
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		hiddenSelections[]=
		{
			"Camo",
			"Camo2",
			"Gloves",
			"insignia"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\Data\gen3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\merrells_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\mechanix_g_co.paa"
		};
		nakedUniform="U_BasicBody";
		uniformClass="rhs_uniform_g3_mc";
		class Wounds
		{
		};
		class EventHandlers: EventHandlers
		{
			class RHS_NVG_Check
			{
				init="_this call rhsusf_fnc_nvgHandler";
			};
		};
	};
	class rhsusf_socom_mc_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_g3_mc";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\Data\gen3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\merrells_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\Mechanix_tan_co.paa"
		};
	};
	class rhsusf_socom_g3_m81_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_g3_m81";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\Data\gen3_m81_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\merrells_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\Mechanix_black_co.paa"
		};
	};
	class rhsusf_socom_g3_aor2_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_g3_aor2";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\Data\gen3_aor2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\merrells_blk_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\mechanix_black_co.paa"
		};
	};
	class rhsusf_socom_g3_tan_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_g3_m81";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\Data\gen3_tan_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\merrells_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\Mechanix_tan2_co.paa"
		};
	};
	class rhsusf_socom_g3_rgr_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_g3_m81";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\Data\gen3_rgr_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\merrells_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\Mechanix_green_co.paa"
		};
	};
	class rhsusf_socom_g3_blk_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_g3_m81";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\Data\gen3_blk_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\Merrells_blk_co.paa",
			"rhsusf\addons\rhsusf_infantry2\data\Mechanix_black_co.paa"
		};
	};
	class rhsusf_army_acu_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_acu_ucp";
		class Wounds
		{
		};
		hiddenSelections[]=
		{
			"camo1",
			"camo2",
			"camo3",
			"identity",
			"insignia"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_01_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_02_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_03_ucp_co.paa"
		};
	};
	class rhsusf_army_acu_ucpd_uniform: rhsusf_army_acu_uniform
	{
		uniformClass="rhs_uniform_acu_ucpd";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_01_ucpd_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_02_ucpd_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_03_ucp_co.paa"
		};
	};
	class rhsusf_army_acu_oefcp_uniform: rhsusf_army_acu_uniform
	{
		uniformClass="rhs_uniform_acu_oefcp";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_01_oefcp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_02_oefcp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_03_ucp_co.paa"
		};
	};
	class rhsusf_army_acu_ocp_uniform: rhsusf_army_acu_uniform
	{
		uniformClass="rhs_uniform_acu_ocp";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_01_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_02_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\acu\data\rhsusf_uniform_acu_03_ucp_co.paa"
		};
	};
	class rhsusf_airforce_abu_uniform: rhsusf_socom_uniform_base
	{
		uniformClass="rhs_uniform_abu";
		class Wounds
		{
		};
		hiddenSelections[]=
		{
			"insignia"
		};
	};
	class rhsusf_army_bdu_erdl_uniform: rhsusf_airforce_abu_uniform
	{
		uniformClass="rhs_uniform_bdu_erdl";
		class Wounds
		{
		};
		hiddenSelections[]=
		{
			"insignia"
		};
	};
	class rhsusf_infantry_army_base: SoldierWB
	{
		side=1;
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO"
		};
		faceType="Man_A3";
		genericNames="NATOMen";
		portrait="";
		icon="iconMan";
		accuracy=2.3;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		minFireTime=7;
		canCarryBackPack=1;
		nakedUniform="U_BasicBody";
		uniformClass="rhs_uniform_cu_ocp";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		class Wounds
		{
		};
		weapons[]=
		{
			"rhs_m4a1_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_compm4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ocp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ocp"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		hiddenSelections[]=
		{
			"camo1",
			"camo2",
			"camo3",
			"insignia"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_pockets_co.paa"
		};
		class EventHandlers: EventHandlers
		{
			class RHS_NVG_Check
			{
				init="_this call rhsusf_fnc_nvgHandler";
			};
		};
	};
	class rhsusf_infantry_army_1stcav: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_cu_ocp_1stcav";
		hiddenSelections[]=
		{
			"camo1",
			"camo2",
			"camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\1CAV_ocp_co.paa"
		};
	};
	class rhsusf_infantry_army_10th: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_cu_ocp_10th";
		hiddenSelections[]=
		{
			"camo1",
			"camo2",
			"camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\10MT_ocp_co.paa"
		};
	};
	class rhsusf_army_ocp_rifleman_1stcav: rhsusf_infantry_army_1stcav
	{
	};
	class rhsusf_army_ocp_rifleman_10th: rhsusf_infantry_army_10th
	{
	};
	class rhsusf_army_ocp_rifleman: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			5,
			"rhsusf_ach_helmet_ocp_alt",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ocp",
			5,
			"rhsusf_ach_helmet_ESS_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_rifleman_m4: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		weapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_rifleman_m16: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		weapons[]=
		{
			"rhs_weap_m16a4_grip_acog",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_grip_acog",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_rifleman_m590: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		weapons[]=
		{
			"rhs_weap_M590_8RD",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M590_8RD",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ"
		};
		backpack="rhsusf_falconii_breach";
	};
	class rhsusf_army_ocp_riflemanl: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		camouflage=1.2;
		cost=90000;
		headgearList[]={};
		linkedItems[]=
		{
			"rhs_Booniehat_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_Booniehat_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_compm4",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_riflemanat: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=180000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		weapons[]=
		{
			"rhs_m4_grip_compm4",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_grip_compm4",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_m136_mag"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsusf_army_ocp_grenadier: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=130000;
		threat[]={1,0.30000001,0.1};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			5,
			"rhsusf_ach_helmet_ocp_alt",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ocp",
			5,
			"rhsusf_ach_helmet_ESS_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_iotv_ocp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_iotv_ocp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_m203_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_m203_compm4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_m67"
		};
		backpack="rhsusf_falconii_gr";
	};
	class rhsusf_army_ocp_autorifleman: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			5,
			"rhsusf_ach_helmet_ocp_alt",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ocp",
			5,
			"rhsusf_ach_helmet_ESS_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m249_light_L_elcan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m249_light_L_elcan",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
		backpack="rhsusf_assault_eagleaiii_ocp_ar";
	};
	class rhsusf_army_ocp_autoriflemana: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=110000;
		camouflage=1.5;
		weapons[]=
		{
			"rhs_m4_grip_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"Throw",
			"Put"
		};
		backpack="rhsusf_assault_eagleaiii_ocp_ar";
	};
	class rhsusf_army_ocp_machinegunner: rhsusf_army_ocp_autorifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		icon="iconManMG";
		weapons[]=
		{
			"rhs_weap_m240b_elcan",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m240b_elcan",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m67"
		};
		backpack="rhsusf_assault_eagleaiii_ocp_mg";
	};
	class rhsusf_army_ocp_machinegunnera: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=110000;
		camouflage=1.5;
		backpack="rhsusf_assault_eagleaiii_ocp_mg";
		weapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_marksman: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			5,
			"rhsusf_ach_helmet_ocp_alt",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ocp",
			5,
			"rhsusf_ach_helmet_ESS_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m14ebrri_leu",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m14ebrri_leu",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m62_Mag",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_officer: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_officer_s"
				};
				speechPlural[]=
				{
					"veh_infantry_officer_p"
				};
			};
		};
		nameSound="veh_infantry_officer_s";
		cost=600000;
		camouflage=1.6;
		sensitivity=3;
		icon="iconManOfficer";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			5,
			"rhsusf_ach_helmet_headset_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ocp",
			5,
			"rhsusf_ach_helmet_headset_ess_ocp_alt",
			2,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp_norotos",
			"rhsusf_iotv_ocp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp_norotos",
			"rhsusf_iotv_ocp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog2",
			"rhsusf_weap_m9",
			"Throw",
			"Put",
			"rhsusf_bino_m24"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog2",
			"rhsusf_weap_m9",
			"Throw",
			"Put",
			"rhsusf_bino_m24"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m18_red",
			"rhs_mag_m18_purple",
			"rhs_mag_m18_yellow",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_army_ocp_squadleader: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			5,
			"rhsusf_ach_helmet_headset_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ocp",
			5,
			"rhsusf_ach_helmet_headset_ess_ocp_alt",
			2,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ocp",
			"rhsusf_iotv_ocp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ocp",
			"rhsusf_iotv_ocp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog3",
			"Throw",
			"Put",
			"rhsusf_bino_m24_ARD"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog3",
			"Throw",
			"Put",
			"rhsusf_bino_m24_ARD"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m18_red",
			"rhs_mag_m18_purple",
			"rhs_mag_m18_yellow",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_army_ocp_teamleader: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=450000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			5,
			"rhsusf_ach_helmet_headset_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ocp",
			5,
			"rhsusf_ach_helmet_headset_ess_ocp_alt",
			2,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_iotv_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_iotv_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog2",
			"Throw",
			"Put",
			"rhsusf_bino_m24"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2",
			"Throw",
			"Put",
			"rhsusf_bino_m24"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m18_purple",
			"rhs_mag_m18_yellow",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_jfo: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		icon="iconManLeader";
		cost=400000;
		sensitivity=3.2;
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			5,
			"rhsusf_ach_helmet_ocp_alt",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ocp",
			5,
			"rhsusf_ach_helmet_ESS_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		weapons[]=
		{
			"rhs_m4a1_m203s_ACOG",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_m203s_ACOG",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_camo_ocp",
			"rhsusf_iotv_ocp_grenadier",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_camo_ocp",
			"rhsusf_iotv_ocp_grenadier",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Red",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Red",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Red",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_M585_white",
			"rhs_mag_M715_green",
			"rhs_mag_M661_green",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_purple",
			"Chemlight_green",
			"I_IR_Grenade"
		};
		backpack="rhsusf_falconii_gr";
	};
	class rhsusf_army_ocp_fso: rhsusf_army_ocp_jfo
	{
		uniformClass="rhs_uniform_acu_oefcp";
		icon="iconManLeader";
		cost=200000;
		sensitivity=3.2;
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			5,
			"rhsusf_ach_helmet_headset_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ocp",
			5,
			"rhsusf_ach_helmet_headset_ess_ocp_alt",
			2,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ocp",
			"rhsusf_iotv_ocp_grenadier",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ocp",
			"rhsusf_iotv_ocp_grenadier",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_javelin: rhsusf_army_ocp_riflemanat
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=190000;
		threat[]={1,1,0.80000001};
		camouflage=1.5;
		icon="iconManAT";
		weapons[]=
		{
			"rhs_m4_compm4",
			"rhs_weap_fgm148",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"rhs_weap_fgm148",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_fgm148_magazine_AT",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_javelin_assistant: rhsusf_army_ocp_riflemanat
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=190000;
		threat[]={1,1,0.80000001};
		camouflage=1.5;
		icon="iconManAT";
		backpack="rhsusf_assault_eagleaiii_ocp_at";
		weapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_maaws: rhsusf_army_ocp_riflemanat
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=190000;
		threat[]={1,1,0.80000001};
		camouflage=1.5;
		icon="iconManAT";
		backpack="rhsusf_assault_eagleaiii_ocp_maaws";
		weapons[]=
		{
			"rhs_m4_compm4",
			"rhs_weap_maaws_optic",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"rhs_weap_maaws_optic",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_maaws_HEAT",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_aa: rhsusf_army_ocp_riflemanat
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=200000;
		threat[]={1,1,0.80000001};
		camouflage=1.5;
		icon="iconManAT";
		weapons[]=
		{
			"rhs_weap_m16a4",
			"rhs_weap_fim92",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4",
			"rhs_weap_fim92",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_fim92_mag"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsusf_army_ocp_uav: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		weapons[]=
		{
			"rhs_weap_m16a4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag"
		};
		backpack="B_rhsusf_B_BACKPACK";
	};
	class rhsusf_army_ocp_medic: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_compm4",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m18_red"
		};
		attendant=1;
		icon="iconManMedic";
		backpack="rhsusf_assault_eagleaiii_ocp_medic";
	};
	class rhsusf_army_ocp_engineer: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=220000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		engineer=1;
		detectSkill=60;
		threat[]={1,0.5,0.1};
		icon="iconManEngineer";
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ocp",
			"rhsusf_iotv_ocp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ocp",
			"rhsusf_iotv_ocp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_grip_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_grip_compm4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m18_red"
		};
		backpack="rhsusf_assault_eagleaiii_ocp_engineer";
	};
	class rhsusf_army_ocp_explosives: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=115000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		detectSkill=80;
		threat[]={1,0.5,0.1};
		icon="iconManExplosive";
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ocp",
			"rhsusf_iotv_ocp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ocp",
			"rhsusf_iotv_ocp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog2",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog2",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"Chemlight_green",
			"Chemlight_green",
			"rhs_mag_m18_red"
		};
		backpack="rhsusf_assault_eagleaiii_ocp_demo";
	};
	class rhsusf_army_ocp_sniper: rhsusf_army_ocp_marksman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=350000;
		threat[]={1,0.30000001,0.30000001};
		camouflage=0.40000001;
		sensitivity=3.5;
		primaryAmmoCoef=0.2;
		secondaryAmmoCoef=0.050000001;
		handgunAmmoCoef=0.1;
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_bare_des",
			5,
			"rhsusf_ach_bare_des_ess",
			5,
			"rhsusf_ach_bare_des_headset",
			5,
			"rhsusf_ach_bare_des_headset_ess",
			5,
			"rhsusf_ach_bare_semi",
			5,
			"rhsusf_ach_bare_semi_ess",
			5,
			"rhsusf_ach_bare_semi_headset",
			5,
			"rhsusf_ach_bare_semi_headset_ess",
			5,
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			3
		};
		linkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_iotv_ocp_teamleader",
			"rhsusf_acc_M2010S",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_iotv_ocp_teamleader",
			"rhsusf_acc_M2010S",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_XM2010_d_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_XM2010_d_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_5Rnd_300winmag_xm2010",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_army_ocp_sniper_m24sws: rhsusf_army_ocp_sniper
	{
		uniformClass="rhs_uniform_acu_oefcp";
		linkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_iotv_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_iotv_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m24sws_d_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m24sws_d_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_5Rnd_762x51_m993_Mag",
			"rhsusf_5Rnd_762x51_m993_Mag",
			"rhsusf_5Rnd_762x51_m993_Mag",
			"rhsusf_5Rnd_762x51_m993_Mag",
			"rhsusf_5Rnd_762x51_m993_Mag",
			"rhsusf_5Rnd_762x51_m62_Mag",
			"rhsusf_5Rnd_762x51_m62_Mag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_army_ocp_sniper_m107: rhsusf_army_ocp_sniper
	{
		uniformClass="rhs_uniform_acu_oefcp";
		linkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_iotv_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_iotv_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_M107_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M107_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_army_ocp_crewman: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		engineer=1;
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_cvc_helmet",
			1,
			"rhsusf_cvc_alt_helmet",
			1,
			"rhsusf_cvc_ess",
			1
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		linkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_iotv_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_iotv_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"rhsusf_bino_m24_ARD",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"rhsusf_bino_m24_ARD",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
		fsmDanger="rhsusf\addons\rhsusf_c_troops\scripts\dangerCrew.fsm";
	};
	class rhsusf_army_ocp_combatcrewman: rhsusf_army_ocp_crewman
	{
		uniformClass="rhs_uniform_acu_oefcp";
		linkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_iotv_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_iotv_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_driver: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		engineer=1;
		headgearList[]={};
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhsusf_patrolcap_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_patrolcap_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_red"
		};
	};
	class rhsusf_army_ocp_driver_armored: rhsusf_army_ocp_driver
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			5,
			"rhsusf_ach_helmet_ocp_alt",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ocp",
			5,
			"rhsusf_ach_helmet_ESS_ocp_alt",
			2,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			1
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ocp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ocp"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_iotv_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_helipilot: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_acu_oefcp";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_pilot_s"
				};
				speechPlural[]=
				{
					"veh_infantry_pilot_p"
				};
			};
		};
		nameSound="veh_infantry_pilot_s";
		cost=160000;
		camouflage=2;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhsusf_patrolcap_ocp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhsusf_patrolcap_ocp"
		};
		linkedItems[]=
		{
			"rhsusf_hgu56p",
			"rhsusf_spcs_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_hgu56p",
			"rhsusf_spcs_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_hgu56p",
			1,
			"rhsusf_hgu56p_visor",
			1,
			"rhsusf_hgu56p_black",
			15,
			"rhsusf_hgu56p_visor_black",
			15,
			"rhsusf_hgu56p_usa",
			0.001,
			"rhsusf_hgu56p_visor_usa",
			0.001
		};
		weapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_red"
		};
	};
	class rhsusf_army_ocp_ah64_pilot: rhsusf_army_ocp_helipilot
	{
		uniformClass="rhs_uniform_acu_oefcp";
		headgearList[]={};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_patrolcap_ocp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_patrolcap_ocp"
		};
		linkedItems[]=
		{
			"rhsusf_ihadss",
			"rhsusf_spcs_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ihadss",
			"rhsusf_spcs_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_helicrew: rhsusf_army_ocp_helipilot
	{
		uniformClass="rhs_uniform_acu_oefcp";
		cost=80000;
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_hgu56p_mask",
			1,
			"rhsusf_hgu56p_visor_mask",
			1,
			"rhsusf_hgu56p_visor_mask_skull",
			1,
			"rhsusf_hgu56p_mask_skull",
			1,
			"rhsusf_hgu56p_visor_mask_mo",
			1,
			"rhsusf_hgu56p_mask_mo",
			1,
			"rhsusf_hgu56p_mask_smiley",
			1,
			"rhsusf_hgu56p_visor_mask_smiley",
			1,
			"rhsusf_hgu56p_mask_black",
			15,
			"rhsusf_hgu56p_visor_mask_black",
			15,
			"rhsusf_hgu56p_visor_mask_black_skull",
			15,
			"rhsusf_hgu56p_mask_black_skull",
			15,
			"rhsusf_hgu56p_visor_mask_Empire_black",
			15
		};
		linkedItems[]=
		{
			"rhsusf_hgu56p_mask",
			"rhsusf_spcs_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_hgu56p_mask",
			"rhsusf_spcs_ocp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_red"
		};
	};
	class rhsusf_army_ucp_rifleman_1stcav: rhsusf_army_ocp_rifleman_1stcav
	{
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		uniformClass="rhs_uniform_cu_ucp_1stcav";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\1CAV_ucp_co.paa"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_rifleman_10th: rhsusf_army_ocp_rifleman_10th
	{
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		uniformClass="rhs_uniform_cu_ucp_10th";
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\10MT_ucp_co.paa"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_rifleman: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_rifleman_m4: rhsusf_army_ocp_rifleman_m4
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_rifleman_m16: rhsusf_army_ocp_rifleman_m16
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_rifleman_m590: rhsusf_army_ocp_rifleman_m590
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_riflemanl: rhsusf_army_ocp_riflemanl
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		headgearList[]={};
		linkedItems[]=
		{
			"rhs_Booniehat_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_Booniehat_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_riflemanat: rhsusf_army_ocp_riflemanat
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_grenadier: rhsusf_army_ocp_grenadier
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_iotv_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_iotv_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_autorifleman: rhsusf_army_ocp_autorifleman
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_ar";
	};
	class rhsusf_army_ucp_autoriflemana: rhsusf_army_ocp_autoriflemana
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		weapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_ar";
	};
	class rhsusf_army_ucp_machinegunner: rhsusf_army_ocp_machinegunner
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_mg";
	};
	class rhsusf_army_ucp_machinegunnera: rhsusf_army_ocp_machinegunnera
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		weapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_compm4",
			"Throw",
			"Put"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_mg";
	};
	class rhsusf_army_ucp_marksman: rhsusf_army_ocp_marksman
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_officer: rhsusf_army_ocp_officer
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ucp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp_alt",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ess_ucp",
			"rhsusf_iotv_ucp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ess_ucp",
			"rhsusf_iotv_ucp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_squadleader: rhsusf_army_ocp_squadleader
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ucp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp_alt",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ucp",
			"rhsusf_iotv_ucp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ucp",
			"rhsusf_iotv_ucp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_teamleader: rhsusf_army_ocp_teamleader
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ucp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp_alt",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_iotv_ucp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_iotv_ucp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_jfo: rhsusf_army_ocp_jfo
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp_norotos",
			"rhsusf_iotv_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp_norotos",
			"rhsusf_iotv_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_fso: rhsusf_army_ocp_fso
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ucp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp_alt",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ucp",
			"rhsusf_iotv_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ucp",
			"rhsusf_iotv_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_javelin: rhsusf_army_ocp_javelin
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_javelin_assistant: rhsusf_army_ocp_javelin_assistant
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		backpack="rhsusf_assault_eagleaiii_ucp_at";
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_maaws: rhsusf_army_ocp_maaws
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		backpack="rhsusf_assault_eagleaiii_ucp_maaws";
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_aa: rhsusf_army_ocp_aa
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_uav: rhsusf_army_ocp_uav
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_medic: rhsusf_army_ocp_medic
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_medic";
	};
	class rhsusf_army_ucp_engineer: rhsusf_army_ocp_engineer
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ucp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp_alt",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ucp",
			"rhsusf_iotv_ucp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ucp",
			"rhsusf_iotv_ucp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_engineer";
	};
	class rhsusf_army_ucp_explosives: rhsusf_army_ocp_explosives
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ucp_alt",
			2,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp_alt",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ucp",
			"rhsusf_iotv_ucp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ucp",
			"rhsusf_iotv_ucp_repair",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_demo";
	};
	class rhsusf_army_ucp_sniper: rhsusf_army_ocp_sniper
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_bare_wood",
			5,
			"rhsusf_ach_bare_wood_ess",
			5,
			"rhsusf_ach_bare_wood_headset",
			5,
			"rhsusf_ach_bare_wood_headset_ess",
			5,
			"rhsusf_ach_bare_semi",
			5,
			"rhsusf_ach_bare_semi_ess",
			5,
			"rhsusf_ach_bare_semi_headset",
			5,
			"rhsusf_ach_bare_semi_headset_ess",
			5,
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_bare_wood",
			"rhsusf_iotv_ucp_rifleman",
			"rhsusf_acc_M2010S",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_wood",
			"rhsusf_iotv_ucp_rifleman",
			"rhsusf_acc_M2010S",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_XM2010_wd_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_XM2010_wd_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ucp_sniper_m24sws: rhsusf_army_ocp_sniper_m24sws
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_bare_wood",
			5,
			"rhsusf_ach_bare_wood_ess",
			5,
			"rhsusf_ach_bare_wood_headset",
			5,
			"rhsusf_ach_bare_wood_headset_ess",
			5,
			"rhsusf_ach_bare_semi",
			5,
			"rhsusf_ach_bare_semi_ess",
			5,
			"rhsusf_ach_bare_semi_headset",
			5,
			"rhsusf_ach_bare_semi_headset_ess",
			5,
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_bare_wood",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_wood",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m24sws_wd_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m24sws_wd_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lrf_Vector21",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ucp_sniper_m107: rhsusf_army_ocp_sniper_m107
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_bare_wood",
			5,
			"rhsusf_ach_bare_wood_ess",
			5,
			"rhsusf_ach_bare_wood_headset",
			5,
			"rhsusf_ach_bare_wood_headset_ess",
			5,
			"rhsusf_ach_bare_semi",
			5,
			"rhsusf_ach_bare_semi_ess",
			5,
			"rhsusf_ach_bare_semi_headset",
			5,
			"rhsusf_ach_bare_semi_headset_ess",
			5,
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_bare_wood",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_wood",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_crewman: rhsusf_army_ocp_crewman
	{
		uniformClass="rhs_uniform_acu_ucp";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_cvc_green_helmet",
			1,
			"rhsusf_cvc_green_alt_helmet",
			1,
			"rhsusf_cvc_green_ess",
			1
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		linkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_pockets_co.paa"
		};
	};
	class rhsusf_army_ucp_combatcrewman: rhsusf_army_ocp_combatcrewman
	{
		uniformClass="rhs_uniform_acu_ucp";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_cvc_green_helmet",
			1,
			"rhsusf_cvc_green_alt_helmet",
			1,
			"rhsusf_cvc_green_ess",
			1
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		linkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_driver: rhsusf_army_ocp_driver
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		headgearList[]={};
		linkedItems[]=
		{
			"rhsusf_patrolcap_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_patrolcap_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_driver_armored: rhsusf_army_ocp_driver_armored
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			5,
			"rhsusf_ach_helmet_ucp_alt",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			2,
			"rhsusf_ach_helmet_ESS_ucp",
			5,
			"rhsusf_ach_helmet_ESS_ucp_alt",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_iotv_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_helipilot: rhsusf_army_ocp_helipilot
	{
		uniformClass="rhs_uniform_cu_ucp";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_hgu56p",
			15,
			"rhsusf_hgu56p_visor",
			15,
			"rhsusf_hgu56p_black",
			2,
			"rhsusf_hgu56p_visor_black",
			2,
			"rhsusf_hgu56p_usa",
			0.001,
			"rhsusf_hgu56p_visor_usa",
			0.001
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhsusf_patrolcap_ucp"
		};
		linkedItems[]=
		{
			"rhsusf_hgu56p",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_hgu56p",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_ah64_pilot: rhsusf_army_ocp_ah64_pilot
	{
		uniformClass="rhs_uniform_cu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_patrolcap_ucp"
		};
		headgearList[]={};
		linkedItems[]=
		{
			"rhsusf_ihadss",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ihadss",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_helicrew: rhsusf_army_ocp_helicrew
	{
		uniformClass="rhs_uniform_acu_ucp";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_hgu56p_mask",
			15,
			"rhsusf_hgu56p_visor_mask",
			15,
			"rhsusf_hgu56p_visor_mask_skull",
			15,
			"rhsusf_hgu56p_mask_skull",
			15,
			"rhsusf_hgu56p_visor_mask_mo",
			15,
			"rhsusf_hgu56p_mask_mo",
			15,
			"rhsusf_hgu56p_mask_smiley",
			15,
			"rhsusf_hgu56p_visor_mask_smiley",
			15,
			"rhsusf_hgu56p_mask_black",
			2,
			"rhsusf_hgu56p_visor_mask_black",
			2,
			"rhsusf_hgu56p_visor_mask_black_skull",
			2,
			"rhsusf_hgu56p_mask_black_skull",
			2,
			"rhsusf_hgu56p_visor_mask_Empire_black",
			2
		};
		linkedItems[]=
		{
			"rhsusf_hgu56p_mask",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_hgu56p_mask",
			"rhsusf_iotv_ucp",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_infantry_army_82nd: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_cu_ocp_82nd";
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\82AB_ocp_co.paa"
		};
	};
	class rhsusf_infantry_army_101st: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ocp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\101AB_ocp_co.paa"
		};
	};
	class rhsusf_army_ocp_rifleman_82nd: rhsusf_infantry_army_82nd
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_rifleman_101st: rhsusf_infantry_army_101st
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_arb_rifleman: rhsusf_army_ocp_rifleman
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_an_m8hc",
			"rhs_mag_an_m8hc",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_arb_riflemanl: rhsusf_army_ocp_arb_rifleman
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		headgearList[]={};
		linkedItems[]=
		{
			"rhs_Booniehat_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_Booniehat_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_rifleman_arb_m16: rhsusf_army_ocp_rifleman_m16
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_arb_rifleman_m590: rhsusf_army_ocp_rifleman_m590
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_arb_riflemanat: rhsusf_army_ocp_riflemanat
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_m136_mag",
			"rhs_mag_an_m8hc"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsusf_army_ocp_arb_maaws: rhsusf_army_ocp_maaws
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_teamleader_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_teamleader_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"rhs_weap_maaws_optic",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"rhs_weap_maaws_optic",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_maaws_HEAT",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_army_ocp_arb_grenadier: rhsusf_army_ocp_grenadier
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_spcs_ocp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_spcs_ocp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_m320_acog",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_m320_acog",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP"
		};
	};
	class rhsusf_army_ocp_arb_autorifleman: rhsusf_army_ocp_autorifleman
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m249_light_S_elcan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m249_light_S_elcan",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhs_mag_m67"
		};
	};
	class rhsusf_army_ocp_arb_autoriflemana: rhsusf_army_ocp_autoriflemana
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_compm4",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_arb_machinegunner: rhsusf_army_ocp_machinegunner
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_machinegunner",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_machinegunner",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ocp_arb_machinegunnera: rhsusf_army_ocp_machinegunnera
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_compm4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_compm4",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_arb_marksman: rhsusf_army_ocp_marksman
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m14ebrri_leu",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m14ebrri_leu",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m118_special_Mag",
			"rhsusf_20Rnd_762x51_m62_Mag",
			"rhsusf_20Rnd_762x51_m62_Mag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_army_ocp_arb_squadleader: rhsusf_army_ocp_squadleader
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			3,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			5,
			"rhsusf_ach_helmet_headset_ess_ocp",
			5,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ess_ocp",
			"rhsusf_spcs_ocp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ess_ocp",
			"rhsusf_spcs_ocp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog3",
			"rhsusf_bino_m24",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog3",
			"rhsusf_bino_m24",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_purple",
			"rhs_mag_mk84",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_green"
		};
	};
	class rhsusf_army_ocp_arb_teamleader: rhsusf_army_ocp_teamleader
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			3,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			5,
			"rhsusf_ach_helmet_headset_ess_ocp",
			5,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_spcs_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ocp",
			"rhsusf_spcs_ocp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog2",
			"rhsusf_bino_m24",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2",
			"rhsusf_bino_m24",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_purple",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_green"
		};
	};
	class rhsusf_army_ocp_arb_medic: rhsusf_army_ocp_medic
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			2,
			"rhsusf_ach_helmet_ocp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			1,
			"rhsusf_ach_helmet_headset_ess_ocp",
			1,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ocp",
			"rhsusf_spcs_ocp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_compm4",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_compm4",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_arb_engineer: rhsusf_army_ocp_engineer
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ocp",
			1,
			"rhsusf_ach_helmet_ocp_norotos",
			3,
			"rhsusf_ach_helmet_ESS_ocp",
			1,
			"rhsusf_ach_helmet_headset_ocp",
			5,
			"rhsusf_ach_helmet_headset_ess_ocp",
			5,
			"rhsusf_ach_helmet_camo_ocp",
			2
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ocp",
			"rhsusf_spcs_ocp_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ocp",
			"rhsusf_spcs_ocp_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ocp_arb_sniper_m107: rhsusf_army_ocp_sniper_m107
	{
		uniformClass="rhs_uniform_cu_ocp_101st";
		linkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_spcs_ocp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_spcs_ocp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_M107_d_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M107_d_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_army_ucp_rifleman_82nd: rhsusf_army_ocp_rifleman_82nd
	{
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		uniformClass="rhs_uniform_cu_ucp_82nd";
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\82AB_ucp_co.paa"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_rifleman_101st: rhsusf_army_ocp_rifleman_101st
	{
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ucp"
		};
		uniformClass="rhs_uniform_cu_ucp_101st";
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\rhsusf_combat01_ucp_pockets_co.paa",
			"rhsusf\addons\rhsusf_infantry\data\patches\101AB_ucp_co.paa"
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_rifleman: rhsusf_army_ocp_arb_rifleman
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_riflemanl: rhsusf_army_ocp_arb_riflemanl
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		headgearList[]={};
		linkedItems[]=
		{
			"rhs_Booniehat_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_Booniehat_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_rifleman_arb_m16: rhsusf_army_ocp_rifleman_arb_m16
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_rifleman_m590: rhsusf_army_ocp_arb_rifleman_m590
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_riflemanat: rhsusf_army_ocp_arb_riflemanat
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_teamleader_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_teamleader_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_maaws: rhsusf_army_ocp_arb_maaws
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_maaws";
	};
	class rhsusf_army_ucp_arb_grenadier: rhsusf_army_ocp_arb_grenadier
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_spcs_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_spcs_ucp_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_autorifleman: rhsusf_army_ocp_arb_autorifleman
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_saw",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_ar";
	};
	class rhsusf_army_ucp_arb_autoriflemana: rhsusf_army_ocp_arb_autoriflemana
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_ar";
	};
	class rhsusf_army_ucp_arb_machinegunner: rhsusf_army_ocp_arb_machinegunner
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_machinegunner",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_machinegunner",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_mg";
	};
	class rhsusf_army_ucp_arb_machinegunnera: rhsusf_army_ocp_arb_machinegunnera
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_rifleman_alt",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_mg";
	};
	class rhsusf_army_ucp_arb_marksman: rhsusf_army_ocp_arb_marksman
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_squadleader: rhsusf_army_ocp_arb_squadleader
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			3,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ess_ucp",
			"rhsusf_spcs_ucp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_headset_ess_ucp",
			"rhsusf_spcs_ucp_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_teamleader: rhsusf_army_ocp_arb_teamleader
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			3,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_spcs_ucp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ess_ucp",
			"rhsusf_spcs_ucp_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_army_ucp_arb_medic: rhsusf_army_ocp_arb_medic
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			2,
			"rhsusf_ach_helmet_ucp_norotos",
			5,
			"rhsusf_ach_helmet_ESS_ucp",
			2,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ucp",
			"rhsusf_spcs_ucp_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_medic";
	};
	class rhsusf_army_ucp_arb_engineer: rhsusf_army_ocp_arb_engineer
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			3,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			5,
			"rhsusf_ach_helmet_headset_ess_ucp",
			5
		};
		linkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ucp",
			"rhsusf_spcs_ucp_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_helmet_ESS_ucp",
			"rhsusf_spcs_ucp_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_ucp_engineer";
	};
	class rhsusf_army_ucp_arb_sniper_m107: rhsusf_army_ocp_arb_sniper_m107
	{
		uniformClass="rhs_uniform_cu_ucp_101st";
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], []] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsusf_ach_bare_wood",
			5,
			"rhsusf_ach_bare_wood_ess",
			5,
			"rhsusf_ach_bare_wood_headset",
			5,
			"rhsusf_ach_bare_wood_headset_ess",
			5,
			"rhsusf_ach_bare_semi",
			5,
			"rhsusf_ach_bare_semi_ess",
			5,
			"rhsusf_ach_bare_semi_headset",
			5,
			"rhsusf_ach_bare_semi_headset_ess",
			5,
			"rhsusf_ach_helmet_ucp",
			1,
			"rhsusf_ach_helmet_ucp_norotos",
			1,
			"rhsusf_ach_helmet_ESS_ucp",
			1,
			"rhsusf_ach_helmet_headset_ucp",
			1,
			"rhsusf_ach_helmet_headset_ess_ucp",
			1
		};
		linkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_spcs_ucp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_ach_bare_des",
			"rhsusf_spcs_ucp_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_M107_w_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M107_w_leu",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_infantry_usmc_base: rhsusf_infantry_army_base
	{
		uniformClass="rhs_uniform_FROG01_wd";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO",
			"rhsusf_g_usmc"
		};
		faceType="Man_A3";
		genericNames="NATOMen";
		class Wounds
		{
		};
		weapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		hiddenSelections[]=
		{
			"camo",
			"insignia"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_frog02_wd_co.paa"
		};
	};
	class rhsusf_usmc_marpat_wd_rifleman: rhsusf_infantry_usmc_base
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_marpat_wd_rifleman_m4: rhsusf_usmc_marpat_wd_rifleman
	{
		weapons[]=
		{
			"rhs_m4_grip_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_grip_acog_usmc",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_marpat_wd_rifleman_light: rhsusf_usmc_marpat_wd_rifleman
	{
		camouflage=1.2;
		cost=90000;
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"Throw",
			"Put"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_wd_riflemanat: rhsusf_usmc_marpat_wd_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=180000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_an_m8hc",
			"rhs_m136_mag"
		};
		secondaryAmmoCoef=0.5;
		class EventHandlers;
	};
	class rhsusf_usmc_marpat_wd_rifleman_law: rhsusf_usmc_marpat_wd_riflemanat
	{
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhs_weap_m72a7",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhs_weap_m72a7",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_an_m8hc",
			"rhs_m72a7_mag"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsusf_usmc_marpat_wd_rifleman_m590: rhsusf_usmc_marpat_wd_rifleman
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_M590_8RD",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M590_8RD",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhsusf_8Rnd_00Buck",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
		backpack="rhsusf_falconii_breach";
	};
	class rhsusf_usmc_marpat_wd_grenadier: rhsusf_usmc_marpat_wd_rifleman
	{
		cost=130000;
		threat[]={1,0.30000001,0.1};
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_m203_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_m203_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP"
		};
		backpack="rhsusf_falconii_coy_gr";
	};
	class rhsusf_usmc_marpat_wd_grenadier_m32: rhsusf_usmc_marpat_wd_grenadier
	{
		weapons[]=
		{
			"rhs_weap_m32",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m32",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M714_white",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ"
		};
		backpack="rhsusf_falconii_gr_m32";
	};
	class rhsusf_usmc_marpat_wd_autorifleman: rhsusf_infantry_usmc_base
	{
		cost=125000;
		threat[]={1,0.1,0.30000001};
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_coy_m27";
		weapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_marpat_wd_autorifleman_m249: rhsusf_usmc_marpat_wd_autorifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_coy_ar";
		weapons[]=
		{
			"rhs_weap_m249_pip_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m249_pip_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_100Rnd_556x45_M855_soft_pouch",
			"rhsusf_100Rnd_556x45_M855_soft_pouch",
			"rhsusf_100Rnd_556x45_M855_soft_pouch",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_marpat_wd_autorifleman_m249_ass: rhsusf_usmc_marpat_wd_rifleman
	{
		cost=110000;
		camouflage=1.5;
		backpack="rhsusf_assault_eagleaiii_coy_ar";
	};
	class rhsusf_usmc_marpat_wd_machinegunner: rhsusf_usmc_marpat_wd_autorifleman
	{
		icon="iconManMG";
		backpack="rhsusf_assault_eagleaiii_coy_mg";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m240b_usmc",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m240b_usmc",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_marpat_wd_machinegunner_ass: rhsusf_usmc_marpat_wd_rifleman
	{
		cost=110000;
		camouflage=1.5;
		weapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"Throw",
			"Put"
		};
		backpack="rhsusf_assault_eagleaiii_coy_mg";
	};
	class rhsusf_usmc_marpat_wd_officer: rhsusf_usmc_marpat_wd_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_officer_s"
				};
				speechPlural[]=
				{
					"veh_infantry_officer_p"
				};
			};
		};
		nameSound="veh_infantry_officer_s";
		cost=600000;
		camouflage=1.6;
		sensitivity=3;
		icon="iconManOfficer";
		linkedItems[]=
		{
			"rhs_8point_marpatwd",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_8point_marpatwd",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green"
		};
	};
	class rhsusf_usmc_marpat_wd_squadleader: rhsusf_usmc_marpat_wd_rifleman
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_grip_acog3_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_grip_acog3_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"Chemlight_red",
			"Chemlight_red",
			"rhs_mag_m67",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_purple",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_red"
		};
	};
	class rhsusf_usmc_marpat_wd_teamleader: rhsusf_infantry_usmc_base
	{
		cost=450000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_m203_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_m203_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_purple"
		};
		backpack="rhsusf_falconii_coy_gr";
	};
	class rhsusf_usmc_marpat_wd_jfo: rhsusf_usmc_marpat_wd_rifleman
	{
		icon="iconManLeader";
		cost=450000;
		sensitivity=3.2;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_m203s_acog_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_m203s_acog_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"Chemlight_green",
			"Chemlight_red",
			"I_IR_Grenade",
			"I_IR_Grenade",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_purple"
		};
		backpack="rhsusf_falconii_gr";
	};
	class rhsusf_usmc_marpat_wd_fso: rhsusf_usmc_marpat_wd_rifleman
	{
		icon="iconManLeader";
		cost=200000;
		sensitivity=3.2;
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_alt_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_alt_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog2_usmc",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog2_usmc",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_purple"
		};
	};
	class rhsusf_usmc_marpat_wd_smaw: rhsusf_usmc_marpat_wd_riflemanat
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=190000;
		threat[]={1,1,0.80000001};
		camouflage=1.5;
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhs_weap_smaw_gr_optic",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhs_weap_smaw_gr_optic",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_smaw_SR",
			"rhs_mag_smaw_SR",
			"rhs_mag_smaw_SR",
			"rhs_mag_smaw_HEDP",
			"rhs_mag_m67"
		};
		backpack="rhsusf_assault_eagleaiii_coy_assaultman";
	};
	class rhsusf_usmc_marpat_wd_javelin: rhsusf_usmc_marpat_wd_smaw
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhs_weap_fgm148",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhs_weap_fgm148",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_fgm148_magazine_AT",
			"rhs_mag_m67"
		};
		backpack="";
	};
	class rhsusf_usmc_marpat_wd_javelin_assistant: rhsusf_usmc_marpat_wd_smaw
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67"
		};
		backpack="rhsusf_assault_eagleaiii_coy_at";
	};
	class rhsusf_usmc_marpat_wd_stinger: rhsusf_usmc_marpat_wd_smaw
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhs_weap_fim92",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhs_weap_fim92",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_fim92_mag",
			"rhs_mag_m67"
		};
		secondaryAmmoCoef=0.5;
		backpack="rhsusf_assault_eagleaiii_coy_aa";
	};
	class rhsusf_usmc_marpat_wd_engineer: rhsusf_usmc_marpat_wd_rifleman
	{
		cost=220000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		engineer=1;
		detectSkill=60;
		threat[]={1,0.5,0.1};
		icon="iconManEngineer";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
		backpack="rhsusf_assault_eagleaiii_coy_engineer";
	};
	class rhsusf_usmc_marpat_wd_explosives: rhsusf_usmc_marpat_wd_rifleman
	{
		cost=115000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		detectSkill=80;
		threat[]={1,0.5,0.1};
		icon="iconManExplosive";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog2_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog2_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"Chemlight_green",
			"Chemlight_green",
			"ClaymoreDirectionalMine_Remote_Mag",
			"rhs_mag_m18_red"
		};
		backpack="rhsusf_assault_eagleaiii_coy_eod";
	};
	class rhsusf_usmc_marpat_wd_uav: rhsusf_usmc_marpat_wd_rifleman
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67"
		};
		backpack="B_rhsusf_B_BACKPACK";
	};
	class rhsusf_usmc_marpat_wd_marksman: rhsusf_usmc_marpat_wd_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_sr25_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_marpat_wd_spotter: rhsusf_usmc_marpat_wd_marksman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_SF_s"
				};
				speechPlural[]=
				{
					"veh_infantry_SF_p"
				};
			};
		};
		nameSound="veh_infantry_SF_s";
		cost=250000;
		threat[]={1,0.30000001,0.30000001};
		camouflage=0.60000002;
		sensitivity=3.5;
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_acog_usmc",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_an_m8hc",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_marpat_wd_sniper: rhsusf_usmc_marpat_wd_marksman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=350000;
		threat[]={1,0.30000001,0.30000001};
		camouflage=0.40000001;
		sensitivity=3.5;
		primaryAmmoCoef=0.2;
		secondaryAmmoCoef=0.050000001;
		handgunAmmoCoef=0.1;
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m40_wd_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m40_wd_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_10Rnd_762x51_m118_special_Mag",
			"rhsusf_10Rnd_762x51_m118_special_Mag",
			"rhsusf_10Rnd_762x51_m118_special_Mag",
			"rhsusf_10Rnd_762x51_m118_special_Mag",
			"rhsusf_10Rnd_762x51_m118_special_Mag",
			"rhsusf_10Rnd_762x51_m118_special_Mag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_marpat_wd_sniper_m110: rhsusf_usmc_marpat_wd_sniper
	{
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_sr25_sup_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_sup_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_marpat_wd_sniper_M107: rhsusf_usmc_marpat_wd_sniper
	{
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_M107_premier",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M107_premier",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_marpat_wd_crewman: rhsusf_infantry_usmc_base
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		engineer=1;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		linkedItems[]=
		{
			"rhsusf_cvc_green_helmet",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_helmet",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
		fsmDanger="rhsusf\addons\rhsusf_c_troops\scripts\dangerCrew.fsm";
	};
	class rhsusf_usmc_marpat_wd_combatcrewman: rhsusf_usmc_marpat_wd_crewman
	{
		linkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_wd_driver: rhsusf_infantry_usmc_base
	{
		engineer=1;
		linkedItems[]=
		{
			"rhs_8point_marpatwd",
			"rhsusf_spc_light",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_8point_marpatwd",
			"rhsusf_spc_light",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_marpat_wd_gunner: rhsusf_usmc_marpat_wd_driver
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_wd_helipilot: rhsusf_infantry_usmc_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_pilot_s"
				};
				speechPlural[]=
				{
					"veh_infantry_pilot_p"
				};
			};
		};
		nameSound="veh_infantry_pilot_s";
		cost=160000;
		camouflage=2;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		linkedItems[]=
		{
			"rhsusf_hgu56p",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_hgu56p",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4_carryhandle",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_marpat_wd_helicrew: rhsusf_usmc_marpat_wd_helipilot
	{
		cost=80000;
		linkedItems[]=
		{
			"rhsusf_hgu56p_mask",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_hgu56p_mask",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_marpat_d_rifleman: rhsusf_usmc_marpat_wd_rifleman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_rifleman_light: rhsusf_usmc_marpat_wd_rifleman_light
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_rifleman_m4: rhsusf_usmc_marpat_wd_rifleman_m4
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_riflemanat: rhsusf_usmc_marpat_wd_riflemanat
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_rifleman_law: rhsusf_usmc_marpat_wd_rifleman_law
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_rifleman_m590: rhsusf_usmc_marpat_wd_rifleman_m590
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_grenadier: rhsusf_usmc_marpat_wd_grenadier
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_grenadier_m32: rhsusf_usmc_marpat_wd_grenadier_m32
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_autorifleman: rhsusf_usmc_marpat_wd_autorifleman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_autorifleman_m249: rhsusf_usmc_marpat_wd_autorifleman_m249
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_autorifleman_m249_ass: rhsusf_usmc_marpat_wd_autorifleman_m249_ass
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_machinegunner: rhsusf_usmc_marpat_wd_machinegunner
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_machinegunner_ass: rhsusf_usmc_marpat_wd_machinegunner_ass
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_officer: rhsusf_usmc_marpat_wd_officer
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhs_8point_marpatd",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_8point_marpatd",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_squadleader: rhsusf_usmc_marpat_wd_squadleader
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_teamleader: rhsusf_usmc_marpat_wd_teamleader
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_fso: rhsusf_usmc_marpat_wd_fso
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_alt_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_alt_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_jfo: rhsusf_usmc_marpat_wd_jfo
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog_usmc",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog_usmc",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_marpat_d_engineer: rhsusf_usmc_marpat_wd_engineer
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_explosives: rhsusf_usmc_marpat_wd_explosives
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_uav: rhsusf_usmc_marpat_wd_uav
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_smaw: rhsusf_usmc_marpat_wd_smaw
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_javelin: rhsusf_usmc_marpat_wd_javelin
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_javelin_assistant: rhsusf_usmc_marpat_wd_javelin_assistant
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_stinger: rhsusf_usmc_marpat_wd_stinger
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_marksman: rhsusf_usmc_marpat_wd_marksman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_spotter: rhsusf_usmc_marpat_wd_spotter
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_grip_acog_usmc",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_marpat_d_sniper: rhsusf_usmc_marpat_wd_sniper
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m40_d_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m40_d_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_marpat_d_sniper_m110: rhsusf_usmc_marpat_wd_sniper_m110
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_sr25_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_marpat_d_sniper_m107: rhsusf_usmc_marpat_wd_sniper_M107
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos_arc",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_crewman: rhsusf_usmc_marpat_wd_crewman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_frog02_d_co.paa"
		};
	};
	class rhsusf_usmc_marpat_d_combatcrewman: rhsusf_usmc_marpat_wd_combatcrewman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_driver: rhsusf_usmc_marpat_wd_driver
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhs_8point_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_8point_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_gunner: rhsusf_usmc_marpat_wd_gunner
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_marpat_d_helipilot: rhsusf_usmc_marpat_wd_helipilot
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_marpat_d_helicrew: rhsusf_usmc_marpat_wd_helicrew
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_lar_marpat_wd_rifleman: rhsusf_usmc_marpat_wd_rifleman
	{
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_grip_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_grip_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_rifleman_light: rhsusf_usmc_lar_marpat_wd_rifleman
	{
		camouflage=1.2;
		cost=90000;
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_riflemanat: rhsusf_usmc_lar_marpat_wd_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=180000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		weapons[]=
		{
			"rhs_m4_acog2_usmc",
			"rhs_weap_M136",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog2_usmc",
			"rhs_weap_M136",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_m136_mag"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsusf_usmc_lar_marpat_wd_grenadier_m32: rhsusf_usmc_lar_marpat_wd_rifleman
	{
		cost=130000;
		threat[]={1,0.30000001,0.1};
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m32_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m32_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M714_white",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_autorifleman: rhsusf_infantry_usmc_base
	{
		cost=125000;
		threat[]={1,0.1,0.30000001};
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_squadleader: rhsusf_usmc_lar_marpat_wd_rifleman
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_an_m8hc",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_teamleader: rhsusf_infantry_usmc_base
	{
		cost=450000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_m203s_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_m203s_acog_usmc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M585_white",
			"rhs_mag_M585_white",
			"rhs_mag_M585_white",
			"rhs_mag_m67"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_marksman: rhsusf_usmc_lar_marpat_wd_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatwd_headset",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_sr25_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_usmc",
			"rhsusf_weap_m9",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_crewman: rhsusf_infantry_usmc_base
	{
		engineer=1;
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		linkedItems[]=
		{
			"rhsusf_cvc_green_helmet",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_helmet",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4_acog_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_combatcrewman: rhsusf_usmc_lar_marpat_wd_crewman
	{
		linkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_spc_crewman",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_wd_machinegunner: rhsusf_usmc_lar_marpat_wd_combatcrewman
	{
		icon="iconManMG";
		backpack="rhsusf_assault_eagleaiii_coy_mg";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m240b_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m240b_usmc",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_usmc_lar_marpat_d_rifleman: rhsusf_usmc_lar_marpat_wd_rifleman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_rifleman_light: rhsusf_usmc_lar_marpat_wd_rifleman_light
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_riflemanat: rhsusf_usmc_lar_marpat_wd_riflemanat
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_grenadier_m32: rhsusf_usmc_lar_marpat_wd_grenadier_m32
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_autorifleman: rhsusf_usmc_lar_marpat_wd_autorifleman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_squadleader: rhsusf_usmc_lar_marpat_wd_squadleader
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_teamleader: rhsusf_usmc_lar_marpat_wd_teamleader
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_marksman: rhsusf_usmc_lar_marpat_wd_marksman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_lwh_helmet_marpatd_ess",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_crewman: rhsusf_usmc_lar_marpat_wd_crewman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_combatcrewman: rhsusf_usmc_lar_marpat_wd_combatcrewman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_helmet",
			"rhsusf_spc_crewman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_lar_marpat_d_machinegunner: rhsusf_usmc_lar_marpat_wd_machinegunner
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_infantry_usmc_recon_base: rhsusf_infantry_usmc_base
	{
		uniformClass="rhs_uniform_FROG01_wd";
		camouflage=0.60000002;
		sensitivity=3.5;
		detectSkill=30;
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO",
			"rhsusf_g_usmc"
		};
		faceType="Man_A3";
		genericNames="NATOMen";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14"
		};
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup",
			"rhsusf_weap_m1911a1",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup",
			"rhsusf_weap_m1911a1",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red"
		};
		class EventHandlers;
	};
	class rhsusf_usmc_recon_marpat_wd_rifleman: rhsusf_infantry_usmc_recon_base
	{
		class EventHandlers;
	};
	class rhsusf_usmc_recon_marpat_wd_rifleman_at: rhsusf_infantry_usmc_recon_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=180000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog2_usmc_sup",
			"rhsusf_weap_m1911a1",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog2_usmc_sup",
			"rhsusf_weap_m1911a1",
			"rhs_weap_M136",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red",
			"rhs_m136_mag"
		};
		secondaryAmmoCoef=0.5;
		class EventHandlers;
	};
	class rhsusf_usmc_recon_marpat_wd_grenadier_m32: rhsusf_usmc_recon_marpat_wd_rifleman
	{
		cost=130000;
		threat[]={1,0.30000001,0.1};
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m32",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m32",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M433_HEDP",
			"rhsusf_mag_6Rnd_M714_white",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red"
		};
		backpack="rhsusf_falconii_gr_m32";
	};
	class rhsusf_usmc_recon_marpat_wd_autorifleman: rhsusf_infantry_usmc_recon_base
	{
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_tan",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_tan",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc_sup",
			"rhsusf_weap_m1911a1",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc_sup",
			"rhsusf_weap_m1911a1",
			"Binocular",
			"Throw",
			"Put"
		};
		backpack="rhsusf_falconii_recon";
		class EventHandlers;
	};
	class rhsusf_usmc_recon_marpat_wd_machinegunner: rhsusf_infantry_usmc_recon_base
	{
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m240b_usmc",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m240b_usmc",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_50Rnd_762x51",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red"
		};
		backpack="rhsusf_pack_slackman_m240";
	};
	class rhsusf_usmc_recon_marpat_wd_machinegunner_m249: rhsusf_usmc_recon_marpat_wd_machinegunner
	{
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m249_para_usmc",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m249_para_usmc",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_100Rnd_556x45_M855_soft_pouch",
			"rhsusf_100Rnd_556x45_M855_soft_pouch",
			"rhsusf_100Rnd_556x45_M855_soft_pouch",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red"
		};
		backpack="rhsusf_pack_slackman_m249";
	};
	class rhsusf_usmc_recon_marpat_wd_marksman: rhsusf_infantry_usmc_recon_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red",
			"rhs_mag_mk84"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_sniper_M107: rhsusf_usmc_recon_marpat_wd_marksman
	{
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_tan",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_M107_w_premier",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M107_w_premier",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_officer: rhsusf_infantry_usmc_recon_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_officer_s"
				};
				speechPlural[]=
				{
					"veh_infantry_officer_p"
				};
			};
		};
		nameSound="veh_infantry_officer_s";
		cost=600000;
		camouflage=1.6;
		sensitivity=3;
		icon="iconManOfficer";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_tan_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_tan_headset",
			"rhsusf_spc_squadleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_acog2_usmc_sup",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_acog2_usmc_sup",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_teamleader: rhsusf_infantry_usmc_recon_base
	{
		cost=450000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_m203_acog3_usmc_sup",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_m203_acog3_usmc_sup",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag_Tracer_Red",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_mk3a2",
			"rhs_mag_mk3a2",
			"rhs_mag_mk84",
			"Chemlight_green",
			"Chemlight_green",
			"Chemlight_red",
			"Chemlight_red"
		};
		backpack="rhsusf_falconii_recon";
	};
	class rhsusf_usmc_recon_marpat_wd_rifleman_fast: rhsusf_usmc_recon_marpat_wd_rifleman
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_rifleman_at_fast: rhsusf_usmc_recon_marpat_wd_rifleman_at
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_autorifleman_fast: rhsusf_usmc_recon_marpat_wd_autorifleman
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_machinegunner_m249_fast: rhsusf_usmc_recon_marpat_wd_machinegunner_m249
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_marksman_fast: rhsusf_usmc_recon_marpat_wd_marksman
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_teamleader_fast: rhsusf_usmc_recon_marpat_wd_teamleader
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_rifleman_lite: rhsusf_usmc_recon_marpat_wd_rifleman
	{
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_lush",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_rifleman_at_lite: rhsusf_usmc_recon_marpat_wd_rifleman_at
	{
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_lush",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_autorifleman_lite: rhsusf_usmc_recon_marpat_wd_autorifleman
	{
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_lush",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_machinegunner_m249_lite: rhsusf_usmc_recon_marpat_wd_machinegunner_m249
	{
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_lush",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_marksman_lite: rhsusf_usmc_recon_marpat_wd_marksman
	{
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_lush",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_wd_teamleader_lite: rhsusf_usmc_recon_marpat_wd_teamleader
	{
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_lush",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatwd",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_rifleman: rhsusf_usmc_recon_marpat_wd_rifleman
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_recon_marpat_d_rifleman_at: rhsusf_usmc_recon_marpat_wd_rifleman_at
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_recon_marpat_d_autorifleman: rhsusf_usmc_recon_marpat_wd_autorifleman
	{
		uniformClass="rhs_uniform_FROG01_d";
		weapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m27iar_bipod_acog_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_recon_marpat_d_grenadier_m32: rhsusf_usmc_recon_marpat_wd_grenadier_m32
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_machinegunner: rhsusf_usmc_recon_marpat_wd_machinegunner
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_recon_marpat_d_machinegunner_m249: rhsusf_usmc_recon_marpat_wd_machinegunner_m249
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd_norotos",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_marksman: rhsusf_usmc_recon_marpat_wd_marksman
	{
		uniformClass="rhs_uniform_FROG01_d";
		weapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_recon_marpat_d_sniper_M107: rhsusf_usmc_recon_marpat_wd_sniper_M107
	{
		uniformClass="rhs_uniform_FROG01_d";
		weapons[]=
		{
			"rhs_weap_M107_d_premier",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M107_d_premier",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_recon_marpat_d_officer: rhsusf_usmc_recon_marpat_wd_officer
	{
		uniformClass="rhs_uniform_FROG01_d";
		weapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_recon_marpat_d_teamleader: rhsusf_usmc_recon_marpat_wd_teamleader
	{
		uniformClass="rhs_uniform_FROG01_d";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_tan_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_tan_headset",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_rifleman_fast: rhsusf_usmc_recon_marpat_wd_rifleman_fast
	{
		uniformClass="rhs_uniform_FROG01_d";
		weapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_recon_marpat_d_rifleman_at_fast: rhsusf_usmc_recon_marpat_wd_rifleman_at_fast
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_recon_marpat_d_autorifleman_fast: rhsusf_usmc_recon_marpat_wd_autorifleman_fast
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_recon_marpat_d_machinegunner_m249_fast: rhsusf_usmc_recon_marpat_wd_machinegunner_m249_fast
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_recon_marpat_d_marksman_fast: rhsusf_usmc_recon_marpat_wd_marksman_fast
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_recon_marpat_d_teamleader_fast: rhsusf_usmc_recon_marpat_wd_teamleader_fast
	{
		uniformClass="rhs_uniform_FROG01_d";
	};
	class rhsusf_usmc_recon_marpat_d_rifleman_lite: rhsusf_usmc_recon_marpat_wd_rifleman_lite
	{
		uniformClass="rhs_uniform_FROG01_d";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_arid",
			"Head_NATO_camo_semiarid"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"Binocular",
			"Throw",
			"Put"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_rifleman_at_lite: rhsusf_usmc_recon_marpat_wd_rifleman_at_lite
	{
		uniformClass="rhs_uniform_FROG01_d";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_arid",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_autorifleman_lite: rhsusf_usmc_recon_marpat_wd_autorifleman_lite
	{
		uniformClass="rhs_uniform_FROG01_d";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_arid",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_machinegunner_m249_lite: rhsusf_usmc_recon_marpat_wd_machinegunner_m249_lite
	{
		uniformClass="rhs_uniform_FROG01_d";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_arid",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_iar",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_usmc_recon_marpat_d_marksman_lite: rhsusf_usmc_recon_marpat_wd_marksman_lite
	{
		uniformClass="rhs_uniform_FROG01_d";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_arid",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_m1911a1",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
	};
	class rhsusf_usmc_recon_marpat_d_teamleader_lite: rhsusf_usmc_recon_marpat_wd_teamleader_lite
	{
		uniformClass="rhs_uniform_FROG01_d";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO_camo_arid",
			"Head_NATO_camo_semiarid"
		};
		linkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_booniehat2_marpatd",
			"rhsusf_spc_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_infantry_socom_armysf_base: rhsusf_socom_mc_uniform
	{
		side=1;
		selectionClan="clan";
		identityTypes[]=
		{
			"LanguageENG_F",
			"Head_NATO"
		};
		accuracy=2.3;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		nakedUniform="U_BasicBody";
		uniformClass="rhs_uniform_g3_mc";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ocp"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_14",
			"rhsusf_patrolcap_ocp"
		};
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_eotech",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_eotech",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
			"rhs_mag_an_m14_th3",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_m67"
		};
		class EventHandlers;
	};
	class rhsusf_infantry_socom_armysf_rifleman: rhsusf_infantry_socom_armysf_base
	{
	};
	class rhsusf_infantry_socom_marsoc_base: rhsusf_socom_g3_m81_uniform
	{
		side=1;
		accuracy=3;
		camouflage=0.60000002;
		sensitivity=3.3;
		detectSkill=30;
		uniformClass="rhs_uniform_g3_m81";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhs_Booniehat_m81",
			"rhsusf_acc_nt4_black"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhs_Booniehat_m81",
			"rhsusf_acc_nt4_black"
		};
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_mk84",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_socom_marsoc_cso: rhsusf_infantry_socom_marsoc_base
	{
	};
	class rhsusf_socom_marsoc_cso_mk17: rhsusf_infantry_socom_marsoc_base
	{
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhs_Booniehat_m81",
			"rhsusf_acc_aac_scarh_silencer"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhs_Booniehat_m81",
			"rhsusf_acc_aac_scarh_silencer"
		};
		linkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk17_CQC_SU230A_marsoc",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk17_CQC_SU230A_marsoc",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_mk84",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_socom_marsoc_cso_cqb: rhsusf_infantry_socom_marsoc_base
	{
		linkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk18_eotech",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk18_eotech",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_socom_marsoc_cso_light: rhsusf_socom_marsoc_cso_cqb
	{
		cost=250000;
		threat[]={1,0.30000001,0.30000001};
		camouflage=0.5;
		sensitivity=3.3;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		linkedItems[]=
		{
			"rhs_Booniehat_m81",
			"rhsusf_mbav_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_Booniehat_m81",
			"rhsusf_mbav_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk18_eotech_sup",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk18_eotech_sup",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsusf_socom_marsoc_cso_mk17_light: rhsusf_socom_marsoc_cso_light
	{
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		linkedItems[]=
		{
			"H_Cap_headphones",
			"rhsusf_plateframe_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Cap_headphones",
			"rhsusf_plateframe_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk17_CQC_G33_sup_marsoc",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk17_CQC_G33_sup_marsoc",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhs_mag_20Rnd_SCAR_762x51_m80a1_epr",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_mk84",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_socom_marsoc_cso_breacher: rhsusf_socom_marsoc_cso_cqb
	{
		icon="iconManExplosive";
		linkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsusf_assault_eagleaiii_coy_demo";
	};
	class rhsusf_socom_marsoc_cso_grenadier: rhsusf_infantry_socom_marsoc_base
	{
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_grenadier",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4a1_blockII_M203_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4a1_blockII_M203_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_an_m8hc",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red"
		};
		backpack="rhsusf_falconii_coy_gr";
	};
	class rhsusf_socom_marsoc_teamleader: rhsusf_infantry_socom_marsoc_base
	{
		nameSound="veh_infantry_officer_s";
		icon="iconManOfficer";
		camouflage=1.6;
		sensitivity=3;
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230_sup",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230_sup",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green",
			"rhs_mag_m18_yellow",
			"I_IR_Grenade",
			"I_IR_Grenade",
			"rhs_mag_m67"
		};
	};
	class rhsusf_socom_marsoc_teamchief: rhsusf_socom_marsoc_teamleader
	{
		icon="iconManLeader";
		cost=450000;
		sensitivity=3.2;
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_headset",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_headset",
			"rhsusf_mbav_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
	};
	class rhsusf_socom_marsoc_elementleader: rhsusf_socom_marsoc_cso_cqb
	{
		icon="iconManLeader";
		cost=450000;
		camouflage=1.4;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_mar_ut_pelt",
			"rhsusf_plateframe_teamleader",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk18_eotech",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk18_eotech",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsusf_socom_marsoc_jtac: rhsusf_infantry_socom_marsoc_base
	{
		icon="iconManLeader";
		cost=450000;
		sensitivity=3.2;
		linkedItems[]=
		{
			"rhsusf_opscore_mar_fg_pelt",
			"rhsusf_mbav_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_mar_fg_pelt",
			"rhsusf_mbav_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_eotech_soc",
			"rhsusf_weap_glock17g4",
			"Laserdesignator",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_eotech_soc",
			"rhsusf_weap_glock17g4",
			"Laserdesignator",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_yellow",
			"rhs_mag_m18_purple",
			"rhs_mag_m18_purple",
			"I_IR_Grenade",
			"I_IR_Grenade",
			"Laserbatteries",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsusf_socom_marsoc_jfo: rhsusf_socom_marsoc_jtac
	{
		icon="iconManLeader";
		cost=450000;
		sensitivity=3.2;
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_headset",
			"rhsusf_mbav_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_alt_headset",
			"rhsusf_mbav_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk18_SU230",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk18_SU230",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_black",
			"Throw",
			"Put"
		};
	};
	class rhsusf_socom_marsoc_cso_eod: rhsusf_infantry_socom_marsoc_base
	{
		cost=115000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		detectSkill=80;
		threat[]={1,0.5,0.1};
		icon="iconManExplosive";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos",
			"rhsusf_mbav_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk18_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk18_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		backpack="rhsusf_assault_eagleaiii_coy_eod";
	};
	class rhsusf_socom_marsoc_cso_mechanic: rhsusf_infantry_socom_marsoc_base
	{
		cost=220000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		engineer=1;
		detectSkill=60;
		threat[]={1,0.5,0.1};
		icon="iconManEngineer";
		backpack="rhsusf_assault_eagleaiii_coy_engineer";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_tan",
			"rhsusf_mbav_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_tan",
			"rhsusf_mbav_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_eotech_soc",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_eotech_soc",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
	};
	class rhsusf_socom_marsoc_sarc: rhsusf_infantry_socom_marsoc_base
	{
		cost=290000;
		attendant=1;
		icon="iconManMedic";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_tan",
			"rhsusf_mbav_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_tan",
			"rhsusf_mbav_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk18_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk18_SU230",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		backpack="rhsusf_falconii_sarc";
		class EventHandlers;
	};
	class rhsusf_socom_marsoc_spotter: rhsusf_infantry_socom_marsoc_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=250000;
		threat[]={1,0.30000001,0.30000001};
		camouflage=0.60000002;
		sensitivity=3.5;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		linkedItems[]=
		{
			"rhs_Booniehat_m81",
			"rhsusf_mbav_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_Booniehat_m81",
			"rhsusf_mbav_light",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230_sup",
			"rhsusf_weap_glock17g4",
			"Leupold_Mk4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m4a1_blockII_KAC_SU230_sup",
			"rhsusf_weap_glock17g4",
			"Leupold_Mk4",
			"Throw",
			"Put"
		};
	};
	class rhsusf_socom_marsoc_sniper: rhsusf_socom_marsoc_spotter
	{
		cost=350000;
		threat[]={1,0.60000002,0.60000002};
		camouflage=0.60000002;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhs_Booniehat_m81",
			"rhsusf_mbav_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_Booniehat_m81",
			"rhsusf_mbav_mg",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_sr25_sup_marsoc",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_20Rnd_762x51_SR25_m118_special_Mag",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_socom_marsoc_sniper_m107: rhsusf_socom_marsoc_sniper
	{
		cost=350000;
		threat[]={1,0.60000002,0.60000002};
		camouflage=0.60000002;
		sensitivity=3.3;
		weapons[]=
		{
			"rhs_weap_M107_premier",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_M107_premier",
			"rhsusf_weap_glock17g4",
			"rhsusf_bino_lerca_1200_tan",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_M33",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_10Rnd_STD_50BMG_mk211",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_an_m8hc",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsusf_socom_marsoc_marksman: rhsusf_socom_marsoc_cso
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhs_Booniehat_m81",
			"rhsusf_acc_aac_scarh_silencer"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15",
			"rhs_Booniehat_m81",
			"rhsusf_acc_aac_scarh_silencer"
		};
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc",
			"rhsusf_plateframe_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc",
			"rhsusf_plateframe_marksman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mk17_LB_DMR_marsoc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mk17_LB_DMR_marsoc",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_SCAR_762x51_m118_special",
			"rhs_mag_20Rnd_SCAR_762x51_m118_special",
			"rhs_mag_20Rnd_SCAR_762x51_m118_special",
			"rhs_mag_20Rnd_SCAR_762x51_m118_special",
			"rhs_mag_20Rnd_SCAR_762x51_m118_special",
			"rhs_mag_20Rnd_SCAR_762x51_m118_special",
			"Chemlight_red",
			"rhs_mag_an_m8hc",
			"rhs_mag_mk84",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsusf_socom_swcc_crewman: rhsusf_infantry_socom_marsoc_base
	{
		uniformClass="rhs_uniform_g3_aor2";
		Items[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsusf_ANPVS_15"
		};
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_headset",
			"rhsusf_spc_patchless",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_arc_alt_headset",
			"rhsusf_spc_patchless",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_eotech",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_eotech",
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_ranger",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_ranger",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_ranger",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_ranger",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_ranger",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_ranger",
			"rhs_mag_30Rnd_556x45_M855A1_Stanag_ranger",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"Chemlight_red",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green",
			"rhs_mag_m18_yellow"
		};
	};
	class rhsusf_socom_swcc_officer: rhsusf_socom_swcc_crewman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_officer_s"
				};
				speechPlural[]=
				{
					"veh_infantry_officer_p"
				};
			};
		};
		nameSound="veh_infantry_officer_s";
		cost=600000;
		camouflage=1.6;
		sensitivity=3;
		icon="iconManOfficer";
		linkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_spc_patchless_radio",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_ess",
			"rhsusf_spc_patchless_radio",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"rhsusf_mag_15Rnd_9x19_JHP",
			"Chemlight_red",
			"rhs_mag_m18_red",
			"rhs_mag_m18_green",
			"rhs_mag_m18_yellow"
		};
	};
	class rhsusf_assault_eagleaiii_coy_medic: rhsusf_assault_eagleaiii_coy
	{
	};
	class rhsusf_falconii_sarc: rhsusf_falconii
	{
	};
	class rhsusf_navy_marpat_wd_medic: rhsusf_usmc_marpat_wd_rifleman_m4
	{
		backpack="rhsusf_assault_eagleaiii_coy_medic";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatwd",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		cost=290000;
		attendant=1;
		icon="iconManMedic";
	};
	class rhsusf_navy_marpat_d_medic: rhsusf_usmc_marpat_d_rifleman_m4
	{
		backpack="rhsusf_assault_eagleaiii_coy_medic";
		linkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_helmet_marpatd",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		cost=290000;
		attendant=1;
		icon="iconManMedic";
	};
	class rhsusf_navy_sarc_w: rhsusf_infantry_usmc_recon_base
	{
		cost=290000;
		attendant=1;
		icon="iconManMedic";
		linkedItems[]=
		{
			"rhsusf_mich_bare_norotos_tan",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_mich_bare_norotos_tan",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhs_mag_30Rnd_556x45_Mk318_Stanag",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_mk3a2",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
		backpack="rhsusf_falconii_sarc";
		class EventHandlers;
	};
	class rhsusf_navy_sarc_d: rhsusf_navy_sarc_w
	{
		uniformClass="rhs_uniform_FROG01_d";
		weapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_m4a1_grip_acog2_usmc_sup_tan",
			"rhsusf_weap_m9",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsusf_navy_sarc_w_fast: rhsusf_navy_sarc_w
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_navy_sarc_d_fast: rhsusf_navy_sarc_d
	{
		linkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_opscore_ut",
			"rhsusf_spc_corpsman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsusf_airforce_jetpilot: rhsusf_usmc_marpat_wd_rifleman_m4
	{
		uniformClass="U_B_HeliPilotCoveralls";
		headgearList[]={};
		linkedItems[]=
		{
			"RHS_jetpilot_usaf",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"RHS_jetpilot_usaf",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_m9",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhsusf_mag_15Rnd_9x19_FMJ",
			"rhs_mag_m18_purple",
			"rhs_mag_an_m8hc"
		};
		cost=165000;
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_pilot_s"
				};
				speechPlural[]=
				{
					"veh_infantry_pilot_p"
				};
			};
		};
		nameSound="veh_infantry_pilot_s";
		class Wounds
		{
		};
	};
	class rhsusf_airforce_pilot: rhsusf_airforce_jetpilot
	{
		backpack="";
		uniformClass="U_B_HeliPilotCoveralls";
		headgearList[]={};
		linkedItems[]=
		{
			"H_Cap_headphones",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Cap_headphones",
			"ItemGPS",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
};
class UniformItem;
class CfgGlasses
{
	class G_Combat;
	class None
	{
		identityTypes[]+=
		{
			"rhsusf_g_army",
			5,
			"rhsusf_g_usmc",
			10
		};
	};
	class rhs_googles_black: G_Combat
	{
		displayname="$STR_RHS_GOOGLES_BLACK_NAME";
		identityTypes[]=
		{
			"rhsusf_g_army",
			80,
			"rhsusf_g_usmc",
			5
		};
	};
	class rhs_googles_clear: G_Combat
	{
		displayname="$STR_RHS_GOOGLES_CLEAR_NAME";
		identityTypes[]=
		{
			"rhsusf_g_army",
			15,
			"rhsusf_g_usmc",
			85
		};
	};
	class rhs_googles_yellow: G_Combat
	{
		displayname="$STR_RHS_GOOGLES_CLEAR_NAME";
	};
	class rhs_googles_orange: G_Combat
	{
		displayname="$STR_RHS_GOOGLES_CLEAR_NAME";
	};
	class rhs_ess_black: G_Combat
	{
		displayname="$STR_RHS_ESS_BLACK_NAME";
		identityTypes[]=
		{
			"rhsusf_g_army",
			80,
			"rhsusf_g_usmc",
			5
		};
	};
	class rhsusf_oakley_goggles_base: G_Combat
	{
		displayname="SI Ballistic 2.0";
		hiddenSelections[]=
		{
			"Camo2",
			"Camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
		identityTypes[]=
		{
			"rhsusf_g_army",
			0,
			"rhsusf_g_usmc",
			0
		};
	};
	class rhsusf_oakley_goggles_blk: rhsusf_oakley_goggles_base
	{
		displayname="SI Ballistic 2.0 (Black)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_oakley_goggles_clr: rhsusf_oakley_goggles_base
	{
		displayname="SI Ballistic 2.0 (Clear)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\glass_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_oakley_goggles_ylw: rhsusf_oakley_goggles_base
	{
		displayname="SI Ballistic 2.0 (Yellow)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\yellow_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh_base: G_Combat
	{
		displayname="Shemagh";
		hiddenSelections[]=
		{
			"Camo"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_co.paa"
		};
		identityTypes[]=
		{
			"rhsusf_g_army",
			0,
			"rhsusf_g_usmc",
			0
		};
	};
	class rhsusf_shemagh_white: rhsusf_shemagh_base
	{
		displayname="Shemagh (White)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_co.paa"
		};
	};
	class rhsusf_shemagh_tan: rhsusf_shemagh_base
	{
		displayname="Shemagh (Tan)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_tan_co.paa"
		};
	};
	class rhsusf_shemagh_od: rhsusf_shemagh_base
	{
		displayname="Shemagh (OD)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_od_co.paa"
		};
	};
	class rhsusf_shemagh_grn: rhsusf_shemagh_base
	{
		displayname="Shemagh (Green)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_grn_co.paa"
		};
	};
	class rhsusf_shemagh_gogg_base: G_Combat
	{
		displayname="Shemagh w/Goggles";
		hiddenSelections[]=
		{
			"Camo",
			"Camo2",
			"Camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\shemagh_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
		identityTypes[]=
		{
			"rhsusf_g_army",
			0,
			"rhsusf_g_usmc",
			0
		};
	};
	class rhsusf_shemagh_gogg_white: rhsusf_shemagh_gogg_base
	{
		displayname="Shemagh w/Goggles (White)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh_gogg_tan: rhsusf_shemagh_gogg_base
	{
		displayname="Shemagh w/Goggles (Tan)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_tan_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh_gogg_od: rhsusf_shemagh_gogg_base
	{
		displayname="Shemagh w/Goggles (OD)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_od_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh_gogg_grn: rhsusf_shemagh_gogg_base
	{
		displayname="Shemagh w/Goggles (Green)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh_grn_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh2_base: G_Combat
	{
		displayname="Shemagh (alt)";
		hiddenSelections[]=
		{
			"Camo"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_co.paa"
		};
		identityTypes[]=
		{
			"rhsusf_g_army",
			0,
			"rhsusf_g_usmc",
			0
		};
	};
	class rhsusf_shemagh2_white: rhsusf_shemagh2_base
	{
		displayname="Shemagh (White/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_co.paa"
		};
	};
	class rhsusf_shemagh2_tan: rhsusf_shemagh2_base
	{
		displayname="Shemagh (Tan/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_tan_co.paa"
		};
	};
	class rhsusf_shemagh2_od: rhsusf_shemagh2_base
	{
		displayname="Shemagh (OD/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_od_co.paa"
		};
	};
	class rhsusf_shemagh2_grn: rhsusf_shemagh2_base
	{
		displayname="Shemagh (Green/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_grn_co.paa"
		};
	};
	class rhsusf_shemagh2_gogg_base: G_Combat
	{
		displayname="Shemagh w/Goggles (alt)";
		hiddenSelections[]=
		{
			"Camo",
			"Camo2",
			"Camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
		identityTypes[]=
		{
			"rhsusf_g_army",
			0,
			"rhsusf_g_usmc",
			0
		};
	};
	class rhsusf_shemagh2_gogg_white: rhsusf_shemagh2_gogg_base
	{
		displayname="Shemagh w/Goggles (White/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh2_gogg_tan: rhsusf_shemagh2_gogg_base
	{
		displayname="Shemagh w/Goggles (Tan/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_tan_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh2_gogg_od: rhsusf_shemagh2_gogg_base
	{
		displayname="Shemagh w/Goggles (OD/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_od_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
	class rhsusf_shemagh2_gogg_grn: rhsusf_shemagh2_gogg_base
	{
		displayname="Shemagh w/Goggles (Green/alt)";
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\shemagh\data\shemagh2_grn_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\dark_ca.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\goggles\data\oakleysblk_co.paa"
		};
	};
};
class CfgWeapons
{
	class Uniform_Base;
	class rhs_uniform_g3_mc: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_socom_mc_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_g3_m81: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_socom_g3_m81_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_g3_aor2: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_socom_g3_aor2_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_g3_tan: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_socom_g3_tan_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_g3_rgr: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_socom_g3_rgr_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_g3_blk: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_socom_g3_blk_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ocp: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ocp_crewman";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ucp: rhs_uniform_cu_ocp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ucp_crewman";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ocp_1stcav: rhs_uniform_cu_ocp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ocp_rifleman_1stcav";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ucp_1stcav: rhs_uniform_cu_ucp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ucp_rifleman_1stcav";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ocp_82nd: rhs_uniform_cu_ocp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ocp_rifleman_82nd";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ucp_82nd: rhs_uniform_cu_ucp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ucp_rifleman_82nd";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ocp_101st: rhs_uniform_cu_ocp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ocp_rifleman_101st";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ucp_101st: rhs_uniform_cu_ucp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ucp_rifleman_101st";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ocp_10th: rhs_uniform_cu_ocp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ocp_rifleman_10th";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ucp_10th: rhs_uniform_cu_ucp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_ucp_rifleman_10th";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_cu_ucp_patchless: rhs_uniform_cu_ocp
	{
	};
	class rhs_uniform_cu_ocp_patchless: rhs_uniform_cu_ucp
	{
	};
	class rhs_uniform_FROG01_d: rhs_uniform_cu_ocp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_usmc_marpat_d_crewman";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_FROG01_wd: rhs_uniform_FROG01_d
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_usmc_marpat_wd_rifleman";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_FROG01_m81: rhs_uniform_g3_m81
	{
	};
	class rhs_uniform_acu_ucp: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_acu_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_acu_ucpd: rhs_uniform_acu_ucp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_acu_ucpd_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_acu_oefcp: rhs_uniform_acu_ucp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_acu_oefcp_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_acu_ocp: rhs_uniform_acu_ucp
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_acu_ocp_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_abu: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_airforce_abu_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhs_uniform_bdu_erdl: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsusf_army_bdu_erdl_uniform";
			containerClass="Supply40";
			mass=40;
		};
	};
	class ItemCore;
	class H_HelmetB: ItemCore
	{
		class ItemInfo;
	};
	class HeadGearItem;
	class InventoryItem_Base_F;
	class InventoryMuzzleItem_Base_F;
	class InventoryOpticsItem_Base_F;
	class rhsusf_patrolcap_ocp: H_HelmetB
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]={};
		hiddenSelectionsTextures[]={};
		class ItemInfo: HeadGearItem
		{
			mass=5;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\rhsusf_patrolcap";
			allowedSlots[]={801,901,701,605};
		};
	};
	class rhsusf_patrolcap_ucp: rhsusf_patrolcap_ocp
	{
		hiddenSelections[]=
		{
			"camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\patrolcap_ucp_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo1"
			};
		};
	};
	class rhs_xmas_antlers: ItemCore
	{
		weaponPoolAvailable=1;
		hiddenSelections[]={};
		hiddenSelectionsTextures[]={};
		class ItemInfo: HeadGearItem
		{
			mass=10;
			allowedSlots[]={801,901,701,605};
			uniformModel="\rhsusf\addons\rhsusf_xmas\rhs_xmas_antlers";
			hiddenSelections[]={};
		};
	};
	class rhs_Booniehat_ocp: ItemCore
	{
		hiddenSelections[]=
		{
			"camo"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\booniehat_ocp_co"
		};
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		class ItemInfo: HeadGearItem
		{
			mass=10;
			allowedSlots[]={801,901,701,605};
			uniformModel="\A3\Characters_F\Common\booniehat";
			hiddenSelections[]=
			{
				"camo"
			};
		};
	};
	class rhs_Booniehat_ucp: rhs_Booniehat_ocp
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\booniehat_ucp_co"
		};
	};
	class rhs_Booniehat_marpatd: rhs_Booniehat_ocp
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\booniehat_marpatd_co"
		};
	};
	class rhs_Booniehat_marpatwd: rhs_Booniehat_ocp
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\booniehat_marpatwd_co"
		};
	};
	class rhs_Booniehat_m81: rhs_Booniehat_ocp
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\booniehat_m81_co"
		};
	};
	class rhs_booniehat2_marpatd: ItemCore
	{
		weaponPoolAvailable=1;
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"camo"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hats\data\uniform_bdu_caps_marpatd_co"
		};
		class ItemInfo: HeadGearItem
		{
			mass=10;
			allowedSlots[]={801,901,701,605};
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\hats\boonie";
			hiddenSelections[]=
			{
				"camo"
			};
		};
	};
	class rhs_booniehat2_marpatwd: rhs_booniehat2_marpatd
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hats\data\uniform_bdu_caps_marpatwd_co"
		};
	};
	class rhs_8point_marpatd: ItemCore
	{
		weaponPoolAvailable=1;
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"camo"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hats\data\uniform_bdu_caps_marpatd_co"
		};
		class ItemInfo: HeadGearItem
		{
			mass=10;
			allowedSlots[]={801,901,701,605};
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\hats\8point";
			hiddenSelections[]=
			{
				"camo"
			};
		};
	};
	class rhs_8point_marpatwd: rhs_8point_marpatd
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hats\data\uniform_bdu_caps_marpatwd_co"
		};
	};
	class rhsusf_ach_helmet_ocp: H_HelmetB
	{
		allowedFacewear[]=
		{
			"",
			7,
			"rhs_ess_black",
			2,
			"rhs_googles_black",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			hiddenSelections[]=
			{
				"Camo",
				"Camo1"
			};
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_01";
			allowedSlots[]={801,901,701,605};
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=6;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsusf_ach_helmet_ocp_alt: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_ocp_norotos: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_norotos_01";
		};
	};
	class rhsusf_ach_helmet_ucp: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_ucp_alt: rhsusf_ach_helmet_ucp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_ach_helmet_ucp_norotos: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_norotos_01";
		};
	};
	class rhsusf_ach_helmet_M81: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_M81_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_DCU: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry3\gear\head\data\rhs_helmet_ach_DCU_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_DCU_early: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry3\gear\head\data\rhs_helmet_ach_DCU_early_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry3\gear\head\ach_early";
		};
	};
	class rhsusf_ach_helmet_DCU_early_rhino: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry3\gear\head\data\rhs_helmet_ach_DCU_early_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry3\gear\head\ach_early_rhino";
		};
	};
	class rhsusf_ach_helmet_camo_ocp: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\camo_net_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_camo";
			hiddenSelections[]=
			{
				"Camo",
				"Camo1",
				"Camo2"
			};
		};
	};
	class rhsusf_ach_helmet_headset_ocp: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_headset";
		};
	};
	class rhsusf_ach_helmet_headset_ocp_alt: rhsusf_ach_helmet_headset_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_headset_ucp: rhsusf_ach_helmet_headset_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_headset_ucp_alt: rhsusf_ach_helmet_headset_ucp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_ach_helmet_ESS_ocp: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_ess_01";
		};
	};
	class rhsusf_ach_helmet_ESS_ocp_alt: rhsusf_ach_helmet_ESS_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_ESS_ucp: rhsusf_ach_helmet_ESS_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_ESS_ucp_alt: rhsusf_ach_helmet_ESS_ucp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_ach_helmet_headset_ess_ocp: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_headset_ess";
		};
	};
	class rhsusf_ach_helmet_headset_ess_ocp_alt: rhsusf_ach_helmet_headset_ess_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ocp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_headset_ess_ucp: rhsusf_ach_helmet_headset_ess_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_ach_helmet_headset_ess_ucp_alt: rhsusf_ach_helmet_headset_ess_ucp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_ucp_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_ach_bare: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_green_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_01";
		};
	};
	class rhsusf_ach_bare_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_green_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_ess_01";
		};
	};
	class rhsusf_ach_bare_headset: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_green_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset";
		};
	};
	class rhsusf_ach_bare_headset_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_green_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset_ess";
		};
	};
	class rhsusf_ach_bare_tan: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_01";
		};
	};
	class rhsusf_ach_bare_tan_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_ess_01";
		};
	};
	class rhsusf_ach_bare_tan_headset: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset";
		};
	};
	class rhsusf_ach_bare_tan_headset_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset_ess";
		};
	};
	class rhsusf_ach_bare_wood: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_wood_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_01";
		};
	};
	class rhsusf_ach_bare_wood_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_wood_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_ess_01";
		};
	};
	class rhsusf_ach_bare_wood_headset: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_wood_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset";
		};
	};
	class rhsusf_ach_bare_wood_headset_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_wood_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset_ess";
		};
	};
	class rhsusf_ach_bare_des: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_des_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_01";
		};
	};
	class rhsusf_ach_bare_des_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_des_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_ess_01";
		};
	};
	class rhsusf_ach_bare_des_headset: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_des_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset";
		};
	};
	class rhsusf_ach_bare_des_headset_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_des_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset_ess";
		};
	};
	class rhsusf_ach_bare_semi: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_semi_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_01";
		};
	};
	class rhsusf_ach_bare_semi_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_semi_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_ess_01";
		};
	};
	class rhsusf_ach_bare_semi_headset: rhsusf_ach_helmet_ocp
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_semi_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset";
		};
	};
	class rhsusf_ach_bare_semi_headset_ess: rhsusf_ach_helmet_ocp
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo",
			"Camo1"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_mich_semi_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\ach_bare_headset_ess";
		};
	};
	class rhsusf_opscore_01: rhsusf_ach_helmet_ocp
	{
		descriptionshort="OpsCore FAST Ballistic Helmet";
		hiddenSelections[]=
		{
			"opscore",
			"comtac",
			"NSW",
			"contour",
			"UNCOV_FLAG",
			"ms2000"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"",
			"",
			"",
			"",
			""
		};
		class ItemInfo: ItemInfo
		{
			mass=30;
			armor=6;
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\opscore";
			hiddenSelections[]=
			{
				"opscore",
				"comtac",
				"NSW",
				"contour",
				"UNCOV_FLAG",
				"ms2000"
			};
		};
	};
	class rhsusf_opscore_fg: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			""
		};
	};
	class rhsusf_opscore_fg_pelt: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"",
			"",
			"",
			"",
			""
		};
	};
	class rhsusf_opscore_fg_pelt_nsw: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_fg_pelt_cam: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"",
			""
		};
	};
	class rhsusf_opscore_ut: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa"
		};
	};
	class rhsusf_opscore_ut_pelt: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			""
		};
	};
	class rhsusf_opscore_ut_pelt_cam: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"",
			""
		};
	};
	class rhsusf_opscore_ut_pelt_nsw: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_ut_pelt_nsw_cam: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_bk: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_blk_co.paa"
		};
	};
	class rhsusf_opscore_bk_pelt: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_blk_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa"
		};
	};
	class rhsusf_opscore_mc: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_MC_co.paa"
		};
	};
	class rhsusf_opscore_mc_pelt: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_MC2_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa"
		};
	};
	class rhsusf_opscore_mc_pelt_nsw: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_MC2_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_aor1: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_aor1_co.paa"
		};
	};
	class rhsusf_opscore_aor1_pelt: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_aor1_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa"
		};
	};
	class rhsusf_opscore_aor1_pelt_nsw: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_aor1_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_aor2: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_aor2_co.paa"
		};
	};
	class rhsusf_opscore_aor2_pelt: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_aor2_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa"
		};
	};
	class rhsusf_opscore_aor2_pelt_nsw: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_aor2_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_paint: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_Paint2_co.paa"
		};
	};
	class rhsusf_opscore_paint_pelt: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_Paint1_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa"
		};
	};
	class rhsusf_opscore_paint_pelt_nsw: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_Paint3_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_paint_pelt_nsw_cam: rhsusf_opscore_01
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_Paint3_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_cover: rhsusf_opscore_01
	{
		hiddenSelections[]=
		{
			"opscore",
			"comtac",
			"NSW",
			"contour",
			"cover",
			"ms2000"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_cb_co.paa",
			""
		};
		class ItemInfo: ItemInfo
		{
			mass=30;
			armor=6;
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\opscore_cover";
			hiddenSelections[]=
			{
				"opscore",
				"comtac",
				"NSW",
				"contour",
				"cover",
				"ms2000"
			};
		};
	};
	class rhsusf_opscore_mc_cover: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_co.paa",
			""
		};
	};
	class rhsusf_opscore_mc_cover_pelt: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_co.paa",
			""
		};
	};
	class rhsusf_opscore_mc_cover_pelt_nsw: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_mc_cover_pelt_cam: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_rg_cover: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_rg_co.paa",
			""
		};
	};
	class rhsusf_opscore_rg_cover_pelt: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_rg_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_coy_cover: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"",
			"",
			"",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_cb_co.paa",
			""
		};
	};
	class rhsusf_opscore_coy_cover_pelt: rhsusf_opscore_cover
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\opscore_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\nsw_co.paa",
			"",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\fs_cover_cb_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
	};
	class rhsusf_opscore_mar_01: rhsusf_ach_helmet_ocp
	{
		descriptionshort="OpsCore FAST Maritime Helmet";
		hiddenSelections[]=
		{
			"Camo1",
			"Camo2",
			"comtac"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\maritime_tan.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\tan2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=30;
			armor=6;
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\opscore\maritime";
			hiddenSelections[]=
			{
				"Camo1",
				"Camo2",
				"comtac"
			};
		};
	};
	class rhsusf_opscore_mar_ut: rhsusf_opscore_mar_01
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\maritime_tan.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\tan2_co.paa",
			""
		};
	};
	class rhsusf_opscore_mar_ut_pelt: rhsusf_opscore_mar_01
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\maritime_tan.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\tan2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_co.paa"
		};
	};
	class rhsusf_opscore_mar_fg: rhsusf_opscore_mar_01
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\maritime_od.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\od2_co.paa",
			""
		};
	};
	class rhsusf_opscore_mar_fg_pelt: rhsusf_opscore_mar_01
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\maritime_od.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\od2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\head\opscore\data\comtac_fg_co.paa"
		};
	};
	class rhsusf_opscore_01_tan: rhsusf_opscore_01
	{
	};
	class rhsusf_opscore_02: rhsusf_opscore_01
	{
	};
	class rhsusf_opscore_02_tan: rhsusf_opscore_01
	{
	};
	class rhsusf_opscore_03_ocp: rhsusf_opscore_01
	{
	};
	class rhsusf_opscore_04_ocp: rhsusf_opscore_01
	{
	};
	class rhsusf_cvc_helmet: rhsusf_opscore_01
	{
		allowedFacewear[]=
		{
			"",
			2,
			"rhs_ess_black",
			2,
			"rhs_googles_black",
			2,
			"rhs_googles_clear",
			1,
			"rhsusf_shemagh2_tan",
			5,
			"rhsusf_shemagh2_gogg_tan",
			10
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\cvc\data\rhs_cvc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"Camo1"
			};
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\cvc\rhs_cvc";
		};
	};
	class rhsusf_cvc_alt_helmet: rhsusf_cvc_helmet
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\cvc\data\rhs_cvc_alt_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\cvc\rhs_cvc_alt";
		};
	};
	class rhsusf_cvc_green_helmet: rhsusf_cvc_helmet
	{
		allowedFacewear[]=
		{
			"",
			5,
			"rhs_ess_black",
			2,
			"rhs_googles_black",
			2,
			"rhs_googles_clear",
			1,
			"rhsusf_shemagh2_od",
			3,
			"rhsusf_shemagh2_gogg_od",
			5
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\cvc\data\rhs_cvc_green_co.paa"
		};
	};
	class rhsusf_cvc_green_alt_helmet: rhsusf_cvc_alt_helmet
	{
		allowedFacewear[]=
		{
			"",
			5,
			"rhs_ess_black",
			2,
			"rhs_googles_black",
			2,
			"rhs_googles_clear",
			1,
			"rhsusf_shemagh2_od",
			3,
			"rhsusf_shemagh2_gogg_od",
			5
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\cvc\data\rhs_cvc_green_alt_co.paa"
		};
	};
	class rhsusf_cvc_ess: rhsusf_cvc_helmet
	{
		allowedFacewear[]=
		{
			"",
			2,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1,
			"rhsusf_shemagh2_tan",
			4
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\cvc\data\rhs_cvc_alt_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\cvc\rhs_cvc_alt_ess";
		};
	};
	class rhsusf_cvc_green_ess: rhsusf_cvc_ess
	{
		allowedFacewear[]=
		{
			"",
			2,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1,
			"rhsusf_shemagh2_od",
			4
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\head\cvc\data\rhs_cvc_green_alt_co.paa"
		};
	};
	class rhsusf_hgu56p: rhsusf_opscore_01
	{
		allowedFacewear[]={};
		hiddenSelections[]=
		{
			"camo",
			"patches"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_olv_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches_co.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56.rvmat",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches.rvmat"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"patches"
			};
			mass=40;
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor: rhsusf_hgu56p
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class rhsusf_hgu56p_mask: rhsusf_hgu56p
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask: rhsusf_hgu56p
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_skull: rhsusf_hgu56p_visor_mask
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_olv_skull_co.paa"
		};
	};
	class rhsusf_hgu56p_mask_skull: rhsusf_hgu56p_mask
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_olv_skull_co.paa"
		};
	};
	class rhsusf_hgu56p_visor_mask_mo: rhsusf_hgu56p_visor_mask
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_olv_mo_co.paa"
		};
	};
	class rhsusf_hgu56p_mask_mo: rhsusf_hgu56p_mask
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_olv_mo_co.paa"
		};
	};
	class rhsusf_hgu56p_mask_smiley: rhsusf_hgu56p
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_smiley_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_smiley: rhsusf_hgu56p_mask_smiley
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_black: rhsusf_hgu56p
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_blk_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor_black: rhsusf_hgu56p_black
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class rhsusf_hgu56p_mask_black: rhsusf_hgu56p_black
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_black: rhsusf_hgu56p_black
	{
		hiddenSelections[]=
		{
			"camo",
			"patches"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_blk_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches_usPirate_co.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56.rvmat",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches.rvmat"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"patches"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_black_skull: rhsusf_hgu56p_visor_mask_black
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_blk_skull_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches_usPirate_co.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56.rvmat",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches.rvmat"
		};
	};
	class rhsusf_hgu56p_mask_black_skull: rhsusf_hgu56p_visor_mask_black_skull
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_Empire_black: rhsusf_hgu56p_black
	{
		hiddenSelections[]=
		{
			"camo",
			"patches"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_blk_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches_empire_co.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56.rvmat",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches.rvmat"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"patches"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_green: rhsusf_hgu56p
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_green_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor_green: rhsusf_hgu56p_green
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class rhsusf_hgu56p_mask_green: rhsusf_hgu56p_green
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_green: rhsusf_hgu56p_green
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_mask_green_mo: rhsusf_hgu56p_mask_green
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_green_mo_co.paa"
		};
	};
	class rhsusf_hgu56p_visor_mask_green_mo: rhsusf_hgu56p_visor_mask_green
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_green_mo_co.paa"
		};
	};
	class rhsusf_hgu56p_tan: rhsusf_hgu56p
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor_tan: rhsusf_hgu56p_tan
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class rhsusf_hgu56p_mask_tan: rhsusf_hgu56p_tan
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_tan: rhsusf_hgu56p_tan
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_pink: rhsusf_hgu56p
	{
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_glossy.rvmat"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_pink_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor_pink: rhsusf_hgu56p_pink
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class rhsusf_hgu56p_mask_pink: rhsusf_hgu56p_pink
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_pink: rhsusf_hgu56p_pink
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_usa: rhsusf_hgu56p
	{
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_glossy.rvmat"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_usa_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor_usa: rhsusf_hgu56p_usa
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class rhsusf_hgu56p_saf: rhsusf_hgu56p
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_saf_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor_saf: rhsusf_hgu56p_saf
	{
		hiddenSelections[]=
		{
			"camo",
			"visor"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_saf_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_visor_yellow_ca.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56.rvmat",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_visor.rvmat"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"visor"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class rhsusf_hgu56p_mask_saf: rhsusf_hgu56p_saf
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_mask";
		};
	};
	class rhsusf_hgu56p_visor_mask_saf: rhsusf_hgu56p_visor_saf
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor_mask";
		};
	};
	class rhsusf_hgu56p_white: rhsusf_hgu56p
	{
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_glossy.rvmat"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_white_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
	};
	class rhsusf_hgu56p_visor_white: rhsusf_hgu56p_white
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56_visor";
		};
	};
	class NVGoggles;
	class rhsusf_ANPVS_14: NVGoggles
	{
		Scope=2;
		ScopeCurator=2;
		descriptionShort="AN/PVS-14 Monocular Night Vision Device";
		class ItemInfo
		{
			type=616;
			hmdType=0;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\nvg\ANPVS_14_DOWN";
			mass=20;
		};
	};
	class rhsusf_ANPVS_15: rhsusf_ANPVS_14
	{
		descriptionShort="AN/PVS-15 Night Vision Binocular";
		class ItemInfo
		{
			type=616;
			hmdType=0;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\nvg\ANPVS_15_DOWN";
			mass=20;
		};
	};
	class rhsusf_Rhino: rhsusf_ANPVS_14
	{
		visionMode[]=
		{
			"Normal",
			"Normal"
		};
		descriptionShort="Empty Rhino NVG Mount";
		class ItemInfo
		{
			type=616;
			hmdType=0;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\nvg\Rhino_DOWN";
			mass=5;
		};
	};
	class rhsusf_anvis_nvg_bc_caps: NVGoggles
	{
		Scope=1;
		ScopeCurator=1;
		nameSound="nvgoggles";
		simulation="NVGoggles";
		showEmpty=0;
		muzzlePos="usti hlavne";
		muzzleEnd="konec hlavne";
		value=5;
		class ItemInfo
		{
			type=616;
			hmdType=0;
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\anvis\ANVIS_bc_caps_down.p3d";
			mass=4;
		};
	};
	class rhsusf_ANVIS: NVGoggles
	{
		Scope=1;
		ScopeCurator=1;
		descriptionShort="ANVIS-9 Night Vision Binocular";
		nameSound="nvgoggles";
		simulation="NVGoggles";
		showEmpty=0;
		muzzlePos="usti hlavne";
		muzzleEnd="konec hlavne";
		value=5;
		class ItemInfo
		{
			type=616;
			hmdType=0;
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\anvis\ANVIS_down";
			mass=20;
		};
	};
	class rhsusf_mich_helmet_marpatwd: H_HelmetB
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"Camo1",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_rhino_wd_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"Camo1",
				"Camo2"
			};
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01";
			allowedSlots[]={801,901,701,605};
			mass=40;
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=6;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsusf_mich_helmet_marpatwd_headset: rhsusf_mich_helmet_marpatwd
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01_bowman";
		};
	};
	class rhsusf_mich_helmet_marpatd: rhsusf_mich_helmet_marpatwd
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_rhino_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_mich_helmet_marpatd_headset: rhsusf_mich_helmet_marpatwd_headset
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_rhino_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_mich_helmet_marpatwd_alt: rhsusf_mich_helmet_marpatwd
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01_alt";
		};
	};
	class rhsusf_mich_helmet_marpatwd_alt_headset: rhsusf_mich_helmet_marpatwd
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01_alt_bowman";
		};
	};
	class rhsusf_mich_helmet_marpatd_alt: rhsusf_mich_helmet_marpatwd_alt
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_rhino_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_mich_helmet_marpatd_alt_headset: rhsusf_mich_helmet_marpatwd_alt_headset
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_rhino_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
	};
	class rhsusf_mich_helmet_marpatwd_norotos: rhsusf_mich_helmet_marpatwd
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_norotos_wd_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01_nor1";
		};
	};
	class rhsusf_mich_helmet_marpatwd_norotos_headset: rhsusf_mich_helmet_marpatwd_norotos
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01_nor1_bowman";
		};
	};
	class rhsusf_mich_helmet_marpatd_norotos: rhsusf_mich_helmet_marpatwd_norotos
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_norotos_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa"
		};
	};
	class rhsusf_mich_helmet_marpatd_norotos_headset: rhsusf_mich_helmet_marpatwd_norotos_headset
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_norotos_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa"
		};
	};
	class rhsusf_mich_helmet_marpatwd_norotos_arc: rhsusf_mich_helmet_marpatwd
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_norotos_arc_wd_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01_nor1_arc";
		};
	};
	class rhsusf_mich_helmet_marpatwd_norotos_arc_headset: rhsusf_mich_helmet_marpatwd_norotos_arc
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_01_nor1_arc_bowman";
		};
	};
	class rhsusf_mich_helmet_marpatd_norotos_arc: rhsusf_mich_helmet_marpatwd_norotos_arc
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_norotos_arc_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa"
		};
	};
	class rhsusf_mich_helmet_marpatd_norotos_arc_headset: rhsusf_mich_helmet_marpatwd_norotos_arc_headset
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_covered_norotos_arc_des_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa"
		};
	};
	class rhsusf_mich_bare: rhsusf_mich_helmet_marpatwd
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_od_rhino_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"Camo",
				"Camo1",
				"Camo2"
			};
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01";
		};
	};
	class rhsusf_mich_bare_headset: rhsusf_mich_bare
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1",
			"Camo2",
			"bowman"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_od_rhino_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"Camo",
				"Camo1",
				"Camo2",
				"bowman"
			};
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_bowman";
		};
	};
	class rhsusf_mich_bare_alt: rhsusf_mich_bare
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_od_rhino_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_alt";
		};
	};
	class rhsusf_mich_bare_norotos: rhsusf_mich_bare
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_norotos_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1";
		};
	};
	class rhsusf_mich_bare_norotos_headset: rhsusf_mich_bare_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_norotos_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_alt: rhsusf_mich_bare_norotos
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_alt";
		};
	};
	class rhsusf_mich_bare_norotos_alt_headset: rhsusf_mich_bare_norotos_headset
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_alt_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_arc: rhsusf_mich_bare
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_od_norotos_arc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc";
		};
	};
	class rhsusf_mich_bare_norotos_arc_headset: rhsusf_mich_bare_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_od_norotos_arc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_od_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_helmet_ach_acc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_arc_alt: rhsusf_mich_bare_norotos_arc
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_alt";
		};
	};
	class rhsusf_mich_bare_norotos_arc_alt_headset: rhsusf_mich_bare_norotos_arc_headset
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_alt_bowman";
		};
	};
	class rhsusf_mich_bare_tan: rhsusf_mich_bare
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_tan_rhino_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01";
		};
	};
	class rhsusf_mich_bare_tan_headset: rhsusf_mich_bare_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_tan_rhino_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_bowman";
		};
	};
	class rhsusf_mich_bare_alt_tan: rhsusf_mich_bare_tan
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_alt";
		};
	};
	class rhsusf_mich_bare_norotos_tan: rhsusf_mich_bare_tan
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_norotos_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1";
		};
	};
	class rhsusf_mich_bare_norotos_tan_headset: rhsusf_mich_bare_tan_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_norotos_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_alt_tan: rhsusf_mich_bare_norotos_tan
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_alt";
		};
	};
	class rhsusf_mich_bare_norotos_alt_tan_headset: rhsusf_mich_bare_norotos_tan_headset
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_alt_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_arc_tan: rhsusf_mich_bare_norotos_tan
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_tan_norotos_arc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc";
		};
	};
	class rhsusf_mich_bare_norotos_arc_tan_headset: rhsusf_mich_bare_norotos_tan_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_tan_norotos_arc_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_arc_alt_tan: rhsusf_mich_bare_norotos_arc_tan
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_alt";
		};
	};
	class rhsusf_mich_bare_norotos_arc_alt_tan_headset: rhsusf_mich_bare_norotos_arc_tan_headset
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_alt_bowman";
		};
	};
	class rhsusf_mich_bare_semi: rhsusf_mich_bare
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_rhino_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01";
		};
	};
	class rhsusf_mich_bare_semi_headset: rhsusf_mich_bare_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_rhino_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_bowman";
		};
	};
	class rhsusf_mich_bare_alt_semi: rhsusf_mich_bare_semi
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_alt";
		};
	};
	class rhsusf_mich_bare_norotos_semi: rhsusf_mich_bare_semi
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_rhino_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1";
		};
	};
	class rhsusf_mich_bare_norotos_semi_headset: rhsusf_mich_bare_semi_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_rhino_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_alt_semi: rhsusf_mich_bare_norotos_semi
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_alt";
		};
	};
	class rhsusf_mich_bare_norotos_alt_semi_headset: rhsusf_mich_bare_norotos_semi_headset
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_alt_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_arc_semi: rhsusf_mich_bare_norotos_alt_semi
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_norotos_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc";
		};
	};
	class rhsusf_mich_bare_norotos_arc_semi_headset: rhsusf_mich_bare_norotos_semi_headset
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bare_mich_norotos_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\mich_acc_painted_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\ach_acc_tan_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\bowman_base_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_bowman";
		};
	};
	class rhsusf_mich_bare_norotos_arc_alt_semi: rhsusf_mich_bare_norotos_arc_semi
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_alt";
		};
	};
	class rhsusf_mich_bare_norotos_arc_alt_semi_headset: rhsusf_mich_bare_norotos_arc_semi_headset
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\mich_bare_01_nor1_arc_alt_bowman";
		};
	};
	class rhsusf_lwh_helmet_marpatd: H_HelmetB
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_d_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\lwh_wd";
			allowedSlots[]={801,901,701,605};
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=6;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsusf_lwh_helmet_marpatd_ess: rhsusf_lwh_helmet_marpatd
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_d_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\lwh_wd_ess01";
			hiddenSelections[]=
			{
				"camo",
				"Camo2"
			};
			hiddenSelectionsTextures[]=
			{
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_d_co.paa",
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
			};
		};
	};
	class rhsusf_lwh_helmet_marpatd_headset: rhsusf_lwh_helmet_marpatd
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_d_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\lwh_wd_headset";
			hiddenSelections[]=
			{
				"camo",
				"Camo2"
			};
			hiddenSelectionsTextures[]=
			{
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_d_co.paa",
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
			};
		};
	};
	class rhsusf_lwh_helmet_marpatwd: rhsusf_lwh_helmet_marpatd
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa"
		};
	};
	class rhsusf_lwh_helmet_marpatwd_ess: rhsusf_lwh_helmet_marpatd_ess
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"Camo2"
			};
			hiddenSelectionsTextures[]=
			{
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
			};
		};
	};
	class rhsusf_lwh_helmet_marpatwd_blk_ess: rhsusf_lwh_helmet_marpatd_ess
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_wd_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"Camo2"
			};
			hiddenSelectionsTextures[]=
			{
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_wd_co.paa"
			};
		};
	};
	class rhsusf_lwh_helmet_marpatwd_headset: rhsusf_lwh_helmet_marpatd_headset
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"Camo2"
			};
			hiddenSelectionsTextures[]=
			{
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_d_co.paa"
			};
		};
	};
	class rhsusf_lwh_helmet_marpatwd_headset_blk: rhsusf_lwh_helmet_marpatd_headset
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_wd_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"Camo2"
			};
			hiddenSelectionsTextures[]=
			{
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa",
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_acc_wd_co.paa"
			};
		};
	};
	class rhsusf_lwh_helmet_marpatwd_headset_blk2: rhsusf_lwh_helmet_marpatd_headset
	{
		hiddenSelections[]=
		{
			"camo"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo"
			};
			hiddenSelectionsTextures[]=
			{
				"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_wd_co.paa"
			};
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\lwh_wd_headset_2";
		};
	};
	class rhsusf_lwh_helmet_M1942: rhsusf_lwh_helmet_marpatd
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\lwh_m1942_co.paa"
		};
	};
	class rhsusf_Bowman: ItemCore
	{
		hiddenSelections[]=
		{
			"camo"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\Bowman_Base_co.paa"
		};
		class ItemInfo: HeadGearItem
		{
			mass=3;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\rhsusf_bowman";
			hiddenSelections[]=
			{
				"camo"
			};
		};
	};
	class rhsusf_bowman_cap: rhsusf_Bowman
	{
		hiddenSelections[]=
		{
			"camo",
			"Camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\Bowman_Base_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\head\data\Baseball_Cap_Co.paa"
		};
		class ItemInfo: ItemInfo
		{
			mass=3;
			uniformModel="\rhsusf\addons\rhsusf_infantry\gear\head\rhsusf_bowman_cap";
			hiddenSelections[]=
			{
				"camo",
				"Camo2"
			};
		};
	};
	class RHS_jetpilot_usaf: H_HelmetB
	{
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_jetpilot.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry\gear\head\data\rhs_jetpilot_helmet.rvmat"
		};
		class ItemInfo: ItemInfo
		{
			mass=50;
			uniformModel="A3\Characters_F\Common\headgear_helmet_pilot";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=1;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsusf_ihadss: rhsusf_opscore_01
	{
		allowedFacewear[]={};
		hiddenSelections[]=
		{
			"camo",
			"patches"
		};
		hiddenSelectionsTextures[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_olv_co.paa",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches_co.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56.rvmat",
			"\rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\Data\rhs_hgu56_patches.rvmat"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"patches"
			};
			mass=40;
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\head\hgu56\rhs_hgu56";
		};
		subItems[]=
		{
			"rhsusf_ihadss_scope"
		};
	};
	class rhsusf_ihadss_scope: NVGoggles
	{
		visionMode[]=
		{
			"Normal"
		};
	};
	class rhsusf_protech_helmet: H_HelmetB
	{
		allowedFacewear[]=
		{
			"",
			6,
			"rhs_googles_black",
			3,
			"rhs_googles_clear",
			1
		};
		class ItemInfo: ItemInfo
		{
			mass=15;
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\protech\pt_helmet";
			allowedSlots[]={801,901,701,605};
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.80000001;
				};
			};
		};
	};
	class rhsusf_protech_helmet_ess: rhsusf_protech_helmet
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\protech\pt_helmet_ESS";
		};
	};
	class rhsusf_protech_helmet_rhino: rhsusf_protech_helmet
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\protech\pt_helmet_rhino";
		};
	};
	class rhsusf_protech_helmet_rhino_ess: rhsusf_protech_helmet
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="\rhsusf\addons\rhsusf_infantry2\gear\head\protech\pt_helmet_rhino_ESS";
		};
	};
	class Vest_Camo_Base: ItemCore
	{
		class ItemInfo;
	};
	class rhsusf_iotv_ocp_base: Vest_Camo_Base
	{
		hiddenSelections[]=
		{
			"camo",
			"camob",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_iotv_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv1_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa"
		};
		descriptionShort="Armor Level IV";
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camob",
				"camo1",
				"camo2"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv";
			containerClass="Supply20";
			mass=100;
			armor=20;
			descriptionShort="Armor Level IV";
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=28;
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=28;
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ocp: rhsusf_iotv_ocp_base
	{
		hiddenSelections[]=
		{
			"camo",
			"camob",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_iotv_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv1_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa"
		};
	};
	class rhsusf_iotv_ocp_Grenadier: rhsusf_iotv_ocp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Grenadier";
			containerClass="Supply140";
			mass=120;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor="28 + 		3";
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ocp_Medic: rhsusf_iotv_ocp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Medic";
			containerClass="Supply120";
			mass=120;
		};
	};
	class rhsusf_iotv_ocp_Repair: rhsusf_iotv_ocp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Repair";
			containerClass="Supply120";
			mass=120;
		};
	};
	class rhsusf_iotv_ocp_Rifleman: rhsusf_iotv_ocp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_rifleman";
			containerClass="Supply140";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor="28 + 		3";
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ocp_SAW: rhsusf_iotv_ocp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_SAW";
			containerClass="Supply150";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=28;
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ocp_Squadleader: rhsusf_iotv_ocp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Squadleader";
			containerClass="Supply150";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=28;
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ocp_Teamleader: rhsusf_iotv_ocp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Teamleader";
			containerClass="Supply150";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=28;
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ucp_base: rhsusf_iotv_ocp_base
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_iotv_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv1_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa"
		};
	};
	class rhsusf_iotv_ucp: rhsusf_iotv_ocp
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_iotv_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv1_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa"
		};
	};
	class rhsusf_iotv_ucp_Grenadier: rhsusf_iotv_ucp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Grenadier";
			containerClass="Supply140";
			mass=120;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor="28 + 		3";
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ucp_Medic: rhsusf_iotv_ucp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Medic";
			containerClass="Supply120";
			mass=120;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor="28 + 		3";
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ucp_Repair: rhsusf_iotv_ucp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Repair";
			containerClass="Supply120";
			mass=120;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor="28 + 		3";
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ucp_Rifleman: rhsusf_iotv_ucp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_rifleman";
			containerClass="Supply140";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor="28 + 		3";
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ucp_SAW: rhsusf_iotv_ucp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_SAW";
			containerClass="Supply150";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=28;
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ucp_Squadleader: rhsusf_iotv_ucp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Squadleader";
			containerClass="Supply150";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=28;
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_iotv_ucp_Teamleader: rhsusf_iotv_ucp_base
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_iotv_Teamleader";
			containerClass="Supply150";
			mass=140;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=12;
					PassThrough=0.40000001;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=28;
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.1;
				};
			};
		};
	};
	class rhsusf_spc: rhsusf_iotv_ocp_base
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spc01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear1_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_usmc_camelbak_co.paa"
		};
		descriptionShort="Armor Level IV";
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc";
			containerClass="Supply20";
			mass=80;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=28;
					PassThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_rifleman: rhsusf_spc
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_rifleman";
			containerClass="Supply140";
			mass=100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_iar: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_iar";
			containerClass="Supply160";
			mass=120;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_corpsman: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_corpsman";
			containerClass="Supply120";
			mass=100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_crewman: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_crewman";
			containerClass="Supply80";
			mass=80;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					passThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					passThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_light: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_light";
			containerClass="Supply100";
			mass=100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_marksman: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_marksman";
			containerClass="Supply80";
			mass=80;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_mg: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_mg";
			containerClass="Supply120";
			mass=100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_sniper: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_scoutsniper";
			containerClass="Supply80";
			mass=80;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=28;
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_squadleader: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_squadleader";
			containerClass="Supply140";
			mass=100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spc_teamleader: rhsusf_spc_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spc_teamleader";
			containerClass="Supply160";
			mass=120;
		};
	};
	class rhsusf_spc_patchless: rhsusf_spc_light
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spc02_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear1_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_usmc_camelbak_co.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spc_02.rvmat",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_gear1.rvmat",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_gear2.rvmat",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_usmc_camelbak.rvmat"
		};
	};
	class rhsusf_spc_patchless_radio: rhsusf_spc_squadleader
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spc02_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear1_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_usmc_camelbak_co.paa"
		};
		hiddenSelectionsMaterials[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spc_02.rvmat",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_gear1.rvmat",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_gear2.rvmat",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_usmc_camelbak.rvmat"
		};
	};
	class rhsusf_spcs_ocp: rhsusf_iotv_ocp_base
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_camelback_ocp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_gear1_iotv_oCP_co.paa"
		};
		descriptionShort="Armor Level IV";
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_camelback_ocp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_gear1_iotv_oCP_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs";
			containerClass="Supply140";
			mass=100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					passThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=28;
					PassThrough=0.1;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=12;
					passThrough=0.40000001;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_spcs_ocp_squadleader: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_squadleader";
		};
	};
	class rhsusf_spcs_ocp_teamleader: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4",
			"camo5"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4",
				"camo5"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
				"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_teamleader";
		};
	};
	class rhsusf_spcs_ocp_teamleader_alt: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_teamleader_alt";
		};
	};
	class rhsusf_spcs_ocp_saw: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_saw";
		};
	};
	class rhsusf_spcs_ocp_grenadier: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_grenadier";
		};
	};
	class rhsusf_spcs_ocp_rifleman: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_rifleman";
		};
	};
	class rhsusf_spcs_ocp_rifleman_alt: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_rifleman_alt";
		};
	};
	class rhsusf_spcs_ocp_medic: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_medic";
		};
	};
	class rhsusf_spcs_ocp_crewman: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_crewman";
		};
	};
	class rhsusf_spcs_ocp_machinegunner: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_machinegunner";
		};
	};
	class rhsusf_spcs_ocp_sniper: rhsusf_spcs_ocp
	{
		hiddenSelections[]=
		{
			"camo",
			"camo1",
			"camo2",
			"camo3",
			"camo4",
			"camo5"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
			"rhsusf\addons\rhsusf_weapons2\mk14\data\mk14_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo1",
				"camo2",
				"camo3",
				"camo4",
				"camo5"
			};
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ocp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
				"rhsusf\addons\rhsusf_weapons2\mk14\data\mk14_co.paa"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry\gear\vests\rhs_spcs_sniper";
		};
	};
	class rhsusf_spcs_ucp: rhsusf_spcs_ocp
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_camelback_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_gear1_iotv_UCP_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_camelback_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_gear1_iotv_UCP_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_squadleader: rhsusf_spcs_ocp_squadleader
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_teamleader: rhsusf_spcs_ocp_teamleader
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
				"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_teamleader_alt: rhsusf_spcs_ocp_teamleader_alt
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_saw: rhsusf_spcs_ocp_saw
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_grenadier: rhsusf_spcs_ocp_grenadier
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_rifleman: rhsusf_spcs_ocp_rifleman
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_rifleman_alt: rhsusf_spcs_ocp_rifleman_alt
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_medic: rhsusf_spcs_ocp_medic
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\m4\data\magazine_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_crewman: rhsusf_spcs_ocp_crewman
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\mag_proxies\data\pmag_black_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_machinegunner: rhsusf_spcs_ocp_machinegunner
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
			};
		};
	};
	class rhsusf_spcs_ucp_sniper: rhsusf_spcs_ocp_sniper
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
			"rhsusf\addons\rhsusf_weapons2\mk14\data\mk14_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelectionsTextures[]=
			{
				"rhsusf\addons\rhsusf_infantry\data\rhs_spcs_ucp_01_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv3_ucp_co.paa",
				"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
				"rhsusf\addons\rhsusf_weapons\grenades\m18\data\m18_green_ca.paa",
				"rhsusf\addons\rhsusf_weapons2\mk14\data\mk14_co.paa"
			};
		};
	};
	class rhsusf_mbav: rhsusf_iotv_ocp_base
	{
		hiddenSelections[]=
		{
			"camo"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_tan_co.paa"
		};
		descriptionShort="Armor Level IV";
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\mbav_clean";
			containerClass="Supply20";
			mass=60;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=28;
					PassThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_mbav_light: rhsusf_mbav
	{
		hiddenSelections[]=
		{
			"camo",
			"camo2"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_tan_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"camo",
				"camo2"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\mbav_light";
			containerClass="Supply100";
			mass=80;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=28;
					PassThrough=0.1;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.40000001;
				};
			};
		};
	};
	class rhsusf_mbav_rifleman: rhsusf_mbav_light
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\mbav_rifleman";
			containerClass="Supply140";
		};
	};
	class rhsusf_mbav_mg: rhsusf_mbav_light
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\mbav_mgunner";
			containerClass="Supply160";
		};
	};
	class rhsusf_mbav_grenadier: rhsusf_mbav_light
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\mbav_grenadier";
			containerClass="Supply150";
			class HitpointsProtectionInfo: HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor="28 + 		3";
					PassThrough=0.1;
				};
				class Diaphragm: Diaphragm
				{
				};
				class Body: Body
				{
				};
			};
		};
	};
	class rhsusf_mbav_medic: rhsusf_mbav_light
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\mbav_medic";
			containerClass="Supply160";
		};
	};
	class rhsusf_plateframe_sapi: rhsusf_iotv_ocp_base
	{
		hiddenSelections[]=
		{
			"Camo",
			"Camo1",
			"Camo2",
			"Camo3",
			"Camo4",
			"Camo5",
			"Camo6",
			"Camo7"
		};
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\data\rhs_plateframe_sapi_tan_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear1_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\rhs_30rn_usgi_alum_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\rhs_30rn_magpul_black_co.paa",
			"rhsusf\addons\rhsusf_weapons3\mk17\data\mk17_co.paa"
		};
		descriptionShort="Armor Level III";
		class ItemInfo: ItemInfo
		{
			hiddenSelections[]=
			{
				"Camo",
				"Camo1",
				"Camo2",
				"Camo3",
				"Camo4",
				"Camo5",
				"Camo6",
				"Camo7"
			};
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi";
			containerClass="Supply20";
			mass=60;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=22;
					PassThrough=0.2;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=22;
					PassThrough=0.2;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.2;
				};
			};
		};
	};
	class rhsusf_plateframe_light: rhsusf_plateframe_sapi
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi_light";
			containerClass="Supply100";
			mass=80;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=22;
					PassThrough=0.2;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="22 + 		3";
					PassThrough=0.2;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.2;
				};
			};
		};
	};
	class rhsusf_plateframe_rifleman: rhsusf_plateframe_sapi
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi_rifleman";
			containerClass="Supply160";
			mass=100;
			class HitpointsProtectionInfo
			{
				class Chest
				{
					HitpointName="HitChest";
					armor=22;
					PassThrough=0.2;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor="22 + 		3";
					PassThrough=0.2;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.2;
				};
			};
		};
	};
	class rhsusf_plateframe_teamleader: rhsusf_plateframe_rifleman
	{
		hiddenSelectionsTextures[]=
		{
			"rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\data\rhs_plateframe_sapi_tan_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear1_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_spc_gear2_co.paa",
			"rhsusf\addons\rhsusf_infantry2\gear\vests\mbav\data\mbav_gear_co.paa",
			"rhsusf\addons\rhsusf_infantry\gear\vests\data\rhs_vest_iotv2_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\rhs_30rn_usgi_alum_co.paa",
			"rhsusf\addons\rhsusf_weapons\mag_proxies\data\rhs_30rn_magpul_drab_co.paa",
			"rhsusf\addons\rhsusf_weapons3\mk17\data\mk17_co.paa"
		};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi_teamleader";
		};
	};
	class rhsusf_plateframe_grenadier: rhsusf_plateframe_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi_grenadier";
		};
	};
	class rhsusf_plateframe_machinegunner: rhsusf_plateframe_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi_machinegunner";
		};
	};
	class rhsusf_plateframe_marksman: rhsusf_plateframe_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi_marksmen";
		};
	};
	class rhsusf_plateframe_medic: rhsusf_plateframe_rifleman
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsusf\addons\rhsusf_infantry2\gear\vests\plateframe\rhs_plateframe_sapi_medic";
		};
	};
};
