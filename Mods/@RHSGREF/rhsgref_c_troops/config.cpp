class CfgPatches
{
	class rhsgref_c_troops
	{
		units[]=
		{
			"rhsgref_cdf_reg_rifleman",
			"rhsgref_cdf_reg_rifleman_rpg75",
			"rhsgref_cdf_reg_rifleman_aks74",
			"rhsgref_cdf_reg_rifleman_lite",
			"rhsgref_cdf_reg_rifleman_m70",
			"rhsgref_cdf_reg_grenadier",
			"rhsgref_cdf_reg_machinegunner",
			"rhsgref_cdf_reg_marksman",
			"rhsgref_cdf_reg_general",
			"rhsgref_cdf_reg_officer",
			"rhsgref_cdf_reg_squadleader",
			"rhsgref_cdf_reg_grenadier_rpg",
			"rhsgref_cdf_reg_specialist_aa",
			"rhsgref_cdf_reg_medic",
			"rhsgref_cdf_reg_engineer",
			"rhsgref_cdf_reg_crew",
			"rhsgref_cdf_reg_crew_commander",
			"rhsgref_cdf_air_pilot",
			"rhsgref_cdf_para_rifleman",
			"rhsgref_cdf_para_rifleman_lite",
			"rhsgref_cdf_para_grenadier",
			"rhsgref_cdf_para_autorifleman",
			"rhsgref_cdf_para_machinegunner",
			"rhsgref_cdf_para_marksman",
			"rhsgref_cdf_para_officer",
			"rhsgref_cdf_para_squadleader",
			"rhsgref_cdf_para_grenadier_rpg",
			"rhsgref_cdf_para_specialist_aa",
			"rhsgref_cdf_para_medic",
			"rhsgref_cdf_para_engineer",
			"rhsgref_cdf_para_crew",
			"rhsgref_cdf_ngd_rifleman",
			"rhsgref_cdf_ngd_rifleman_rpg75",
			"rhsgref_cdf_ngd_rifleman_ak74",
			"rhsgref_cdf_ngd_rifleman_lite",
			"rhsgref_cdf_ngd_grenadier",
			"rhsgref_cdf_ngd_machinegunner",
			"rhsgref_cdf_ngd_officer",
			"rhsgref_cdf_ngd_squadleader",
			"rhsgref_cdf_ngd_grenadier_rpg",
			"rhsgref_cdf_ngd_medic",
			"rhsgref_cdf_ngd_engineer",
			"rhsgref_cdf_ngd_crew",
			"rhsgref_cdf_b_reg_rifleman",
			"rhsgref_cdf_b_reg_rifleman_rpg75",
			"rhsgref_cdf_b_reg_rifleman_aks74",
			"rhsgref_cdf_b_reg_rifleman_lite",
			"rhsgref_cdf_b_reg_rifleman_m70",
			"rhsgref_cdf_b_reg_grenadier",
			"rhsgref_cdf_b_reg_machinegunner",
			"rhsgref_cdf_b_reg_marksman",
			"rhsgref_cdf_b_reg_general",
			"rhsgref_cdf_b_reg_officer",
			"rhsgref_cdf_b_reg_squadleader",
			"rhsgref_cdf_b_reg_grenadier_rpg",
			"rhsgref_cdf_b_reg_specialist_aa",
			"rhsgref_cdf_b_reg_medic",
			"rhsgref_cdf_b_reg_engineer",
			"rhsgref_cdf_b_reg_crew",
			"rhsgref_cdf_b_reg_crew_commander",
			"rhsgref_cdf_b_air_pilot",
			"rhsgref_cdf_b_para_rifleman",
			"rhsgref_cdf_b_para_rifleman_lite",
			"rhsgref_cdf_b_para_grenadier",
			"rhsgref_cdf_b_para_autorifleman",
			"rhsgref_cdf_b_para_machinegunner",
			"rhsgref_cdf_b_para_marksman",
			"rhsgref_cdf_b_para_officer",
			"rhsgref_cdf_b_para_squadleader",
			"rhsgref_cdf_b_para_grenadier_rpg",
			"rhsgref_cdf_b_para_specialist_aa",
			"rhsgref_cdf_b_para_medic",
			"rhsgref_cdf_b_para_engineer",
			"rhsgref_cdf_b_para_crew",
			"rhsgref_cdf_b_ngd_rifleman",
			"rhsgref_cdf_b_ngd_rifleman_rpg75",
			"rhsgref_cdf_b_ngd_rifleman_ak74",
			"rhsgref_cdf_b_ngd_rifleman_lite",
			"rhsgref_cdf_b_ngd_grenadier",
			"rhsgref_cdf_b_ngd_machinegunner",
			"rhsgref_cdf_b_ngd_officer",
			"rhsgref_cdf_b_ngd_squadleader",
			"rhsgref_cdf_b_ngd_grenadier_rpg",
			"rhsgref_cdf_b_ngd_medic",
			"rhsgref_cdf_b_ngd_engineer",
			"rhsgref_cdf_b_ngd_crew",
			"rhsgref_cdf_un_rifleman",
			"rhsgref_cdf_un_rifleman_lite",
			"rhsgref_cdf_un_grenadier",
			"rhsgref_cdf_un_machinegunner",
			"rhsgref_cdf_un_general",
			"rhsgref_cdf_un_officer",
			"rhsgref_cdf_un_squadleader",
			"rhsgref_cdf_un_grenadier_rpg",
			"rhsgref_cdf_un_medic",
			"rhsgref_cdf_un_engineer",
			"rhsgref_cdf_un_crew",
			"rhsgref_cdf_un_pilot",
			"rhsgref_nat_rifleman",
			"rhsgref_nat_rifleman_akms",
			"rhsgref_nat_rifleman_aks74",
			"rhsgref_nat_militiaman_kar98k",
			"rhsgref_nat_rifleman_m92",
			"rhsgref_nat_rifleman_mp44",
			"rhsgref_nat_rifleman_vz58",
			"rhsgref_nat_grenadier",
			"rhsgref_nat_autorifleman",
			"rhsgref_nat_warlord",
			"rhsgref_nat_commander",
			"rhsgref_nat_machinegunner",
			"rhsgref_nat_machinegunner_mg42",
			"rhsgref_nat_grenadier_rpg",
			"rhsgref_nat_specialist_aa",
			"rhsgref_nat_hunter",
			"rhsgref_nat_scout",
			"rhsgref_nat_saboteur",
			"rhsgref_nat_medic",
			"rhsgref_nat_crew",
			"rhsgref_nat_pmil_rifleman",
			"rhsgref_nat_pmil_rifleman_m92",
			"rhsgref_nat_pmil_rifleman_akm",
			"rhsgref_nat_pmil_rifleman_aksu",
			"rhsgref_nat_pmil_grenadier",
			"rhsgref_nat_pmil_autorifleman",
			"rhsgref_nat_pmil_commander",
			"rhsgref_nat_pmil_machinegunner",
			"rhsgref_nat_pmil_grenadier_rpg",
			"rhsgref_nat_pmil_specialist_aa",
			"rhsgref_nat_pmil_hunter",
			"rhsgref_nat_pmil_scout",
			"rhsgref_nat_pmil_saboteur",
			"rhsgref_nat_pmil_medic",
			"rhsgref_nat_pmil_crew",
			"rhsgref_ins_rifleman",
			"rhsgref_ins_rifleman_akm",
			"rhsgref_ins_rifleman_aksu",
			"rhsgref_ins_rifleman_aks74",
			"rhsgref_ins_rifleman_RPG26",
			"rhsgref_ins_militiaman_mosin",
			"rhsgref_ins_grenadier",
			"rhsgref_ins_machinegunner",
			"rhsgref_ins_squadleader",
			"rhsgref_ins_commander",
			"rhsgref_ins_grenadier_rpg",
			"rhsgref_ins_specialist_aa",
			"rhsgref_ins_engineer",
			"rhsgref_ins_saboteur",
			"rhsgref_ins_sniper",
			"rhsgref_ins_spotter",
			"rhsgref_ins_medic",
			"rhsgref_ins_crew",
			"rhsgref_ins_pilot",
			"rhsgref_ins_g_rifleman",
			"rhsgref_ins_g_rifleman_akm",
			"rhsgref_ins_g_rifleman_aksu",
			"rhsgref_ins_g_rifleman_aks74",
			"rhsgref_ins_g_rifleman_RPG26",
			"rhsgref_ins_g_militiaman_mosin",
			"rhsgref_ins_g_grenadier",
			"rhsgref_ins_g_machinegunner",
			"rhsgref_ins_g_squadleader",
			"rhsgref_ins_g_commander",
			"rhsgref_ins_g_grenadier_rpg",
			"rhsgref_ins_g_specialist_aa",
			"rhsgref_ins_g_engineer",
			"rhsgref_ins_g_saboteur",
			"rhsgref_ins_g_sniper",
			"rhsgref_ins_g_spotter",
			"rhsgref_ins_g_medic",
			"rhsgref_ins_g_crew",
			"rhsgref_ins_g_pilot",
			"rhsgref_hidf_rifleman",
			"rhsgref_hidf_grenadier",
			"rhsgref_hidf_grenadier_m79",
			"rhsgref_hidf_rifleman_m72",
			"rhsgref_hidf_autorifleman",
			"rhsgref_hidf_autorifleman_assist",
			"rhsgref_hidf_machinegunner",
			"rhsgref_hidf_machinegunner_assist",
			"rhsgref_hidf_marksman",
			"rhsgref_hidf_squadleader",
			"rhsgref_hidf_teamleader",
			"rhsgref_hidf_medic",
			"rhsgref_hidf_sniper",
			"rhsgref_hidf_crewman",
			"rhsgref_hidf_helipilot",
			"rhsgref_hidf_boat_driver",
			"rhsgref_tla_rifleman",
			"rhsgref_tla_rifleman_akms",
			"rhsgref_tla_rifleman_M16",
			"rhsgref_tla_rifleman_pm63",
			"rhsgref_tla_rifleman_vz58",
			"rhsgref_tla_rifleman_m1",
			"rhsgref_tla_rifleman_l1a1",
			"rhsgref_tla_squadleader",
			"rhsgref_tla_grenadier",
			"rhsgref_tla_grenadier_m79",
			"rhsgref_tla_machinegunner",
			"rhsgref_tla_machinegunner_mg42",
			"rhsgref_tla_rifleman_rpg75",
			"rhsgref_tla_specialist_at",
			"rhsgref_tla_medic",
			"rhsgref_tla_crew"
		};
		weapons[]=
		{
			"rhsgref_uniform_ttsko_forest",
			"rhsgref_uniform_ttsko_mountain",
			"rhsgref_uniform_ttsko_urban",
			"rhsgref_uniform_vsr",
			"rhsgref_uniform_para_ttsko_oxblood",
			"rhsgref_uniform_para_ttsko_mountain",
			"rhsgref_uniform_para_ttsko_urban",
			"rhsgref_uniform_specter",
			"rhsgref_uniform_reed",
			"rhsgref_uniform_alpenflage",
			"rhsgref_uniform_flecktarn",
			"rhsgref_uniform_flecktarn_full",
			"rhsgref_uniform_tigerstripe",
			"rhsgref_uniform_woodland",
			"rhsgref_uniform_woodland_olive",
			"rhsgref_uniform_ERDL",
			"rhsgref_uniform_og107",
			"rhsgref_uniform_og107_erdl",
			"rhsgref_uniform_altis_lizard",
			"rhsgref_uniform_olive",
			"rhsgref_uniform_altis_lizard_olive",
			"rhsgref_uniform_dpm",
			"rhsgref_uniform_dpm_olive",
			"rhsgref_uniform_3color_desert",
			"rhsgref_uniform_TLA_1",
			"rhsgref_uniform_TLA_2",
			"rhsgref_un_beret",
			"rhsgref_Booniehat_alpen",
			"rhsgref_bcap_specter",
			"rhsgref_patrolcap_specter",
			"rhsgref_fieldcap",
			"rhsgref_fieldcap_ttsko_forest",
			"rhsgref_fieldcap_ttsko_mountain",
			"rhsgref_fieldcap_ttsko_urban",
			"rhsgref_fieldcap_ttsko_digi",
			"rhsgref_ssh68",
			"rhsgref_ssh68_ttsko_mountain",
			"rhsgref_ssh68_ttsko_forest",
			"rhsgref_ssh68_ttsko_fall",
			"rhsgref_ssh68_ttsko_summer",
			"rhsgref_ssh68_ttsko_dark",
			"rhsgref_ssh68_ttsko_digi",
			"rhsgref_ssh68_emr",
			"rhsgref_ssh68_un",
			"rhsgref_ssh68_vsr",
			"rhsgref_M56",
			"rhsgref_6b27m",
			"rhsgref_6b27m_ttsko_forest",
			"rhsgref_6b27m_ttsko_mountain",
			"rhsgref_6b27m_ttsko_urban",
			"rhsgref_6b27m_ttsko_digi",
			"rhsgref_helmet_M1_bare",
			"rhsgref_helmet_M1_bare_alt01",
			"rhsgref_helmet_M1_liner",
			"rhsgref_hat_M1951",
			"rhsgref_hat_m1941cap",
			"rhsgref_helmet_M1_painted",
			"rhsgref_helmet_M1_painted_alt01",
			"rhsgref_helmet_M1_mit",
			"rhsgref_helmet_M1_erdl",
			"rhsgref_helmet_M1_des",
			"rhsgref_helmet_pasgt_erdl",
			"rhsgref_helmet_pasgt_erdl_rhino",
			"rhsgref_helmet_pasgt_woodland",
			"rhsgref_helmet_pasgt_woodland_rhino",
			"rhsgref_helmet_pasgt_flecktarn",
			"rhsgref_helmet_pasgt_un",
			"rhsgref_helmet_pasgt_press",
			"rhsgref_helmet_pasgt_3color_desert",
			"rhsgref_helmet_pasgt_3color_desert_rhino",
			"rhsgref_helmet_pasgt_altis_lizard",
			"rhsgref_helmet_pasgt_olive",
			"rhsgref_hat_m43cap_heer",
			"rhsgref_hat_m43cap_heer1_tilted",
			"rhsgref_helmet_m1940",
			"rhsgref_helmet_m1940_alt1",
			"rhsgref_helmet_m1940_h",
			"rhsgref_helmet_m1940_camo01",
			"rhsgref_helmet_m1940_camo01_alt1",
			"rhsgref_helmet_m1940_winter",
			"rhsgref_helmet_m1940_winter_alt1",
			"rhsgref_helmet_m1942",
			"rhsgref_helmet_m1942_alt1",
			"rhsgref_helmet_m1942_h",
			"rhsgref_helmet_m1942_camo01",
			"rhsgref_helmet_m1942_camo01_alt1",
			"rhsgref_helmet_m1942_winter",
			"rhsgref_helmet_m1942_winter_alt1",
			"rhsgref_helmet_m1942_heermarshcover",
			"rhsgref_helmet_m1942_heersplintercover",
			"rhsgref_helmet_m1942_heergreycover",
			"rhsgref_helmet_m1942_heerwintercover",
			"rhsgref_6b23_khaki",
			"rhsgref_6b23_khaki_rifleman",
			"rhsgref_6b23_khaki_sniper",
			"rhsgref_6b23_khaki_officer",
			"rhsgref_6b23_khaki_nco",
			"rhsgref_6b23_khaki_medic",
			"rhsgref_6b23_ttsko_forest",
			"rhsgref_6b23_ttsko_forest_rifleman",
			"rhsgref_6b23_ttsko_forest_sniper",
			"rhsgref_6b23_ttsko_mountain",
			"rhsgref_6b23_ttsko_mountain_rifleman",
			"rhsgref_6b23_ttsko_mountain_sniper",
			"rhsgref_6b23_ttsko_mountain_officer",
			"rhsgref_6b23_ttsko_mountain_nco",
			"rhsgref_6b23_ttsko_mountain_medic",
			"rhsgref_6b23_ttsko_urban",
			"rhsgref_6b23_ttsko_urban_rifleman",
			"rhsgref_6b23_ttsko_urban_sniper",
			"rhsgref_6b23_ttsko_urban_officer",
			"rhsgref_6b23_ttsko_urban_nco",
			"rhsgref_6b23_ttsko_urban_medic",
			"rhsgref_6b23_ttsko_digi",
			"rhsgref_6b23_ttsko_digi_rifleman",
			"rhsgref_6b23_ttsko_digi_sniper",
			"rhsgref_6b23_ttsko_digi_officer",
			"rhsgref_6b23_ttsko_digi_nco",
			"rhsgref_6b23_ttsko_digi_medic",
			"rhsgref_otv_khaki",
			"rhsgref_otv_digi",
			"rhsgref_TacVest_ERDL",
			"rhsgref_alice_webbing",
			"rhsgref_chestrig",
			"rhs_6b5",
			"rhs_6b5_officer",
			"rhs_6b5_medic",
			"rhs_6b5_sniper",
			"rhs_6b5_rifleman",
			"rhs_6b5_khaki",
			"rhs_6b5_officer_khaki",
			"rhs_6b5_medic_khaki",
			"rhs_6b5_sniper_khaki",
			"rhs_6b5_rifleman_khaki",
			"rhs_6b5_vsr",
			"rhs_6b5_officer_vsr",
			"rhs_6b5_medic_vsr",
			"rhs_6b5_sniper_vsr",
			"rhs_6b5_rifleman_vsr",
			"rhs_6b5_ttsko",
			"rhs_6b5_officer_ttsko",
			"rhs_6b5_medic_ttsko",
			"rhs_6b5_sniper_ttsko",
			"rhs_6b5_rifleman_ttsko"
		};
		name="GREF Infantry & Equipment";
	};
};
class CfgGroups
{
	class West
	{
		class rhsgref_faction_cdf_b_ground
		{
			name="Chernarus (Ground Forces)";
			class rhs_group_cdf_b_bm21
			{
				aliveCategory="Artillery";
				name="BM-21 Artillery";
				class RHS_SPGPlatoon_ins_g_bm21
				{
					name="BM-21 Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="SERGEANT";
						position[]={60,-60,3};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="CORPORAL";
						position[]={80,-60,3};
					};
				};
				class RHS_SPGSection_ins_g_bm21
				{
					name="BM-21 Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
			};
			class rhsgref_group_cdf_b_reg_infantry
			{
				name="Regular Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_cdf_b_reg_infantry_squad
				{
					name="Squad";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={18,0,0};
					};
				};
				class rhsgref_group_cdf_b_reg_infantry_squad_weap
				{
					name="Squad (Weapons)";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
			};
			class rhs_group_cdf_b_bmp1
			{
				name="Mechanized (BMP-1)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_bmp1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_cdf_b_bmp1k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_cdf_b_bmp1_squad: rhs_group_cdf_b_bmp1_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_cdf_b_bmp1_squad_2mg: rhs_group_cdf_b_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_b_bmp1_squad_sniper: rhs_group_cdf_b_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_marksman";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_b_bmp1_squad_mg_sniper: rhs_group_cdf_b_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_marksman";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
						rank="PRIVATE";
					};
				};
				class rhs_group_cdf_b_bmp1_squad_aa: rhs_group_cdf_b_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_cdf_b_bmp2
			{
				name="Mechanized (BMP-2)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_bmp2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_cdf_b_bmp2k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_cdf_b_bmp2_squad: rhs_group_cdf_b_bmp2_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_cdf_b_bmp2_squad_2mg: rhs_group_cdf_b_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp2d";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_b_bmp2_squad_sniper: rhs_group_cdf_b_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_marksman";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_b_bmp2_squad_mg_sniper: rhs_group_cdf_b_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_marksman";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_m70";
						rank="PRIVATE";
					};
				};
				class rhs_group_cdf_b_bmp2_squad_aa: rhs_group_cdf_b_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_b_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_cdf_b_btr70
			{
				name="Motorized (BTR-70)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_btr70_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr70";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_b_btr70_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_btr70_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_btr70_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_btr70_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_btr70_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_cdf_b_btr60
			{
				name="Motorized (BTR-60)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_btr60_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr60";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_b_btr60_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_btr60_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_btr60_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_btr60_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_btr60_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_cdf_b_ural
			{
				name="Motorized (Ural-4320)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_ural_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_b_ural_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_ural_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_ural_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_ural_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_ural_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_ural_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_cdf_b_gaz66
			{
				name="Motorized (GAZ-66)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_gaz66_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_cdf_b_uaz
			{
				name="Motorized (UAZ)";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (AGS-30)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_ags";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_ags";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_ags";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (DShkM)";
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_dshkm";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_dshkm";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_dshkm";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (SPG-9)";
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_spg9";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_spg9";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz_spg9";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_grenadier";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_specialist_aa";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhsgref_group_cdf_b_para_infantry
			{
				name="Airborne Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_cdf_b_para_infantry_squad
				{
					name="Squad";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_medic";
						rank="PRIVATE";
						position[]={18,0,0};
					};
				};
				class rhsgref_group_cdf_b_para_infantry_squad_weap
				{
					name="Squad (Weapons)";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_grenadier_rpg";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
			};
			class rhs_group_cdf_b_bmd1
			{
				name="Mechanized (BMD-1)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_bmd1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_officer";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd1k";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_b_bmd1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd1_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd1_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd1_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd1_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd1";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_cdf_b_bmd2
			{
				name="Mechanized (BMD-2)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_bmd2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_officer";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd2k";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_b_bmd2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd2_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd2_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd2_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_bmd2_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_bmd2";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_cdf_b_gaz66_para
			{
				name="Motorized (GAZ-66/Para)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_b_gaz66_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_reg_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_b_gaz66_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_cdf_b_gaz66_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_cdf_b_para_rifleman_lite";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_cdf_b_72
			{
				name="T-72 Groups";
				aliveCategory="Armored";
				class RHS_T72baPlatoon
				{
					name="T-72 g.1984 Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72ba_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72ba_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72ba_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baPlatoon_AA
				{
					name="T-72 g.1984 Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72ba_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72ba_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baSection
				{
					name="T-72 g.1984 Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72ba_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72ba_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_T72BBPlatoon
				{
					name="T-72 g.1985 Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72bb_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72bb_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72bb_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBPlatoon_AA
				{
					name="T-72 g.1985 Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72bb_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72bb_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBSection
				{
					name="T-72 g.1985 Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72bb_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_t72bb_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
			class rhs_group_cdf_80
			{
				name="T-80 Groups";
				aliveCategory="Armored";
				class RHS_T80bPlatoon
				{
					name="T-80B Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80b_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80b_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80b_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bPlatoon_AA
				{
					name="T-80B Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80b_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80b_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bSection
				{
					name="T-80B Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80b_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80b_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_T80bvPlatoon
				{
					name="T-80BV Platoon";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80bv_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80bv_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80bv_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bvPlatoon_AA
				{
					name="T-80BV Platoon (Combined)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80bv_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80bv_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bvSection
				{
					name="T-80BV Section";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80bv_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_cdf_b_T80bv_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
		};
		class rhsgref_faction_hidf
		{
			name="Horizon Islands Defence Force";
			class rhsgref_group_hidf_infantry
			{
				name="Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_hidf_infantry_squad
				{
					name="$STR_RHSUSF_GROUPS_SQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_hidf_infantry_weaponsquad
				{
					name="$STR_RHSUSF_GROUPS_WEAPONSQUAD";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner_assist";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner_assist";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_hidf_infantry_squad_sniper
				{
					name="$STR_RHSUSF_GROUPS_SQUAD_SNIPER";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhs_group_hidf_infantry_team
				{
					name="$STR_RHSUSF_GROUPS_TEAM";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_hidf_infantry_team_MG
				{
					name="$STR_RHSUSF_GROUPS_TEAM_MG";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner_assist";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_hidf_infantry_team_AT
				{
					name="$STR_RHSUSF_GROUPS_TEAM_AT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhs_group_hidf_infantry_team_support
				{
					name="$STR_RHSUSF_GROUPS_TEAM_SUPPORT";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_medic";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
			};
			class rhsgref_group_hidf_hmmwv
			{
				name="Motorized (M1025)";
				aliveCategory="Motorized";
				class rhsgref_group_hidf_m1025_gmg
				{
					name="Mot. Team (GMG)";
					side=1;
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_m1025_mk19";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
				class rhsgref_group_hidf_m1025_hmg: rhsgref_group_hidf_m1025_gmg
				{
					name="Mot. Team (HMG)";
					icon="\A3\ui_f\data\map\markers\nato\b_motor_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_m1025_m2";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhsgref_group_hidf_m113
			{
				name="Mechanized (M113)";
				aliveCategory="Mechanized";
				class rhsgref_group_hidf_m113_squad
				{
					name="Squad";
					side=1;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_grenadier_m79";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_m113a3_m2";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsgref_hidf_autorifleman_assist";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
				class rhsgref_group_hidf_m113_squad_2mg: rhsgref_group_hidf_m113_squad
				{
					name="Squad (2x GPMG)";
					icon="\A3\ui_f\data\map\markers\nato\b_mech_inf.paa";
					class Unit0
					{
						side=1;
						vehicle="rhsgref_hidf_teamleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=1;
						vehicle="rhsgref_hidf_m113a3_m2";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner_assist";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman_m72";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=1;
						vehicle="rhsgref_hidf_machinegunner_assist";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=1;
						vehicle="rhsgref_hidf_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
		};
	};
	class Indep
	{
		class rhsgref_faction_cdf_ground
		{
			name="Chernarus (Ground Forces)";
			class rhs_group_cdf_bm21
			{
				aliveCategory="Artillery";
				name="BM-21 Artillery";
				class RHS_SPGPlatoon_ins_g_bm21
				{
					name="BM-21 Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="SERGEANT";
						position[]={60,-60,3};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="CORPORAL";
						position[]={80,-60,3};
					};
				};
				class RHS_SPGSection_ins_g_bm21
				{
					name="BM-21 Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
			};
			class rhsgref_group_cdf_reg_infantry
			{
				name="Regular Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_cdf_reg_infantry_squad
				{
					name="Squad";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={18,0,0};
					};
				};
				class rhsgref_group_cdf_reg_infantry_squad_weap
				{
					name="Squad (Weapons)";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
			};
			class rhs_group_cdf_bmp1
			{
				name="Mechanized (BMP-1)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_bmp1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_cdf_bmp1k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_cdf_bmp1_squad: rhs_group_cdf_bmp1_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_marksman";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_cdf_bmp1_squad_2mg: rhs_group_cdf_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_rifleman_lite";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_bmp1_squad_sniper: rhs_group_cdf_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_marksman";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_bmp1_squad_mg_sniper: rhs_group_cdf_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_marksman";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
					};
				};
				class rhs_group_cdf_bmp1_squad_aa: rhs_group_cdf_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_cdf_bmp2
			{
				name="Mechanized (BMP-2)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_bmp2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_cdf_bmp2k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_cdf_bmp2_squad: rhs_group_cdf_bmp2_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_marksman";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_cdf_bmp2_squad_2mg: rhs_group_cdf_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp2d";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_rifleman_lite";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_bmp2_squad_sniper: rhs_group_cdf_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_marksman";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
					};
				};
				class rhs_group_cdf_bmp2_squad_mg_sniper: rhs_group_cdf_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_marksman";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
					};
				};
				class rhs_group_cdf_bmp2_squad_aa: rhs_group_cdf_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_cdf_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_cdf_btr70
			{
				name="Motorized (BTR-70)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_btr70_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr70";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_btr70_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_btr70_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_btr70_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_btr70_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_btr70_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_cdf_btr60
			{
				name="Motorized (BTR-60)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_btr60_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr60";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_btr60_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_m70";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_btr60_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_btr60_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_btr60_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_btr60_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_cdf_ural
			{
				name="Motorized (Ural-4320)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_ural_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_ural_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_ural_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_cdf_gaz66
			{
				name="Motorized (GAZ-66)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_gaz66_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_officer";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_gaz66_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_gaz66_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_gaz66_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_gaz66_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_gaz66_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_rifleman_rpg75";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_cdf_uaz
			{
				name="Motorized (UAZ)";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (AGS-30)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_ags";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_ags";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_ags";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (DShkM)";
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_dshkm";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_dshkm";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_dshkm";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (SPG-9)";
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_spg9";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_spg9";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz_spg9";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_reg_grenadier";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_specialist_aa";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhsgref_group_cdf_para_infantry
			{
				name="Airborne Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_cdf_para_infantry_squad
				{
					name="Squad";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="PRIVATE";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=2;
						vehicle="rhsgref_cdf_para_medic";
						rank="PRIVATE";
						position[]={18,0,0};
					};
				};
				class rhsgref_group_cdf_para_infantry_squad_weap
				{
					name="Squad (Weapons)";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_para_grenadier_rpg";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
			};
			class rhs_group_cdf_bmd1
			{
				name="Mechanized (BMD-1)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_bmd1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_officer";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_bmd1k";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_bmd1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd1_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd1_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd1_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd1_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd1";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_cdf_bmd2
			{
				name="Mechanized (BMD-2)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_bmd2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_officer";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_bmd2k";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_bmd2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd2_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd2_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd2_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_bmd2_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_bmd2";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_cdf_gaz66_para
			{
				name="Motorized (GAZ-66/Para)";
				aliveCategory="Mechanized";
				class rhs_group_cdf_ural_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_reg_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_cdf_ural_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_para_autorifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_para_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_para_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_marksman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_para_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_cdf_ural_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_para_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_para_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_gaz66_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_para_rifleman_lite";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_cdf_72
			{
				name="T-72 Groups";
				aliveCategory="Armored";
				class RHS_T72baPlatoon
				{
					name="T-72 g.1984 Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_t72ba_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_t72ba_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_t72ba_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baPlatoon_AA
				{
					name="T-72 g.1984 Platoon (Combined)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_t72ba_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_t72ba_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baSection
				{
					name="T-72 g.1984 Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_t72ba_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_t72ba_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_T72BBPlatoon
				{
					name="T-72 g.1985 Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_t72bb_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_t72bb_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_t72bb_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBPlatoon_AA
				{
					name="T-72 g.1985 Platoon (Combined)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_t72bb_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_t72bb_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBSection
				{
					name="T-72 g.1985 Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_t72bb_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_t72bb_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
			class rhs_group_cdf_80
			{
				name="T-80 Groups";
				aliveCategory="Armored";
				class RHS_T80bPlatoon
				{
					name="T-80B Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_T80b_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_T80b_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_T80b_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bPlatoon_AA
				{
					name="T-80B Platoon (Combined)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_T80b_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_T80b_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bSection
				{
					name="T-80B Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_T80b_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_T80b_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_T80bvPlatoon
				{
					name="T-80BV Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_T80bv_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_T80bv_tv";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_T80bv_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bvPlatoon_AA
				{
					name="T-80BV Platoon (Combined)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_T80bv_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_T80bv_tv";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T80bvSection
				{
					name="T-80BV Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_T80bv_tv";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_T80bv_tv";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
		};
		class rhsgref_faction_cdf_ng
		{
			name="Chernarus (National Guard)";
			class rhsgref_group_cdf_ngd_infantry
			{
				name="Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_cdf_ngd_infantry_squad
				{
					name="Squad";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_grenadier_rpg";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_grenadier";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_rifleman";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_grenadier";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=2;
						vehicle="rhsgref_cdf_ngd_rifleman_rpg75";
						rank="PRIVATE";
						position[]={15,0,0};
					};
				};
			};
		};
		class rhsgref_faction_nationalist
		{
			name="Nationalist Militia";
			class rhsgref_group_national_infantry
			{
				name="Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_national_infantry_squad
				{
					name="Group (Mixed)";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_militiaman_kar98k";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_hunter";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_saboteur";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhsgref_group_national_infantry_squad_2
				{
					name="Group (AT)";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_hunter";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhsgref_group_national_infantry_patrol
				{
					name="Cell";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_militiaman_kar98k";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhsgref_group_national_infantry_mg
				{
					name="MG pair";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
				class rhsgref_group_national_infantry_at
				{
					name="AT pair";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\n_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
			};
			class rhs_group_indp_nat_ural
			{
				name="Motorized (Ural-4320)";
				aliveCategory="Mechanized";
				class rhs_group_nat_ural_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_nat_ural_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_akms";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nat_ural_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_militiaman_kar98k";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_nat_ural_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_hunter";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_nat_ural_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_hunter";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_saboteur";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_nat_ural_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_ural_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_saboteur";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_nat_uaz
			{
				name="Motorized (UAZ)";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (AGS-30)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_uaz_ags";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_uaz_ags";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_uaz_ags";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (DShkM)";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_uaz_dshkm";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_uaz_dshkm";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_uaz_dshkm";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (SPG-9)";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_uaz_spg9";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_uaz_spg9";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_uaz_spg9";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_uaz";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_specialist_aa";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_specialist_aa";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhs_group_indp_nat_btr70
			{
				name="Motorized (BTR-70)";
				aliveCategory="Mechanized";
				class rhs_group_nat_btr70_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_btr70";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_nat_btr70_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_nat_rifleman";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_nat_btr70_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_akms";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_nat_btr70_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_m92";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_hunter";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_nat_btr70_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_commander";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_hunter";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_akms";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_nat_btr70_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_nat_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_nat_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_nat_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_nat_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_nat_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_nat_rifleman_akms";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
		};
		class rhsgref_faction_chdkz_g
		{
			name="ChDKZ Insurgents";
			class rhsgref_group_chdkz_ins_gurgents_infantry
			{
				name="Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_chdkz_ins_gurgents_squad
				{
					name="Team";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aks74";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_patrol
				{
					name="Patrol";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_mg
				{
					name="MG pair";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_at
				{
					name="AT pair";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_aa
				{
					name="AA pair";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
			};
			class rhs_group_indp_ins_g_bm21
			{
				aliveCategory="Artillery";
				name="BM-21 Artillery";
				class RHS_SPGPlatoon_ins_g_bm21
				{
					name="BM-21 Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="SERGEANT";
						position[]={60,-60,3};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="CORPORAL";
						position[]={80,-60,3};
					};
				};
				class RHS_SPGSection_ins_g_bm21
				{
					name="BM-21 Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
			};
			class rhs_group_indp_ins_g_bmd1
			{
				name="Mechanized (BMD-1)";
				aliveCategory="Mechanized";
				class rhs_group_rus_ins_g_bmd1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd1";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd1_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd1_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd1_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd1_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd1";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_indp_ins_g_bmd2
			{
				name="Mechanized (BMD-2)";
				aliveCategory="Mechanized";
				class rhs_group_rus_ins_g_bmd2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd2";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd2_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd2_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd2_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_g_bmd2_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_bmd2";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_indp_ins_g_bmp1
			{
				name="Mechanized (BMP-1)";
				aliveCategory="Mechanized";
				class rhs_group_indp_ins_g_bmp1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_ins_g_bmp1k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_indp_ins_g_bmp1_squad: rhs_group_indp_ins_g_bmp1_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_sniper";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_indp_ins_g_bmp1_squad_2mg: rhs_group_indp_ins_g_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_rifleman_RPG26";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_g_bmp1_squad_sniper: rhs_group_indp_ins_g_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_sniper";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_g_bmp1_squad_mg_sniper: rhs_group_indp_ins_g_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_sniper";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
					};
				};
				class rhs_group_indp_ins_g_bmp1_squad_aa: rhs_group_indp_ins_g_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_indp_ins_g_bmp2
			{
				name="Mechanized (BMP-2)";
				aliveCategory="Mechanized";
				class rhs_group_indp_ins_g_bmp2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_ins_g_bmp2k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_indp_ins_g_bmp2_squad: rhs_group_indp_ins_g_bmp2_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_sniper";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_indp_ins_g_bmp2_squad_2mg: rhs_group_indp_ins_g_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_rifleman_RPG26";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_g_bmp2_squad_sniper: rhs_group_indp_ins_g_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_sniper";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_g_bmp2_squad_mg_sniper: rhs_group_indp_ins_g_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_sniper";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
					};
				};
				class rhs_group_indp_ins_g_bmp2_squad_aa: rhs_group_indp_ins_g_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_g_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_indp_ins_g_btr70
			{
				name="Motorized (BTR-70)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_btr70_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr70";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_indp_ins_g_btr60
			{
				name="Motorized (BTR-60)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_btr60_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr60";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_indp_ins_g_ural
			{
				name="Motorized (Ural-4320)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_ural_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_ural_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_indp_ins_g_gaz66
			{
				name="Motorized (GAZ-66)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_g_gaz66_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_g_gaz66_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_g_gaz66_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_g_gaz66_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_g_gaz66_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_g_gaz66_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=2;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_gaz66_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_rifleman_RPG26";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_indp_ins_g_uaz
			{
				name="Motorized (UAZ)";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (AGS-30)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_ags";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_ags";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_ags";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (DShkM)";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_dshkm_chdkz";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_dshkm_chdkz";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_dshkm_chdkz";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (SPG-9)";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_spg9";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_spg9";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz_spg9";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_uaz";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_grenadier";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=2;
						vehicle="rhsgref_ins_g_specialist_aa";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhs_group_indp_ins_g_72
			{
				name="T-72 Groups";
				aliveCategory="Armored";
				class RHS_T72baPlatoon
				{
					name="T-72 g.1984 Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_T72ba";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_T72ba";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_T72ba";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baPlatoon_AA
				{
					name="T-72 g.1984 Platoon (Combined)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_T72ba";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_T72ba";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baSection
				{
					name="T-72 g.1984 Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_T72ba";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_T72ba";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_T72BBPlatoon
				{
					name="T-72 g.1985 Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bb";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bb";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bb";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBPlatoon_AA
				{
					name="T-72 g.1985 Platoon (Combined)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bb";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bb";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBSection
				{
					name="T-72 g.1985 Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bb";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bb";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_t72bcPlatoon
				{
					name="T-72 g.1989 Platoon";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bc";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bc";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bc";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_t72bcPlatoon_AA
				{
					name="T-72 g.1989 Platoon (Combined)";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bc";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bc";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_t72bcSection
				{
					name="T-72 g.1989 Section";
					side=2;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bc";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=2;
						vehicle="rhsgref_ins_g_t72bc";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
		};
	};
	class East
	{
		class rhsgref_faction_chdkz
		{
			name="ChDKZ Insurgents";
			class rhsgref_group_chdkz_insurgents_infantry
			{
				name="Infantry";
				aliveCategory="Infantry";
				class rhsgref_group_chdkz_insurgents_squad
				{
					name="Team";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aks74";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_patrol
				{
					name="Patrol";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={5,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_mg
				{
					name="MG pair";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_at
				{
					name="AT pair";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
				class rhsgref_group_chdkz_infantry_aa
				{
					name="AA pair";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="PRIVATE";
						position[]={-5,0,0};
					};
				};
			};
			class rhs_group_indp_ins_bm21
			{
				aliveCategory="Artillery";
				name="BM-21 Artillery";
				class RHS_SPGPlatoon_ins_bm21
				{
					name="BM-21 Platoon";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="CORPORAL";
						position[]={40,-60,3};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="SERGEANT";
						position[]={60,-60,3};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="CORPORAL";
						position[]={80,-60,3};
					};
				};
				class RHS_SPGSection_ins_bm21
				{
					name="BM-21 Section";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\n_art.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_BM21";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
			};
			class rhs_group_indp_ins_bmd1
			{
				name="Mechanized (BMD-1)";
				aliveCategory="Mechanized";
				class rhs_group_rus_ins_bmd1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_bmd1";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_rus_ins_bmd1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd1_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd1_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd1_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd1";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd1_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd1";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_indp_ins_bmd2
			{
				name="Mechanized (BMD-2)";
				aliveCategory="Mechanized";
				class rhs_group_rus_ins_bmd2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_bmd2";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_rus_ins_bmd2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd2_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="SERGEANT";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="CORPORAL";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd2_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd2_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd2";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_rus_ins_bmd2_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="SERGEANT";
						position[]={5,-5,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="CORPORAL";
						position[]={-5,-5,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={10,-10,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={-10,-10,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_bmd2";
						rank="CORPORAL";
						position[]={5,0,0};
					};
				};
			};
			class rhs_group_indp_ins_bmp1
			{
				name="Mechanized (BMP-1)";
				aliveCategory="Mechanized";
				class rhs_group_indp_ins_bmp1_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_ins_bmp1k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_indp_ins_bmp1_squad: rhs_group_indp_ins_bmp1_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_sniper";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_indp_ins_bmp1_squad_2mg: rhs_group_indp_ins_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_rifleman_RPG26";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_bmp1_squad_sniper: rhs_group_indp_ins_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_sniper";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_bmp1_squad_mg_sniper: rhs_group_indp_ins_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp1p";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_sniper";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
					};
				};
				class rhs_group_indp_ins_bmp1_squad_aa: rhs_group_indp_ins_bmp1_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp1p";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_indp_ins_bmp2
			{
				name="Mechanized (BMP-2)";
				aliveCategory="Mechanized";
				class rhs_group_indp_ins_bmp2_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="CAPTAIN";
						position[]={0,0,0};
					};
					class Unit1: Unit0
					{
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={5,-5,0};
					};
					class Unit2: Unit0
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="LIEUTENANT";
						position[]={-5,-5,0};
					};
					class Unit3: Unit0
					{
						vehicle="rhsgref_ins_bmp2k";
						rank="SERGEANT";
						position[]={19,-22,0};
					};
				};
				class rhs_group_indp_ins_bmp2_squad: rhs_group_indp_ins_bmp2_chq
				{
					name="$STR_RHS_GROUPS_SQUAD";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_sniper";
						rank="SERGEANT";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit0
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={-10,-11,0};
					};
					class Unit5: Unit0
					{
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={31,-39,0};
					};
					class Unit6: Unit0
					{
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={-15,-15,0};
					};
					class Unit7: Unit0
					{
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={37,-44,0};
					};
					class Unit8: Unit0
					{
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
						position[]={-20,-20,0};
					};
				};
				class rhs_group_indp_ins_bmp2_squad_2mg: rhs_group_indp_ins_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_rifleman_RPG26";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_bmp2_squad_sniper: rhs_group_indp_ins_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_rifleman";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_sniper";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_medic";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_rifleman_akm";
					};
				};
				class rhs_group_indp_ins_bmp2_squad_mg_sniper: rhs_group_indp_ins_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_squadleader";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_machinegunner";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp2";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_sniper";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier_rpg";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
					};
					class Unit7: Unit7
					{
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
					};
					class Unit8: Unit8
					{
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
					};
				};
				class rhs_group_indp_ins_bmp2_squad_aa: rhs_group_indp_ins_bmp2_squad
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0: Unit0
					{
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
					};
					class Unit1: Unit1
					{
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
					};
					class Unit2: Unit2
					{
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
					};
					class Unit3: Unit3
					{
						vehicle="rhsgref_ins_bmp2";
						rank="CORPORAL";
					};
					class Unit4: Unit4
					{
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
					};
					class Unit5: Unit5
					{
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
					};
					class Unit6: Unit6
					{
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
					};
				};
			};
			class rhs_group_indp_ins_btr70
			{
				name="Motorized (BTR-70)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_btr70_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr70";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr70_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr70";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_indp_ins_btr60
			{
				name="Motorized (BTR-60)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_btr60_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr60";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_btr60_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_mech_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_btr60";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
			};
			class rhs_group_indp_ins_ural
			{
				name="Motorized (Ural-4320)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_ural_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_ural";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_ural_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_ural_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_indp_ins_gaz66
			{
				name="Motorized (GAZ-66)";
				aliveCategory="Mechanized";
				class rhs_group_chdkz_gaz66_chq
				{
					name="$STR_RHS_GROUPS_COMPANY_HQ";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="CAPTAIN";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_rifleman";
						rank="LIEUTENANT";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_uaz";
						rank="SERGEANT";
						position[]={5,0,0};
					};
				};
				class rhs_group_chdkz_gaz66_squad
				{
					name="$STR_RHS_GROUPS_SQUAD";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_akm";
						rank="PRIVATE";
						position[]={13,0,0};
					};
				};
				class rhs_group_chdkz_gaz66_squad_2mg
				{
					name="$STR_RHS_GROUPS_SQUAD_2MG";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_gaz66_squad_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_aksu";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_gaz66_squad_mg_sniper
				{
					name="$STR_RHS_GROUPS_SQUAD_MG_SNIPER";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_gaz66";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_sniper";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_medic";
						rank="PRIVATE";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={11,0,0};
					};
				};
				class rhs_group_chdkz_gaz66_squad_aa
				{
					name="$STR_RHS_GROUPS_SQUAD_AA";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_gaz66_Zu23";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_rifleman_RPG26";
						rank="PRIVATE";
						position[]={7,0,0};
					};
				};
			};
			class rhs_group_indp_ins_uaz
			{
				name="Motorized (UAZ)";
				aliveCategory="Motorized";
				class BUS_MotInf_Team_GMG
				{
					name="Mot. Team (AGS-30)";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_uaz_ags";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_uaz_ags";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_uaz_ags";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_Team_HMG: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (DShkM)";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_uaz_dshkm";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_uaz_dshkm";
						rank="CORPORAL";
						position[]={-12,-12,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_uaz_dshkm";
						rank="PRIVATE";
						position[]={14,-13,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AT: BUS_MotInf_Team_GMG
				{
					name="Mot. Team (SPG-9)";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_uaz_spg9";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_uaz_spg9";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_uaz_spg9";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_machinegunner";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="PRIVATE";
						position[]={19,-17,-2};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_ins_grenadier_rpg";
						rank="PRIVATE";
						position[]={20,-17,-2};
					};
				};
				class BUS_MotInf_AA: BUS_MotInf_Team_GMG
				{
					name="Mot. Air-defense Team";
					icon="\A3\ui_f\data\map\markers\nato\o_motor_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_uaz";
						rank="SERGEANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_grenadier";
						rank="CORPORAL";
						position[]={13,-12,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="CORPORAL";
						position[]={-12,-13,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_ins_specialist_aa";
						rank="PRIVATE";
						position[]={18,-17,-2};
					};
				};
			};
			class rhs_group_indp_ins_72
			{
				name="T-72 Groups";
				aliveCategory="Armored";
				class RHS_T72baPlatoon
				{
					name="T-72 g.1984 Platoon";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_T72ba";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_T72ba";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_T72ba";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baPlatoon_AA
				{
					name="T-72 g.1984 Platoon (Combined)";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_T72ba";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_T72ba";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72baSection
				{
					name="T-72 g.1984 Section";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_T72ba";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_T72ba";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_T72BBPlatoon
				{
					name="T-72 g.1985 Platoon";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_t72bb";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_t72bb";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_t72bb";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBPlatoon_AA
				{
					name="T-72 g.1985 Platoon (Combined)";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_t72bb";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_t72bb";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_T72BBSection
				{
					name="T-72 g.1985 Section";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_t72bb";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_t72bb";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
				class RHS_t72bcPlatoon
				{
					name="T-72 g.1989 Platoon";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_t72bc";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_t72bc";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_t72bc";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
				};
				class RHS_t72bcPlatoon_AA
				{
					name="T-72 g.1989 Platoon (Combined)";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_t72bc";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_zsu234";
						rank="SERGEANT";
						position[]={20,-30,3};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_ins_t72bc";
						rank="SERGEANT";
						position[]={-20,-30,3};
					};
				};
				class RHS_t72bcSection
				{
					name="T-72 g.1989 Section";
					side=0;
					icon="\A3\ui_f\data\map\markers\nato\o_armor.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_ins_t72bc";
						rank="LIEUTENANT";
						position[]={0,0,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_ins_t72bc";
						rank="SERGEANT";
						position[]={-20,-30,2};
					};
				};
			};
		};
		class rhsgref_faction_tla
		{
			name="Tanoan Liberation Army";
			class rhsgref_group_tla_infantry
			{
				name="Revolutionaries";
				aliveCategory="Infantry";
				class rhsgref_group_tla_insurgent_cell
				{
					name="Insurgent Cell";
					side=0;
					rarityGroup=0.75;
					icon="\A3\ui_f\data\map\markers\nato\o_inf.paa";
					class Unit0
					{
						side=0;
						vehicle="rhsgref_tla_squadleader";
						rank="SERGEANT";
						position[]={0,5,0};
					};
					class Unit1
					{
						side=0;
						vehicle="rhsgref_tla_rifleman_pm63";
						rank="CORPORAL";
						position[]={-5,0,0};
					};
					class Unit2
					{
						side=0;
						vehicle="rhsgref_tla_rifleman";
						rank="PRIVATE";
						position[]={3,0,0};
					};
					class Unit3
					{
						side=0;
						vehicle="rhsgref_tla_machinegunner";
						rank="CORPORAL";
						position[]={5,0,0};
					};
					class Unit4
					{
						side=0;
						vehicle="rhsgref_tla_grenadier";
						rank="PRIVATE";
						position[]={7,0,0};
					};
					class Unit5
					{
						side=0;
						vehicle="rhsgref_tla_rifleman_vz58";
						rank="CORPORAL";
						position[]={9,0,0};
					};
					class Unit6
					{
						side=0;
						vehicle="rhsgref_tla_machinegunner_mg42";
						rank="PRIVATE";
						position[]={11,0,0};
					};
					class Unit7
					{
						side=0;
						vehicle="rhsgref_tla_grenadier_m79";
						rank="PRIVATE";
						position[]={13,0,0};
					};
					class Unit8
					{
						side=0;
						vehicle="rhsgref_tla_specialist_at";
						rank="PRIVATE";
						position[]={15,0,0};
					};
					class Unit9
					{
						side=0;
						vehicle="rhsgref_tla_rifleman_M16";
						rank="PRIVATE";
						position[]={17,0,0};
					};
					class Unit10
					{
						side=0;
						vehicle="rhsgref_tla_medic";
						rank="PRIVATE";
						position[]={19,0,0};
					};
				};
			};
		};
	};
};
class DefaultEventHandlers;
class UniformSlotInfo;
class CfgVehicles
{
	class Land;
	class Man: Land
	{
		class EventHandlers;
	};
	class CAManBase: Man
	{
		class HitPoints;
	};
	class SoldierGB: CAManBase
	{
		class HitPoints: HitPoints
		{
			class HitFace;
			class HitNeck;
			class HitHead;
			class HitPelvis;
			class HitAbdomen;
			class HitDiaphragm;
			class HitChest;
			class HitBody;
			class HitArms;
	};
	class B_FieldPack_oli;
	class B_Carryall_oli;
	class B_TacticalPack_oli;
	class B_AssaultPack_blk;
	class rhsusf_falconii;
	class B_FieldPack_khk;
	class B_FieldPack_cbr;
	class B_TacticalPack_blk;
	class B_Carryall_cbr;
	class rhs_assault_umbts_engineer_empty;
	class Bag_Base;
	class B_AssaultPack_rgr;
	class B_Kitbag_rgr;
	class rhsgref_hidf_alicepack: Bag_Base
	{
		displayname="ALICE Pack (OD)";
		hiddenselectionstextures[]=
		{
			"\rhsgref\addons\rhsgref_infantry\data_tanoa\alicepack_co.paa"
		};
		maximumLoad=320;
		mass=60;
	};
	class rhsgref_wdl_alicepack: rhsgref_hidf_alicepack
	{
		displayname="ALICE Pack (Woodland)";
		hiddenselectionstextures[]=
		{
			"\rhsgref\addons\rhsgref_infantry\data_cdf\alicepack_co_gref_woodland.paa"
		};
	};
	class rhsgref_ttsko_alicepack: rhsgref_hidf_alicepack
	{
		displayname="ALICE Pack (TTsKO)";
		hiddenselectionstextures[]=
		{
			"\rhsgref\addons\rhsgref_infantry\data_cdf\alicepack_co_gref_ttsko.paa"
		};
	};
	class rhs_rpg;
	class rhs_sidor;
	class rhsgref_cdf_ngd_base: SoldierGB
	{
		side=2;
		identityTypes[]=
		{
			"LanguageCZ",
			"Head_Euro"
		};
		genericNames="RussianMen";
		portrait="";
		icon="iconMan";
		accuracy=2.3;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		minFireTime=7;
		canCarryBackPack=1;
		nakedUniform="U_BasicBody";
		uniformClass="rhsgref_uniform_ttsko_forest";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		selectionbodywound="body_injury";
		selectionlarmwound="l_arm_injury";
		selectionlarmwound1="l_arm_injury";
		selectionlarmwound2="hl";
		selectionllegwound="l_leg_injury";
		selectionllegwound1="l_leg_injury";
		selectionllegwound2="hl";
		selectionrarmwound="r_arm_injury";
		selectionrarmwound1="r_arm_injury";
		selectionrarmwound2="hl";
		selectionRLegWound="r_leg_injury";
		selectionRLegWound1="r_leg_injury";
		selectionrlegwound2="hl";
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_mag_rgd5"
		};
	};
	class rhsgref_cdf_ngd_rifleman: rhsgref_cdf_ngd_base
	{
		linkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_ngd_rifleman_rpg75: rhsgref_cdf_ngd_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=180000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		weapons[]=
		{
			"rhs_weap_akm",
			"rhs_weap_rpg75",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"rhs_weap_rpg75",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5",
			"rhs_rpg75_mag"
		};
		linkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_ngd_rifleman_m92: rhsgref_cdf_ngd_base
	{
		linkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_ngd_rifleman_ak74: rhsgref_cdf_ngd_base
	{
		linkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_ak74",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_ak74",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_mag_rgd5"
		};
	};
	class rhsgref_cdf_ngd_rifleman_lite: rhsgref_cdf_ngd_base
	{
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms_folded",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms_folded",
			"Throw",
			"Put"
		};
	};
	class rhsgref_cdf_ngd_grenadier: rhsgref_cdf_ngd_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_ngd_machinegunner: rhsgref_cdf_ngd_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_100Rnd_762x54mmR"
		};
		backpack="rhsgref_cdf_backpack_mg";
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_ngd_officer: rhsgref_cdf_ngd_rifleman
	{
		cost=600000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_makarov_pm",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_makarov_pm",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_nspn_red",
			"rhs_mag_nspn_yellow",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_ngd_squadleader: rhsgref_cdf_ngd_rifleman
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6b5_officer_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6b5_officer_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akmn_gp25",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akmn_gp25",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_VOG25",
			"rhs_GDM40",
			"rhs_GDM40",
			"rhs_VG40OP_white",
			"rhs_VG40OP_white",
			"rhs_mag_nspn_yellow",
			"rhs_mag_rdg2_white",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_ngd_grenadier_rpg: rhsgref_cdf_ngd_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=160000;
		threat[]={1,0.5,0.2};
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_ttsko_forest",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put",
			"rhs_weap_rpg7"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put",
			"rhs_weap_rpg7"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_rpg7_PG7VL_mag"
		};
		backpack="rhs_rpg_vl";
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_ngd_medic: rhsgref_cdf_ngd_rifleman
	{
		attendant=1;
		icon="iconManMedic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6b5_medic_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_forest",
			"rhs_6b5_medic_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		backpack="rhsgref_cdf_backpack_medic";
	};
	class rhsgref_cdf_ngd_engineer: rhsgref_cdf_ngd_rifleman
	{
		cost=220000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		engineer=1;
		detectSkill=60;
		threat[]={1,0.5,0.1};
		icon="iconManEngineer";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		linkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		backpack="rhsgref_cdf_backpack_engineer";
	};
	class rhsgref_cdf_ngd_crew: rhsgref_cdf_ngd_rifleman
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		linkedItems[]=
		{
			"rhs_tsh4",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_tsh4",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_makarov_pm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_makarov_pm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_nspn_red"
		};
	};
	class rhsgref_cdf_reg_base: SoldierGB
	{
		side=2;
		identityTypes[]=
		{
			"LanguageCZ",
			"Head_Euro"
		};
		genericNames="RussianMen";
		portrait="";
		icon="iconMan";
		accuracy=2.3;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		minFireTime=7;
		canCarryBackPack=1;
		nakedUniform="U_BasicBody";
		uniformClass="rhsgref_uniform_ttsko_mountain";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		selectionbodywound="body_injury";
		selectionlarmwound="l_arm_injury";
		selectionlarmwound1="l_arm_injury";
		selectionlarmwound2="hl";
		selectionllegwound="l_leg_injury";
		selectionllegwound1="l_leg_injury";
		selectionllegwound2="hl";
		selectionrarmwound="r_arm_injury";
		selectionrarmwound1="r_arm_injury";
		selectionrarmwound2="hl";
		selectionRLegWound="r_leg_injury";
		selectionRLegWound1="r_leg_injury";
		selectionrlegwound2="hl";
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5"
		};
	};
	class rhsgref_cdf_reg_TTsKO_forest: rhsgref_cdf_reg_base
	{
		uniformClass="rhsgref_uniform_ttsko_forest";
	};
	class rhsgref_cdf_reg_TTsKO_mountain: rhsgref_cdf_reg_base
	{
		uniformClass="rhsgref_uniform_ttsko_mountain";
	};
	class rhsgref_cdf_reg_TTsKO_urban: rhsgref_cdf_reg_base
	{
		uniformClass="rhsgref_uniform_ttsko_urban";
	};
	class rhsgref_cdf_reg_vsr: rhsgref_cdf_reg_base
	{
		uniformClass="rhsgref_uniform_vsr";
	};
	class rhsgref_cdf_reg_rifleman: rhsgref_cdf_reg_base
	{
		weapons[]=
		{
			"rhs_weap_ak74",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_ak74",
			"Throw",
			"Put"
		};
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_reg_rifleman_rpg75: rhsgref_cdf_reg_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=180000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		weapons[]=
		{
			"rhs_weap_ak74",
			"rhs_weap_rpg75",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_ak74",
			"rhs_weap_rpg75",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5",
			"rhs_rpg75_mag"
		};
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_reg_rifleman_m70: rhsgref_cdf_reg_base
	{
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74_2",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74_2",
			"Throw",
			"Put"
		};
	};
	class rhsgref_cdf_reg_rifleman_akm: rhsgref_cdf_reg_rifleman_m70
	{
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
	};
	class rhsgref_cdf_reg_rifleman_lite: rhsgref_cdf_reg_base
	{
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
	};
	class rhsgref_cdf_reg_grenadier: rhsgref_cdf_reg_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_ak74_gp25",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_ak74_gp25",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_reg_machinegunner: rhsgref_cdf_reg_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_rifleman_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_100Rnd_762x54mmR",
			"rhs_mag_rgd5"
		};
		backpack="rhsgref_cdf_backpack_mg";
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_reg_marksman: rhsgref_cdf_reg_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_sniper_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_sniper_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m76_pso",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m76_pso",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhs_mag_rdg2_white",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_reg_general: rhsgref_cdf_reg_rifleman
	{
		cost=1000000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_reg_officer: rhsgref_cdf_reg_rifleman
	{
		cost=600000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_officer_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_officer_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_nspn_red",
			"rhs_mag_nspn_green",
			"rhs_mag_nspn_yellow",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_reg_squadleader: rhsgref_cdf_reg_rifleman
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_officer_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_officer_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74n",
			"rhs_weap_rsp30_green",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74n",
			"rhs_weap_rsp30_green",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_mag_nspn_green",
			"rhs_mag_nspn_yellow",
			"rhs_mag_rdg2_white",
			"rhs_mag_rdg2_white",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5"
		};
	};
	class rhsgref_cdf_reg_grenadier_rpg: rhsgref_cdf_reg_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=160000;
		threat[]={1,0.5,0.2};
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74",
			"Throw",
			"rhs_weap_rpg7_pgo",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74",
			"rhs_weap_rpg7_pgo",
			"Throw",
			"Put"
		};
		backpack="rhs_rpg_at";
		secondaryAmmoCoef=0.5;
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5",
			"rhs_rpg7_PG7VL_mag"
		};
	};
	class rhsgref_cdf_reg_specialist_aa: rhsgref_cdf_reg_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_mag_9k38_rocket"
		};
	};
	class rhsgref_cdf_reg_medic: rhsgref_cdf_reg_rifleman
	{
		attendant=1;
		icon="iconManMedic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_medic_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_medic_ttsko",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_30Rnd_545x39_7N6M_AK",
			"rhs_mag_nspn_yellow"
		};
		backpack="rhsgref_cdf_backpack_medic";
	};
	class rhsgref_cdf_reg_engineer: rhsgref_cdf_reg_rifleman
	{
		cost=220000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		engineer=1;
		detectSkill=60;
		threat[]={1,0.5,0.1};
		icon="iconManEngineer";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_mountain",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		backpack="rhsgref_cdf_backpack_engineer";
	};
	class rhsgref_cdf_reg_crew: rhsgref_cdf_reg_rifleman
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		linkedItems[]=
		{
			"rhs_tsh4_ess",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_tsh4_ess",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_nspn_red"
		};
	};
	class rhsgref_cdf_reg_crew_commander: rhsgref_cdf_reg_crew
	{
		cost=450000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhs_tsh4",
			"rhs_vest_commander",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_tsh4",
			"rhs_vest_commander",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_rdg2_white",
			"rhs_mag_rdg2_white",
			"rhs_mag_nspn_red"
		};
	};
	class rhsgref_cdf_air_pilot: rhsgref_cdf_reg_crew
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhs_zsh7a_mike",
			"rhs_vest_commander",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_zsh7a_mike",
			"rhs_vest_commander",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_para_base: rhsgref_cdf_reg_base
	{
		side=2;
		identityTypes[]=
		{
			"LanguageCZ",
			"Head_Euro"
		};
		genericNames="RussianMen";
		portrait="";
		icon="iconMan";
		accuracy=2.3;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		minFireTime=7;
		nakedUniform="U_BasicBody";
		uniformClass="rhsgref_uniform_para_ttsko_urban";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		class UniformInfo
		{
			class SlotsInfo
			{
				class NVG: UniformSlotInfo
				{
					slotType=602;
				};
				class Scuba: UniformSlotInfo
				{
					slotType=604;
				};
				class Googles: UniformSlotInfo
				{
					slotType=603;
				};
				class Headgear: UniformSlotInfo
				{
					slotType=605;
				};
			};
		};
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsd2",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsd2",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhs_mag_rgn",
			"rhs_mag_rgn"
		};
	};
	class rhsgref_cdf_para_rifleman: rhsgref_cdf_para_base
	{
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_para_rifleman_lite: rhsgref_cdf_para_rifleman
	{
		uniformClass="rhsgref_uniform_para_ttsko_mountain";
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsd2",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsd2",
			"Throw",
			"Put"
		};
	};
	class rhsgref_cdf_para_grenadier: rhsgref_cdf_para_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsd2_bg",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsd2_bg",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_para_autorifleman: rhsgref_cdf_para_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m249_pip_L_para",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m249_pip_L_para",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhsusf_100Rnd_556x45_soft_pouch",
			"rhs_mag_rgn",
			"rhs_mag_rgn"
		};
		backpack="rhsgref_cdf_backpack_mg3";
	};
	class rhsgref_cdf_para_machinegunner: rhsgref_cdf_para_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_rifleman",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m240G",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m240G",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_100Rnd_762x51",
			"rhs_mag_rgn"
		};
		backpack="rhsgref_cdf_backpack_mg2";
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_cdf_para_marksman: rhsgref_cdf_para_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_fieldcap_ttsko_mountain",
			"rhs_6b5_sniper",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m76_pso",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m76_pso",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhs_mag_rdg2_white",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_para_officer: rhsgref_cdf_para_rifleman
	{
		cost=600000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_officer",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_officer",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_nspn_green",
			"rhs_mag_nspn_yellow",
			"rhs_mag_nspn_red",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_para_squadleader: rhsgref_cdf_para_rifleman
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_officer",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_officer",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsd2_eo",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsd2_eo",
			"rhsusf_weap_glock17g4",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_nspn_green",
			"rhs_mag_nspn_yellow",
			"rhs_mag_rdg2_white",
			"rhs_mag_rdg2_white",
			"rhs_mag_rgn",
			"rhs_mag_rgn"
		};
	};
	class rhsgref_cdf_para_grenadier_rpg: rhsgref_cdf_para_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=160000;
		threat[]={1,0.5,0.2};
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_forest",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsd2_ct15x",
			"Throw",
			"rhs_weap_rpg7_pgo",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsd2_ct15x",
			"rhs_weap_rpg7_pgo",
			"Throw",
			"Put"
		};
		backpack="rhs_rpg_at";
		secondaryAmmoCoef=0.5;
		respawnMagazines[]=
		{
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhs_mag_rgn",
			"rhs_mag_rgn",
			"rhs_rpg7_PG7VL_mag"
		};
	};
	class rhsgref_cdf_para_specialist_aa: rhsgref_cdf_para_rifleman
	{
		uniformClass="rhsgref_uniform_para_ttsko_urban";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsd2_ct15x",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsd2_ct15x",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhs_mag_9k38_rocket"
		};
	};
	class rhsgref_cdf_para_medic: rhsgref_cdf_para_rifleman
	{
		attendant=1;
		icon="iconManMedic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		linkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_6b27m_ttsko_digi",
			"rhs_6b5_medic",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsk2",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsk2",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhs_mag_nspn_yellow"
		};
		backpack="rhsgref_cdf_backpack_medic";
	};
	class rhsgref_cdf_para_engineer: rhsgref_cdf_para_rifleman
	{
		cost=220000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		engineer=1;
		detectSkill=60;
		threat[]={1,0.5,0.1};
		icon="iconManEngineer";
		backpack="rhsgref_cdf_backpack_engineer";
	};
	class rhsgref_cdf_para_crew: rhsgref_cdf_para_rifleman
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		linkedItems[]=
		{
			"rhs_tsh4_ess",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_tsh4_ess",
			"rhs_6b5",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_vhsk2",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_vhsk2",
			"rhsusf_weap_glock17g4",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsgref_30rnd_556x45_vhs2",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhsusf_mag_17Rnd_9x19_FMJ",
			"rhs_mag_nspn_red",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_cdf_b_ngd_rifleman: rhsgref_cdf_ngd_rifleman
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_rifleman_rpg75: rhsgref_cdf_ngd_rifleman_rpg75
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_rifleman_m92: rhsgref_cdf_ngd_rifleman_m92
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_rifleman_ak74: rhsgref_cdf_ngd_rifleman_ak74
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_rifleman_lite: rhsgref_cdf_ngd_rifleman_lite
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_grenadier: rhsgref_cdf_ngd_grenadier
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_machinegunner: rhsgref_cdf_ngd_machinegunner
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_officer: rhsgref_cdf_ngd_officer
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_squadleader: rhsgref_cdf_ngd_squadleader
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_grenadier_rpg: rhsgref_cdf_ngd_grenadier_rpg
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_medic: rhsgref_cdf_ngd_medic
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_engineer: rhsgref_cdf_ngd_engineer
	{
		side=1;
	};
	class rhsgref_cdf_b_ngd_crew: rhsgref_cdf_ngd_crew
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_rifleman: rhsgref_cdf_reg_rifleman
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_rifleman_rpg75: rhsgref_cdf_reg_rifleman_rpg75
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_rifleman_m70: rhsgref_cdf_reg_rifleman_m70
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_rifleman_aks74: rhsgref_cdf_reg_rifleman_aks74
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_rifleman_akm: rhsgref_cdf_reg_rifleman_akm
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_rifleman_lite: rhsgref_cdf_reg_rifleman_lite
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_grenadier: rhsgref_cdf_reg_grenadier
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_machinegunner: rhsgref_cdf_reg_machinegunner
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_marksman: rhsgref_cdf_reg_marksman
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_general: rhsgref_cdf_reg_general
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_officer: rhsgref_cdf_reg_officer
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_squadleader: rhsgref_cdf_reg_squadleader
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_grenadier_rpg: rhsgref_cdf_reg_grenadier_rpg
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_specialist_aa: rhsgref_cdf_reg_specialist_aa
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_medic: rhsgref_cdf_reg_medic
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_engineer: rhsgref_cdf_reg_engineer
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_crew: rhsgref_cdf_reg_crew
	{
		side=1;
	};
	class rhsgref_cdf_b_reg_crew_commander: rhsgref_cdf_reg_crew_commander
	{
		side=1;
	};
	class rhsgref_cdf_b_air_pilot: rhsgref_cdf_air_pilot
	{
		side=1;
	};
	class rhsgref_cdf_b_para_rifleman: rhsgref_cdf_para_rifleman
	{
		side=1;
	};
	class rhsgref_cdf_b_para_rifleman_lite: rhsgref_cdf_para_rifleman_lite
	{
		side=1;
	};
	class rhsgref_cdf_b_para_grenadier: rhsgref_cdf_para_grenadier
	{
		side=1;
	};
	class rhsgref_cdf_b_para_autorifleman: rhsgref_cdf_para_autorifleman
	{
		side=1;
	};
	class rhsgref_cdf_b_para_machinegunner: rhsgref_cdf_para_machinegunner
	{
		side=1;
	};
	class rhsgref_cdf_b_para_marksman: rhsgref_cdf_para_marksman
	{
		side=1;
	};
	class rhsgref_cdf_b_para_officer: rhsgref_cdf_para_officer
	{
		side=1;
	};
	class rhsgref_cdf_b_para_squadleader: rhsgref_cdf_para_squadleader
	{
		side=1;
	};
	class rhsgref_cdf_b_para_grenadier_rpg: rhsgref_cdf_para_grenadier_rpg
	{
		side=1;
	};
	class rhsgref_cdf_b_para_specialist_aa: rhsgref_cdf_para_specialist_aa
	{
		side=1;
	};
	class rhsgref_cdf_b_para_medic: rhsgref_cdf_para_medic
	{
		side=1;
	};
	class rhsgref_cdf_b_para_engineer: rhsgref_cdf_para_engineer
	{
		side=1;
	};
	class rhsgref_cdf_b_para_crew: rhsgref_cdf_para_crew
	{
		side=1;
	};
	class rhsgref_cdf_un_rifleman: rhsgref_cdf_reg_rifleman
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_rifleman_lite: rhsgref_cdf_reg_rifleman_lite
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_un_beret",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_un_beret",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_grenadier: rhsgref_cdf_reg_grenadier
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_machinegunner: rhsgref_cdf_reg_machinegunner
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_general: rhsgref_cdf_reg_general
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_un_beret",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_un_beret",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_officer: rhsgref_cdf_reg_officer
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_officer_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_officer_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_squadleader: rhsgref_cdf_reg_squadleader
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_officer_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_officer_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_grenadier_rpg: rhsgref_cdf_reg_grenadier_rpg
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_medic: rhsgref_cdf_reg_medic
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_medic_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_medic_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_engineer: rhsgref_cdf_reg_engineer
	{
		side=2;
		linkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_ssh68_un",
			"rhs_6b5_rifleman_khaki",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_crew: rhsgref_cdf_reg_crew
	{
		side=2;
		linkedItems[]=
		{
			"rhs_tsh4_ess",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_tsh4_ess",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_cdf_un_pilot: rhsgref_cdf_air_pilot
	{
		side=2;
		linkedItems[]=
		{
			"rhs_zsh7a_mike",
			"rhs_vest_commander",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_zsh7a_mike",
			"rhs_vest_commander",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_nat_base: SoldierGB
	{
		side=2;
		identityTypes[]=
		{
			"LanguageCZ",
			"Head_Euro",
			"rhs_scarf",
			"G_CIVIL_male"
		};
		genericNames="RussianMen";
		portrait="";
		icon="iconMan";
		accuracy=1;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1;
		minFireTime=7;
		canCarryBackPack=1;
		nakedUniform="U_BasicBody";
		uniformClass="rhsgref_uniform_woodland";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		selectionbodywound="body_injury";
		selectionlarmwound="l_arm_injury";
		selectionlarmwound1="l_arm_injury";
		selectionlarmwound2="hl";
		selectionllegwound="l_leg_injury";
		selectionllegwound1="l_leg_injury";
		selectionllegwound2="hl";
		selectionrarmwound="r_arm_injury";
		selectionrarmwound1="r_arm_injury";
		selectionrarmwound2="hl";
		selectionrlegwound="r_leg_injury";
		selectionrlegwound1="r_leg_injury";
		selectionrlegwound2="hl";
		class UniformInfo
		{
			class SlotsInfo
			{
				class NVG: UniformSlotInfo
				{
					slotType=602;
				};
				class Scuba: UniformSlotInfo
				{
					slotType=604;
				};
				class Googles: UniformSlotInfo
				{
					slotType=603;
				};
				class Headgear: UniformSlotInfo
				{
					slotType=605;
				};
			};
		};
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_flecktarn: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_flecktarn";
	};
	class rhsgref_nat_flecktarn_full: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_flecktarn_full";
	};
	class rhsgref_nat_tigerstripe: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_tigerstripe";
	};
	class rhsgref_nat_woodland: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_woodland";
	};
	class rhsgref_nat_woodland_olive: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_woodland_olive";
	};
	class rhsgref_nat_ERDL: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_ERDL";
	};
	class rhsgref_nat_olive: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_olive";
	};
	class rhsgref_nat_altis_lizard: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_altis_lizard";
	};
	class rhsgref_nat_altis_lizard_olive: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_altis_lizard_olive";
	};
	class rhsgref_nat_dpm: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_dpm";
	};
	class rhsgref_nat_dpm_olive: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_dpm_olive";
	};
	class rhsgref_nat_3color_desert: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_3color_desert";
	};
	class rhsgref_nat_gorka_1_flecktarn: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_gorka_1_f";
	};
	class rhsgref_nat_rifleman: rhsgref_nat_base
	{
		linkedItems[]=
		{
			"H_Booniehat_oli",
			"G_Bandanna_khk",
			"V_TacVest_camo",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_oli",
			"G_Bandanna_khk",
			"V_TacVest_camo",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
	};
	class rhsgref_nat_rifleman_akms: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_woodland_olive";
		linkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
	};
	class rhsgref_nat_rifleman_aks74: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_woodland_olive";
		linkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_militiaman_kar98k: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_woodland";
		linkedItems[]=
		{
			"H_Booniehat_oli",
			"G_Bandanna_khk",
			"V_BandollierB_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_oli",
			"G_Bandanna_khk",
			"V_BandollierB_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_kar98k",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_kar98k",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhsgref_5Rnd_792x57_kar98k",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_rifleman_m92: rhsgref_nat_base
	{
		linkedItems[]=
		{
			"H_Booniehat_oli",
			"G_Bandanna_tan",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_oli",
			"G_Bandanna_tan",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_MP44",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_MP44",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30Rnd_792x33_SmE_StG",
			"rhsgref_30Rnd_792x33_SmE_StG",
			"rhsgref_30Rnd_792x33_SmE_StG",
			"rhsgref_30Rnd_792x33_SmE_StG",
			"rhsgref_30Rnd_792x33_SmE_StG",
			"rhsgref_30Rnd_792x33_SmE_StG",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_rifleman_vz58: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_dpm_olive";
		linkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_oli",
			"V_TacVest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_oli",
			"V_TacVest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz58p",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58p",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_grenadier: rhsgref_nat_rifleman
	{
		linkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_TacVest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_TacVest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25"
		};
	};
	class rhsgref_nat_warlord: rhsgref_nat_base
	{
		icon="iconManOfficer";
		uniformClass="rhsgref_uniform_flecktarn";
		linkedItems[]=
		{
			"H_Hat_brown",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Hat_brown",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_tt33",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_tt33",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_762x25_8",
			"rhs_mag_762x25_8",
			"rhs_mag_762x25_8",
			"rhs_mag_762x25_8",
			"rhs_mag_rdg2_black",
			"rhs_mag_rdg2_black"
		};
	};
	class rhsgref_nat_commander: rhsgref_nat_base
	{
		icon="iconManLeader";
		uniformClass="rhsgref_uniform_flecktarn";
		linkedItems[]=
		{
			"H_Cap_blk",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Cap_blk",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz58v",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58v",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_mag_rdg2_black",
			"rhs_mag_rdg2_black",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_machinegunner: rhsgref_nat_base
	{
		icon="iconManMG";
		uniformClass="rhsgref_uniform_woodland_olive";
		linkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_Chestrig_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_Chestrig_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_100Rnd_762x54mmR"
		};
		backpack="rhsgref_nat_backpack_mg";
	};
	class rhsgref_nat_machinegunner_mg42: rhsgref_nat_base
	{
		icon="iconManMG";
		uniformClass="rhsgref_uniform_dpm";
		linkedItems[]=
		{
			"G_Balaclava_oli",
			"rhsgref_chestrig",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"G_Balaclava_oli",
			"rhsgref_chestrig",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_mg42",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mg42",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_50Rnd_792x57_SmE_drum",
			"rhsgref_50Rnd_792x57_SmK_drum"
		};
		backpack="rhsgref_nat_backpack_mg42";
	};
	class rhsgref_nat_grenadier_rpg: rhsgref_nat_rifleman
	{
		icon="iconManAT";
		linkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_tan",
			"V_TacVest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_tan",
			"V_TacVest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pm63",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pm63",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_rpg7_PG7V_mag"
		};
		backpack="rhs_rpg_v";
	};
	class rhsgref_nat_specialist_aa: rhsgref_nat_base
	{
		icon="iconManAT";
		uniformClass="rhsgref_uniform_tigerstripe";
		linkedItems[]=
		{
			"H_Booniehat_tan",
			"G_Bandanna_beast",
			"V_TacVest_camo",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_tan",
			"G_Bandanna_beast",
			"V_TacVest_camo",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz58p",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58p",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_mag_9k38_rocket"
		};
	};
	class rhsgref_nat_hunter: rhsgref_nat_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		uniformClass="rhsgref_uniform_woodland";
		linkedItems[]=
		{
			"rhs_balaclava",
			"V_Chestrig_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava",
			"V_Chestrig_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m76",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m76",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76"
		};
	};
	class rhsgref_nat_scout: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_tigerstripe";
		linkedItems[]=
		{
			"H_Booniehat_tan",
			"G_Bandanna_khk",
			"V_TacVest_camo",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_tan",
			"G_Bandanna_khk",
			"V_TacVest_camo",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_45th_akms",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_45th_akms",
			"Throw",
			"Put"
		};
	};
	class rhsgref_nat_saboteur: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_flecktarn";
		cost=115000;
		camouflage=1.6;
		sensitivity=2.5;
		engineer=1;
		canDeactivateMines=1;
		detectSkill=80;
		threat[]={1,0.5,0.1};
		icon="iconManExplosive";
		linkedItems[]=
		{
			"G_Balaclava_blk",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"G_Balaclava_blk",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsgref_nat_backpack_explosives";
	};
	class rhsgref_nat_medic: rhsgref_nat_base
	{
		attendant=1;
		icon="iconManMedic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		linkedItems[]=
		{
			"H_Booniehat_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
		backpack="rhsgref_cdf_backpack_medic";
	};
	class rhsgref_nat_crew: rhsgref_nat_base
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		uniformClass="rhsgref_uniform_woodland";
		linkedItems[]=
		{
			"G_Balaclava_blk",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"G_Balaclava_blk",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_makarov_pm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_makarov_pm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_nspn_red"
		};
	};
	class rhsgref_nat_alpenflage: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_alpenflage";
	};
	class rhsgref_nat_pmil_rifleman: rhsgref_nat_base
	{
		uniformClass="rhsgref_uniform_alpenflage";
		linkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_khk",
			"V_Chestrig_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_khk",
			"V_Chestrig_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz58p_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58p_black",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_pmil_rifleman_m92: rhsgref_nat_pmil_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_oli",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsgref_weap_akm_zenitco",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsgref_weap_akm_zenitco",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_pmil_rifleman_aksu: rhsgref_nat_pmil_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_tan",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_tan",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_pmil_grenadier: rhsgref_nat_pmil_rifleman
	{
		weapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25"
		};
	};
	class rhsgref_nat_pmil_commander: rhsgref_nat_pmil_rifleman
	{
		icon="iconManLeader";
		weapons[]=
		{
			"rhsgref_weap_savz58v_black_rxo1",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsgref_weap_savz58v_black_rxo1",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_mag_rdg2_black",
			"rhs_mag_rdg2_black",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_pmil_machinegunner: rhsgref_nat_pmil_rifleman
	{
		icon="iconManMG";
		linkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_100Rnd_762x54mmR"
		};
		backpack="rhsgref_nat_backpack_mg";
	};
	class rhsgref_nat_pmil_grenadier_rpg: rhsgref_nat_pmil_rifleman
	{
		icon="iconManAT";
		linkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_tan",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"G_Bandanna_tan",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz58v_black",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58v_black",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_rpg7_PG7VL_mag"
		};
		backpack="rhs_rpg_vl";
	};
	class rhsgref_nat_pmil_specialist_aa: rhsgref_nat_pmil_rifleman
	{
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_beast",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_beast",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz58p_black",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58p_black",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_mag_9k38_rocket"
		};
	};
	class rhsgref_nat_pmil_hunter: rhsgref_nat_pmil_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhs_balaclava",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m76_pso",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m76_pso",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhsgref_10Rnd_792x57_m76",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_nat_pmil_scout: rhsgref_nat_pmil_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_khk",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_Booniehat_alpen",
			"G_Bandanna_khk",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsgref_weap_akm_zenitco_sd",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsgref_weap_akm_zenitco_sd",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_nat_pmil_saboteur: rhsgref_nat_pmil_rifleman
	{
		cost=115000;
		camouflage=1.6;
		sensitivity=2.5;
		engineer=1;
		canDeactivateMines=1;
		detectSkill=80;
		threat[]={1,0.5,0.1};
		icon="iconManExplosive";
		weapons[]=
		{
			"rhsgref_weap_savz58v_saboteur",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsgref_weap_savz58v_saboteur",
			"Throw",
			"Put"
		};
		linkedItems[]=
		{
			"G_Balaclava_blk",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"G_Balaclava_blk",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsgref_nat_backpack_explosives";
	};
	class rhsgref_nat_pmil_medic: rhsgref_nat_pmil_rifleman
	{
		attendant=1;
		icon="iconManMedic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		weapons[]=
		{
			"rhs_weap_savz58v_black",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58v_black",
			"Throw",
			"Put"
		};
		backpack="rhsgref_cdf_backpack_medic";
	};
	class rhsgref_nat_pmil_crew: rhsgref_nat_pmil_rifleman
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		linkedItems[]=
		{
			"G_Balaclava_blk",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"G_Balaclava_blk",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_makarov_pm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_makarov_pm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_nspn_red"
		};
	};
	class rhsgref_ins_base: SoldierGB
	{
		side=0;
		identityTypes[]=
		{
			"LanguageRUS",
			"Head_Euro",
			"rhs_scarf",
			"G_CIVIL_male"
		};
		genericNames="RussianMen";
		portrait="";
		icon="iconMan";
		accuracy=1;
		sensitivity=3;
		threat[]={1,0.1,0.1};
		camouflage=1;
		minFireTime=7;
		canCarryBackPack=1;
		selectionClan="clan";
		nakedUniform="U_BasicBody";
		uniformClass="rhsgref_ins_uniform_specter";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		selectionbodywound="body_injury";
		selectionlarmwound="l_arm_injury";
		selectionlarmwound1="l_arm_injury";
		selectionlarmwound2="hl";
		selectionllegwound="l_leg_injury";
		selectionllegwound1="l_leg_injury";
		selectionllegwound2="hl";
		selectionrarmwound="r_arm_injury";
		selectionrarmwound1="r_arm_injury";
		selectionrarmwound2="hl";
		selectionrlegwound="r_leg_injury";
		selectionrlegwound1="r_leg_injury";
		selectionrlegwound2="hl";
		class UniformInfo
		{
			class SlotsInfo
			{
				class NVG: UniformSlotInfo
				{
					slotType=602;
				};
				class Scuba: UniformSlotInfo
				{
					slotType=604;
				};
				class Googles: UniformSlotInfo
				{
					slotType=603;
				};
				class Headgear: UniformSlotInfo
				{
					slotType=605;
				};
			};
		};
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_ak103_dtk",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_ak103_dtk",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_polymer",
			"rhs_30Rnd_762x39mm_polymer",
			"rhs_30Rnd_762x39mm_polymer",
			"rhs_30Rnd_762x39mm_polymer",
			"rhs_30Rnd_762x39mm_polymer",
			"rhs_30Rnd_762x39mm_polymer",
			"rhs_mag_rgn",
			"rhs_mag_rgn",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_ins_reed: rhsgref_ins_base
	{
		uniformClass="rhsgref_uniform_reed";
	};
	class rhsgref_ins_uniform_izlom: rhsgref_ins_base
	{
		uniformClass="rhs_uniform_mvd_izlom";
	};
	class rhsgref_ins_uniform_specter: rhsgref_ins_base
	{
		uniformClass="rhsgref_uniform_specter";
	};
	class rhsgref_ins_uniform_gorka_y: rhsgref_ins_base
	{
		uniformClass="rhs_uniform_gorka_r_y";
	};
	class rhsgref_ins_uniform_gorka_g: rhsgref_ins_uniform_gorka_y
	{
		uniformClass="rhs_uniform_gorka_r_g";
	};
	class rhsgref_ins_rifleman: rhsgref_ins_uniform_specter
	{
		linkedItems[]=
		{
			"rhsgref_bcap_specter",
			"rhs_6sh92",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_bcap_specter",
			"rhs_6sh92",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
	};
	class rhsgref_ins_rifleman_akm: rhsgref_ins_reed
	{
		linkedItems[]=
		{
			"rhs_beanie_green",
			"V_Tacvest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"V_Tacvest_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_ins_rifleman_aksu: rhsgref_ins_reed
	{
		linkedItems[]=
		{
			"rhs_beanie_green",
			"V_Chestrig_rgr",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"V_Chestrig_rgr",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_mag_rgn",
			"rhs_mag_rgn",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_ins_rifleman_aks74: rhsgref_ins_rifleman_aksu
	{
		linkedItems[]=
		{
			"rhs_beanie_green",
			"V_Chestrig_rgr",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie_green",
			"V_Chestrig_rgr",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74_2",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74_2",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_mag_rgd5",
			"rhs_mag_rgd5",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_ins_rifleman_RPG26: rhsgref_ins_uniform_specter
	{
		linkedItems[]=
		{
			"rhs_beanie",
			"G_Bandanna_blk",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_beanie",
			"G_Bandanna_blk",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"rhs_weap_rpg26",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"rhs_weap_rpg26",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_rpg26_mag"
		};
	};
	class rhsgref_ins_militiaman_mosin: rhsgref_ins_reed
	{
		linkedItems[]=
		{
			"H_Booniehat_tan",
			"G_Bandanna_khk",
			"V_BandollierB_rgr",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_tan",
			"G_Bandanna_khk",
			"V_BandollierB_rgr",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m38",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m38",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhsgref_5Rnd_762x54_m38",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_ins_grenadier: rhsgref_ins_uniform_specter
	{
		linkedItems[]=
		{
			"rhsgref_bcap_specter",
			"rhs_6sh92_digi_vog",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_bcap_specter",
			"rhs_6sh92_digi_vog",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm_gp25",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm",
			"rhs_30Rnd_762x39mm",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25"
		};
	};
	class rhsgref_ins_machinegunner: rhsgref_ins_reed
	{
		linkedItems[]=
		{
			"H_Bandanna_gry",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Bandanna_gry",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_100Rnd_762x54mmR"
		};
		backpack="rhs_sidorMG";
	};
	class rhsgref_ins_squadleader: rhsgref_ins_uniform_specter
	{
		linkedItems[]=
		{
			"rhsgref_patrolcap_specter",
			"rhs_6sh92_digi_radio",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_patrolcap_specter",
			"rhs_6sh92_digi_radio",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"rhs_weap_rsp30_red",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"rhs_weap_rsp30_red",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_AK_green",
			"rhs_mag_rdg2_white",
			"rhs_mag_rdg2_white",
			"rhs_mag_rgn"
		};
	};
	class rhsgref_ins_commander: rhsgref_ins_reed
	{
		linkedItems[]=
		{
			"H_Beret_blk",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Beret_blk",
			"rhs_vest_pistol_holster",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_makarov_pm",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_makarov_pm",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_9x18_8_57N181S",
			"rhs_mag_rdg2_white",
			"rhs_mag_rgn"
		};
	};
	class rhsgref_ins_grenadier_rpg: rhsgref_ins_uniform_izlom
	{
		linkedItems[]=
		{
			"G_Balaclava_blk",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"G_Balaclava_blk",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm",
			"rhs_rpg7_PG7VL_mag"
		};
		backpack="rhs_rpg_vl";
	};
	class rhsgref_ins_specialist_aa: rhsgref_ins_reed
	{
		linkedItems[]=
		{
			"rhs_balaclava1_olive",
			"rhs_6sh92",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava1_olive",
			"rhs_6sh92",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"rhs_weap_igla",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_mag_9k38_rocket"
		};
	};
	class rhsgref_ins_engineer: rhsgref_ins_uniform_gorka_g
	{
		cost=220000;
		camouflage=1.6;
		sensitivity=2.5;
		canDeactivateMines=1;
		engineer=1;
		detectSkill=60;
		threat[]={1,0.5,0.1};
		icon="iconManEngineer";
		linkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_ssh68",
			"rhs_6sh92_digi",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		backpack="rhsgref_ins_pack_demo";
	};
	class rhsgref_ins_saboteur: rhsgref_ins_uniform_specter
	{
		cost=115000;
		camouflage=1.6;
		sensitivity=2.5;
		engineer=1;
		canDeactivateMines=1;
		detectSkill=80;
		threat[]={1,0.5,0.1};
		icon="iconManExplosive";
		linkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava1_olive",
			"V_TacVest_blk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74un_sup",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74un_sup",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7U1_AK",
			"rhs_30Rnd_545x39_7U1_AK",
			"rhs_30Rnd_545x39_7U1_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_30Rnd_545x39_7N6M_plum_AK",
			"rhs_mag_nspn_yellow"
		};
		backpack="rhsgref_ins_pack_saboteur";
	};
	class rhsgref_ins_sniper: rhsgref_ins_uniform_izlom
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"H_Booniehat_oli",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Booniehat_oli",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsgref_weap_ak103_pso1m2",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsgref_weap_ak103_pso1m2",
			"Binocular",
			"Throw",
			"Put"
		};
	};
	class rhsgref_ins_spotter: rhsgref_ins_uniform_gorka_y
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		linkedItems[]=
		{
			"rhs_balaclava",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_balaclava",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"Binocular",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_mag_rgn",
			"rhs_mag_rgn",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_ins_medic: rhsgref_ins_uniform_specter
	{
		attendant=1;
		icon="iconManMedic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		weapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_30Rnd_762x39mm_bakelite",
			"rhs_mag_rgn",
			"rhs_mag_rgn",
			"rhs_mag_rdg2_white"
		};
		backpack="rhsgref_cdf_backpack_medic";
	};
	class rhsgref_ins_crew: rhsgref_ins_uniform_specter
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		linkedItems[]=
		{
			"rhs_tsh4",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhs_tsh4",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_mag_nspn_red"
		};
	};
	class rhsgref_ins_pilot: rhsgref_ins_uniform_specter
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"H_Cap_headphones",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"H_Cap_headphones",
			"rhs_6sh46",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_aks74u",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_30Rnd_545x39_7N6_AK",
			"rhs_mag_rdg2_white",
			"rhs_mag_nspn_red"
		};
	};
	class rhsgref_ins_g_rifleman: rhsgref_ins_rifleman
	{
		side=2;
	};
	class rhsgref_ins_g_rifleman_akm: rhsgref_ins_rifleman_akm
	{
		side=2;
	};
	class rhsgref_ins_g_rifleman_aksu: rhsgref_ins_rifleman_aksu
	{
		side=2;
	};
	class rhsgref_ins_g_rifleman_aks74: rhsgref_ins_rifleman_aks74
	{
		side=2;
	};
	class rhsgref_ins_g_rifleman_RPG26: rhsgref_ins_rifleman_RPG26
	{
		side=2;
	};
	class rhsgref_ins_g_militiaman_mosin: rhsgref_ins_militiaman_mosin
	{
		side=2;
	};
	class rhsgref_ins_g_grenadier: rhsgref_ins_grenadier
	{
		side=2;
	};
	class rhsgref_ins_g_machinegunner: rhsgref_ins_machinegunner
	{
		side=2;
	};
	class rhsgref_ins_g_squadleader: rhsgref_ins_squadleader
	{
		side=2;
	};
	class rhsgref_ins_g_commander: rhsgref_ins_commander
	{
		side=2;
	};
	class rhsgref_ins_g_grenadier_rpg: rhsgref_ins_grenadier_rpg
	{
		side=2;
	};
	class rhsgref_ins_g_specialist_aa: rhsgref_ins_specialist_aa
	{
		side=2;
	};
	class rhsgref_ins_g_engineer: rhsgref_ins_engineer
	{
		side=2;
	};
	class rhsgref_ins_g_saboteur: rhsgref_ins_saboteur
	{
		side=2;
	};
	class rhsgref_ins_g_sniper: rhsgref_ins_sniper
	{
		side=2;
	};
	class rhsgref_ins_g_spotter: rhsgref_ins_spotter
	{
		side=2;
	};
	class rhsgref_ins_g_medic: rhsgref_ins_medic
	{
		side=2;
	};
	class rhsgref_ins_g_crew: rhsgref_ins_crew
	{
		side=2;
	};
	class rhsgref_ins_g_pilot: rhsgref_ins_pilot
	{
		side=2;
	};
	class rhsgref_hidf_base: SoldierWB
	{
		side=1;
		identityTypes[]=
		{
			"LanguageENGFRE_F",
			"Head_Tanoan"
		};
		genericNames="TanoanMen";
		portrait="";
		icon="iconMan";
		accuracy=2.3;
		sensitivity=1.75;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		minFireTime=7;
		canCarryBackPack=1;
		nakedUniform="U_BasicBody";
		uniformClass="rhs_uniform_bdu_erdl";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		selectionbodywound="body_injury";
		selectionlarmwound="l_arm_injury";
		selectionlarmwound1="l_arm_injury";
		selectionlarmwound2="hl";
		selectionllegwound="l_leg_injury";
		selectionllegwound1="l_leg_injury";
		selectionllegwound2="hl";
		selectionrarmwound="r_arm_injury";
		selectionrarmwound1="r_arm_injury";
		selectionrarmwound2="hl";
		selectionrlegwound="r_leg_injury";
		selectionrlegwound1="r_leg_injury";
		selectionrlegwound2="hl";
		class UniformInfo
		{
			class SlotsInfo
			{
				class NVG: UniformSlotInfo
				{
					slotType=602;
				};
				class Scuba: UniformSlotInfo
				{
					slotType=604;
				};
				class Googles: UniformSlotInfo
				{
					slotType=603;
				};
				class Headgear: UniformSlotInfo
				{
					slotType=605;
				};
			};
		};
		Items[]=
		{
			"FirstAidKit",
			"rhsgref_hat_M1951"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsgref_hat_M1951"
		};
		linkedItems[]=
		{
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
	};
	class rhsgref_hidf_ERDL: rhsgref_hidf_base
	{
		uniformClass="rhsgref_uniform_ERDL";
	};
	class rhsgref_hidf_og107: rhsgref_hidf_base
	{
		uniformClass="rhsgref_uniform_og107";
	};
	class rhsgref_hidf_og107_erdl: rhsgref_hidf_base
	{
		uniformClass="rhsgref_uniform_og107_erdl";
	};
	class rhsgref_hidf_rifleman: rhsgref_hidf_base
	{
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], nil] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsgref_helmet_M1_bare",
			1,
			"rhsgref_helmet_M1_bare_alt01",
			1,
			"rhsgref_helmet_M1_painted",
			1,
			"rhsgref_helmet_M1_painted_alt01",
			1
		};
		linkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put"
		};
	};
	class rhsgref_hidf_grenadier: rhsgref_hidf_rifleman
	{
		cost=130000;
		threat[]={1,0.30000001,0.1};
		linkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_carryhandle_M203",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_carryhandle_M203",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_30Rnd_556x45_M855_Stanag",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_m67"
		};
		backpack="rhsgref_hidf_backpack_gr";
	};
	class rhsgref_hidf_grenadier_m79: rhsgref_hidf_grenadier
	{
		linkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m79",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m79",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_M714_white",
			"rhs_mag_M714_white",
			"rhs_mag_M715_green",
			"rhs_mag_M716_yellow"
		};
		backpack="";
	};
	class rhsgref_hidf_rifleman_m72: rhsgref_hidf_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=160000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		linkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1",
			"rhs_weap_m72a7",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1",
			"rhs_weap_m72a7",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_m67",
			"rhs_mag_m67",
			"rhs_m72a7_mag"
		};
		secondaryAmmoCoef=0.5;
	};
	class rhsgref_hidf_autorifleman: rhsgref_hidf_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsgref_helmet_M1_bare_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_bare_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m249",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m249",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_200rnd_556x45_M855_mixed_box",
			"rhsusf_200rnd_556x45_M855_mixed_box",
			"rhs_mag_m67",
			"rhs_mag_m67"
		};
		backpack="rhsgref_hidf_alicepack_ar";
	};
	class rhsgref_hidf_autorifleman_assist: rhsgref_hidf_rifleman
	{
		cost=110000;
		backpack="rhsgref_hidf_alicepack_ar";
	};
	class rhsgref_hidf_machinegunner: rhsgref_hidf_autorifleman
	{
		weapons[]=
		{
			"rhs_weap_m240G",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m240G",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_100Rnd_762x51",
			"rhsusf_100Rnd_762x51",
			"rhsusf_100Rnd_762x51",
			"rhs_mag_m67"
		};
		backpack="rhsgref_hidf_alicepack_mg";
	};
	class rhsgref_hidf_machinegunner_assist: rhsgref_hidf_rifleman
	{
		cost=110000;
		backpack="rhsgref_hidf_alicepack_mg";
	};
	class rhsgref_hidf_marksman: rhsgref_hidf_rifleman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=150000;
		sensitivity=3.3;
		Items[]=
		{
			"FirstAidKit",
			"rhsgref_hat_M1951",
			"rhsgref_acc_l1a1_anpvs2"
		};
		RespawnItems[]=
		{
			"FirstAidKit",
			"rhsgref_hat_M1951",
			"rhsgref_acc_l1a1_anpvs2"
		};
		linkedItems[]=
		{
			"rhsgref_helmet_M1_painted_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_painted_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1_suit",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1_suit",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_762x51_m61_fnfal",
			"rhs_mag_20Rnd_762x51_m61_fnfal",
			"rhs_mag_20Rnd_762x51_m61_fnfal",
			"rhs_mag_20Rnd_762x51_m61_fnfal",
			"rhs_mag_20Rnd_762x51_m61_fnfal",
			"rhs_mag_20Rnd_762x51_m61_fnfal",
			"rhs_mag_20Rnd_762x51_m62_fnfal",
			"rhs_mag_m67"
		};
	};
	class rhsgref_hidf_squadleader: rhsgref_hidf_rifleman
	{
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_helmet_M1_painted",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_painted",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_m18_red",
			"rhs_mag_m18_purple",
			"rhs_mag_m18_yellow",
			"rhs_mag_m67",
			"Chemlight_red",
			"Chemlight_red"
		};
	};
	class rhsgref_hidf_teamleader: rhsgref_hidf_rifleman
	{
		cost=450000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_bare",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_m18_purple",
			"rhs_mag_m18_yellow",
			"rhs_mag_m67"
		};
	};
	class rhsgref_hidf_medic: rhsgref_hidf_rifleman
	{
		attendant=1;
		icon="iconManMedic";
		backpack="rhsgref_hidf_alicepack_medic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		linkedItems[]=
		{
			"rhsgref_helmet_M1_painted_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_painted_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_m18_red"
		};
	};
	class rhsgref_hidf_sniper: rhsgref_hidf_marksman
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_sniper_s"
				};
				speechPlural[]=
				{
					"veh_infantry_sniper_p"
				};
			};
		};
		nameSound="veh_infantry_sniper_s";
		cost=350000;
		threat[]={1,0.30000001,0.30000001};
		camouflage=0.40000001;
		sensitivity=3.5;
		primaryAmmoCoef=0.2;
		secondaryAmmoCoef=0.050000001;
		handgunAmmoCoef=0.1;
		linkedItems[]=
		{
			"rhsgref_helmet_M1_painted_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_helmet_M1_painted_alt01",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m24sws_blk_leu",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m24sws_blk_leu",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnMagazines[]=
		{
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m118_special_Mag",
			"rhsusf_5Rnd_762x51_m62_Mag",
			"rhsusf_5Rnd_762x51_m62_Mag",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsgref_hidf_crewman: rhsgref_hidf_base
	{
		cost=90000;
		camouflage=1.6;
		sensitivity=2.5;
		engineer=1;
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhsusf_cvc_green_alt_helmet",
			"rhsgref_TacVest_ERDL",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_cvc_green_alt_helmet",
			"rhsgref_TacVest_ERDL",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m3a1",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m3a1",
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_1143x23_M1911B_SMG",
			"rhsgref_30rnd_1143x23_M1911B_SMG",
			"rhsgref_30rnd_1143x23_M1911B_SMG",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsgref_hidf_helipilot: rhsgref_hidf_base
	{
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_pilot_s"
				};
				speechPlural[]=
				{
					"veh_infantry_pilot_p"
				};
			};
		};
		nameSound="veh_infantry_pilot_s";
		cost=160000;
		camouflage=2;
		Items[]=
		{
			"FirstAidKit"
		};
		RespawnItems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhsusf_hgu56p_green",
			"rhsgref_TacVest_ERDL",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsusf_hgu56p_green",
			"rhsgref_TacVest_ERDL",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhsusf_weap_m1911a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhsusf_mag_7x45acp_MHP",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsgref_hidf_boat_driver: rhsgref_hidf_base
	{
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], nil] call BIS_fnc_unitHeadgear;}";
		};
		headgearList[]=
		{
			"rhsgref_helmet_M1_bare",
			1,
			"rhsgref_helmet_M1_bare_alt01",
			1,
			"rhsgref_helmet_M1_painted",
			1,
			"rhsgref_helmet_M1_painted_alt01",
			1,
			"rhsgref_helmet_M1_liner",
			3,
			"rhsgref_hat_m1941cap",
			7
		};
		linkedItems[]=
		{
			"rhsgref_hat_m1941cap",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_hat_m1941cap",
			"rhsgref_alice_webbing",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m1garand_sa43",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m1garand_sa43",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhs_mag_m18_red",
			"rhs_mag_an_m8hc"
		};
	};
	class rhsgref_tla_base: SoldierGB
	{
		side=0;
		identityTypes[]=
		{
			"LanguageFRE_F",
			"Head_Tanoan"
		};
		genericNames="TanoanMen";
		portrait="";
		icon="iconMan";
		accuracy=2.3;
		sensitivity=1.75;
		threat[]={1,0.1,0.1};
		camouflage=1.4;
		minFireTime=7;
		canCarryBackPack=1;
		nakedUniform="U_BasicBody";
		uniformClass="rhsgref_uniform_TLA_1";
		primaryAmmoCoef=0.40000001;
		secondaryAmmoCoef=0.2;
		handgunAmmoCoef=0.1;
		cost=100000;
		class UniformInfo
		{
			class SlotsInfo
			{
				class NVG: UniformSlotInfo
				{
					slotType=602;
				};
				class Scuba: UniformSlotInfo
				{
					slotType=604;
				};
				class Googles: UniformSlotInfo
				{
					slotType=603;
				};
				class Headgear: UniformSlotInfo
				{
					slotType=605;
				};
			};
		};
		class EventHandlers: EventHandlers
		{
			init="if (local (_this select 0)) then {[(_this select 0), [], nil] call BIS_fnc_unitHeadgear;}";
		};
		items[]=
		{
			"FirstAidKit"
		};
		respawnitems[]=
		{
			"FirstAidKit"
		};
		linkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_ssh68",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_ssh68",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_tla_para1: rhsgref_tla_base
	{
		uniformClass="rhsgref_uniform_TLA_1";
	};
	class rhsgref_tla_para2: rhsgref_tla_base
	{
		uniformClass="rhsgref_uniform_TLA_2";
	};
	class rhsgref_tla_rifleman: rhsgref_tla_para1
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		linkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_ssh68",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_ssh68",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put"
		};
	};
	class rhsgref_tla_rifleman_akms: rhsgref_tla_para2
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		linkedItems[]=
		{
			"rhsgref_chicom",
			"rhsgref_M56",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"rhsgref_M56",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_75Rnd_762x39mm_89",
			"rhs_75Rnd_762x39mm_89",
			"rhs_75Rnd_762x39mm_89",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_tla_rifleman_M16: rhsgref_tla_rifleman
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		linkedItems[]=
		{
			"rhsgref_alice_webbing",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_alice_webbing",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m16a4_carryhandle",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m16a4_carryhandle",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_20Rnd_556x45_M193_2MAG_Stanag",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_tla_rifleman_pm63: rhsgref_tla_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Booniehat_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Booniehat_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pm63",
			"Throw",
			"Put"
		};
	};
	class rhsgref_tla_rifleman_vz58: rhsgref_tla_rifleman
	{
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz58p",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz58p",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_30Rnd_762x39mm_Savz58",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_tla_rifleman_m1: rhsgref_tla_para2
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Hat_Safari_olive_F",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Hat_Safari_olive_F",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m1garand_sa43",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m1garand_sa43",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhsgref_8Rnd_762x63_M2B_M1rifle",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_tla_rifleman_l1a1: rhsgref_tla_para2
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		linkedItems[]=
		{
			"rhsgref_alice_webbing",
			"H_Booniehat_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_alice_webbing",
			"H_Booniehat_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_l1a1_wood",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_l1a1_wood",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_20Rnd_762x51_m80_fnfal",
			"rhs_mag_f1",
			"rhs_mag_f1"
		};
	};
	class rhsgref_tla_squadleader: rhsgref_tla_para1
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		cost=500000;
		camouflage=1.4;
		sensitivity=3.3;
		icon="iconManLeader";
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akm",
			"Throw",
			"Put",
			"Binocular"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_mag_f1",
			"rhs_mag_f1",
			"rhs_mag_rdg2_white"
		};
	};
	class rhsgref_tla_grenadier: rhsgref_tla_para1
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		linkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_ssh68",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_ssh68",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		cost=130000;
		threat[]={1,0.30000001,0.1};
		weapons[]=
		{
			"rhs_weap_akms_gp25",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms_gp25",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25",
			"rhs_VOG25"
		};
	};
	class rhsgref_tla_grenadier_m79: rhsgref_tla_para2
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Cap_oli",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		cost=130000;
		threat[]={1,0.30000001,0.1};
		weapons[]=
		{
			"rhs_weap_m79",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m79",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M441_HE",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP",
			"rhs_mag_M433_HEDP"
		};
	};
	class rhsgref_tla_machinegunner: rhsgref_tla_para2
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_MG_s"
				};
				speechPlural[]=
				{
					"veh_infantry_MG_p"
				};
			};
		};
		nameSound="veh_infantry_MG_s";
		cost=125000;
		threat[]={1,0.1,0.30000001};
		icon="iconManMG";
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Bandanna_khk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Bandanna_khk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pkm",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_100Rnd_762x54mmR",
			"rhs_100Rnd_762x54mmR",
			"rhs_mag_f1"
		};
		backpack="rhs_sidorMG";
	};
	class rhsgref_tla_machinegunner_mg42: rhsgref_tla_machinegunner
	{
		weapons[]=
		{
			"rhs_weap_mg42",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_mg42",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_50Rnd_792x57_SmE_drum",
			"rhsgref_50Rnd_792x57_SmE_drum",
			"rhsgref_50Rnd_792x57_SmE_drum",
			"rhsgref_50Rnd_792x57_SmE_drum",
			"rhsgref_50Rnd_792x57_SmE_drum",
			"rhsgref_50Rnd_792x57_SmE_drum",
			"rhs_mag_f1"
		};
		backpack="rhsgref_tla_backpack_mg42";
	};
	class rhsgref_tla_rifleman_rpg75: rhsgref_tla_para1
	{
		headgearList[]=
		{
			"rhs_ssh68",
			0.2,
			"rhsgref_M56",
			0.1,
			"H_Hat_Safari_olive_F",
			0.1,
			"H_Cap_oli",
			0.1,
			"H_Booniehat_oli",
			0.2,
			"H_Bandanna_khk",
			0.2,
			"H_HeadBandage_stained_F",
			0.1
		};
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_AT_s"
				};
				speechPlural[]=
				{
					"veh_infantry_AT_p"
				};
			};
		};
		nameSound="veh_infantry_AT_s";
		cost=180000;
		threat[]={1,0.69999999,0.30000001};
		icon="iconManAT";
		secondaryAmmoCoef=0.5;
		linkedItems[]=
		{
			"rhsgref_chicom",
			"rhsgref_M56",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"rhsgref_M56",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_pm63",
			"rhs_weap_rpg75",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_pm63",
			"rhs_weap_rpg75",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_mag_f1",
			"rhs_rpg75_mag"
		};
	};
	class rhsgref_tla_specialist_at: rhsgref_tla_rifleman_rpg75
	{
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Bandanna_khk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Bandanna_khk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_akms",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_akms",
			"rhs_weap_rpg7",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_30Rnd_762x39mm_89",
			"rhs_rpg7_PG7V_mag"
		};
		backpack="rhsgref_tla_backpack_rpg";
	};
	class rhsgref_tla_medic: rhsgref_tla_para1
	{
		headgearList[]=
		{
			"H_Hat_Safari_olive_F",
			0.2,
			"H_Cap_oli",
			0.2,
			"H_Booniehat_oli",
			0.30000001,
			"H_Bandanna_khk",
			0.30000001
		};
		attendant=1;
		icon="iconManMedic";
		class SpeechVariants
		{
			class Default
			{
				speechSingular[]=
				{
					"veh_infantry_medic_s"
				};
				speechPlural[]=
				{
					"veh_infantry_medic_p"
				};
			};
		};
		nameSound="veh_infantry_medic_s";
		linkedItems[]=
		{
			"rhsgref_chicom",
			"H_Bandanna_khk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"H_Bandanna_khk",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_m3a1",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_m3a1",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_30rnd_1143x23_M1911B_2mag_SMG",
			"rhsgref_30rnd_1143x23_M1911B_2mag_SMG",
			"rhsgref_30rnd_1143x23_M1911B_2mag_SMG",
			"rhsgref_30rnd_1143x23_M1911B_2mag_SMG",
			"rhsgref_30rnd_1143x23_M1911B_2mag_SMG",
			"rhsgref_30rnd_1143x23_M1911B_2mag_SMG"
		};
		backpack="rhs_medic_bag_filled";
	};
	class rhsgref_tla_crew: rhsgref_tla_para2
	{
		headgearList[]=
		{
			"rhs_tsh4_ess",
			0.5,
			"rhs_tsh4",
			0.5
		};
		cost=90000;
		linkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_tsh4",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		respawnLinkedItems[]=
		{
			"rhsgref_chicom",
			"rhs_tsh4",
			"ItemMap",
			"ItemCompass",
			"ItemWatch",
			"ItemRadio"
		};
		weapons[]=
		{
			"rhs_weap_savz61",
			"Throw",
			"Put"
		};
		respawnWeapons[]=
		{
			"rhs_weap_savz61",
			"Throw",
			"Put"
		};
		respawnMagazines[]=
		{
			"rhsgref_20rnd_765x17_vz61",
			"rhsgref_20rnd_765x17_vz61",
			"rhsgref_20rnd_765x17_vz61",
			"rhsgref_20rnd_765x17_vz61",
			"rhsgref_20rnd_765x17_vz61"
		};
	};
};
class UniformItem;
class cfgWeapons
{
	class Uniform_Base;
	class rhsgref_uniform_ttsko_forest: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_cdf_reg_TTsKO_forest";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_ttsko_mountain: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_cdf_reg_TTsKO_mountain";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_ttsko_urban: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_cdf_reg_TTsKO_urban";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_vsr: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_cdf_reg_vsr";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_para_ttsko_oxblood: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_cdf_para_TTsKO_summer";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_para_ttsko_mountain: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_cdf_para_TTsKO_fall";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_para_ttsko_urban: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_cdf_para_TTsKO_dark";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_specter: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_ins_uniform_specter";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_reed: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_ins_reed";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_alpenflage: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_alpenflage";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_flecktarn: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_flecktarn";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_flecktarn_full: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_flecktarn_full";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_tigerstripe: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_tigerstripe";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_woodland: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_woodland";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_woodland_olive: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_woodland_olive";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_ERDL: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_hidf_ERDL";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_og107: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_hidf_og107";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_og107_erdl: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_hidf_og107_erdl";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_altis_lizard: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_altis_lizard";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_olive: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_olive";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_altis_lizard_olive: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_altis_lizard_olive";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_dpm: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_dpm";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_dpm_olive: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_dpm_olive";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_3color_desert: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_3color_desert";
			containerClass="Supply40";
			mass=40;
		};
	};
	class rhsgref_uniform_gorka_1_f: Uniform_Base
	{
		class ItemInfo: UniformItem
		{
			uniformModel="-";
			uniformClass="rhsgref_nat_gorka_1_flecktarn";
			containerClass="Supply40";
			mass=40;
		};
	};
	class U_IG_leader: Uniform_Base
	{
		class ItemInfo;
	};
	class rhsgref_uniform_TLA_1: U_IG_leader
	{
		class ItemInfo: ItemInfo
		{
			uniformClass="rhsgref_tla_para1";
		};
	};
	class rhsgref_uniform_TLA_2: U_IG_leader
	{
		class ItemInfo: ItemInfo
		{
			uniformClass="rhsgref_tla_para2";
		};
	};
	class ItemCore;
	class HeadGearItem;
	class H_HelmetB: ItemCore
	{
		class ItemInfo;
	};
	class rhs_6b27m;
	class H_Cap_red;
	class rhs_fieldcap_digi2;
	class rhsgref_un_beret: ItemCore
	{
		weaponPoolAvailable=1;
		class ItemInfo: HeadGearItem
		{
			mass=10;
			uniformModel="\rhsgref\addons\rhsgref_infantry\gear_cdf\head\gref_UN_Beret";
		};
	};
	class rhsgref_Booniehat_alpen: ItemCore
	{
		weaponPoolAvailable=1;
		class ItemInfo: HeadGearItem
		{
			mass=10;
			uniformModel="\A3\Characters_F\Common\booniehat";
		};
	};
	class rhsgref_bcap_specter: H_Cap_red
	{
		allowedFacewear[]=
		{
			"G_Aviator",
			"G_Balaclava_blk",
			"G_Balaclava_oli",
			"G_Bandanna_aviator",
			"G_Bandanna_beast",
			"G_Bandanna_blk",
			"G_Bandanna_khk",
			"G_Bandanna_oli",
			"G_Bandanna_shades",
			"G_Bandanna_sport",
			"G_Bandanna_tan",
			"G_Shades_Black",
			"G_Shades_Blue",
			"G_Shades_Red",
			"G_Shades_Green"
		};
	};
	class rhsgref_patrolcap_specter: rhs_fieldcap_digi2
	{
		class ItemInfo: HeadGearItem
		{
			mass=5;
			uniformModel="\rhsgref\addons\rhsgref_infantry\gear_cdf\head\gref_fieldcap_digi2";
		};
	};
	class rhsgref_fieldcap: H_HelmetB
	{
		allowedFacewear[]=
		{
			"rhs_scarf",
			"G_Aviator"
		};
		class ItemInfo: HeadGearItem
		{
			mass=5;
			uniformModel="\rhsafrf\addons\rhs_infantry\gear\head\rhs_fieldcap";
		};
	};
	class rhsgref_fieldcap_ttsko_forest: rhsgref_fieldcap
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_fieldcap_ttsko_mountain: rhsgref_fieldcap
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_fieldcap_ttsko_urban: rhsgref_fieldcap
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_fieldcap_ttsko_digi: rhsgref_fieldcap
	{
		allowedFacewear[]=
		{
			"rhs_scarf",
			"G_Balaclava_blk"
		};
	};
	class rhsgref_ssh68: H_HelmetB
	{
		allowedFacewear[]=
		{
			"rhs_scarf",
			"G_Aviator"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsgref\addons\rhsgref_infantry\gear\head\rhs_ssh68_cloth";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=2;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_ssh68_ttsko_mountain: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_ttsko_forest: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_ttsko_fall: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_ttsko_summer: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_ttsko_dark: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_ttsko_digi: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_emr: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_un: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_ssh68_vsr: rhsgref_ssh68
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
	};
	class rhsgref_M56: H_HelmetB
	{
		allowedFacewear[]=
		{
			"rhs_scarf",
			"G_Aviator"
		};
		hidddenSelections[]=
		{
			"Camo2"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsgref\addons\rhsgref_infantry\gear\head\m56";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=2;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_6b27m: H_HelmetB
	{
		allowedFacewear[]=
		{
			"rhs_scarf",
			"G_Aviator"
		};
		hiddenSelectionsMaterials[]=
		{
			"rhsgref\addons\rhsgref_infantry\data_cdf\6b27_cdf.rvmat"
		};
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="\rhsgref\addons\rhsgref_infantry\gear_cdf\head\gref_6b27m";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=6;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_6b27m_ttsko_forest: rhsgref_6b27m
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
		hiddenSelectionsMaterials[]=
		{
			"rhsgref\addons\rhsgref_infantry\data_cdf\6b27_cdf.rvmat"
		};
	};
	class rhsgref_6b27m_ttsko_mountain: rhsgref_6b27m
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
		hiddenSelectionsMaterials[]=
		{
			"rhsafrf\addons\rhs_infantry\data\6b27.rvmat"
		};
	};
	class rhsgref_6b27m_ttsko_urban: rhsgref_6b27m
	{
		allowedFacewear[]=
		{
			"G_Squares_Tinted",
			"G_Aviator"
		};
		hiddenSelectionsMaterials[]=
		{
			"rhsgref\addons\rhsgref_infantry\data_cdf\6b27_cdf.rvmat"
		};
	};
	class rhsgref_6b27m_ttsko_digi: rhsgref_6b27m
	{
		allowedFacewear[]=
		{
			"rhs_scarf",
			"G_Balaclava_blk"
		};
		hiddenSelectionsMaterials[]=
		{
			"rhsgref\addons\rhsgref_infantry\data_cdf\6b27_cdf.rvmat"
		};
	};
	class rhsgref_helmet_M1_bare: H_HelmetB
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\M1_strapdown";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=2;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_helmet_M1_bare_alt01: rhsgref_helmet_M1_bare
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\M1_strapup";
		};
	};
	class rhsgref_helmet_M1_liner: H_HelmetB
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\M1_liner";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_helmet_M1_mit: H_HelmetB
	{
		class ItemInfo: ItemInfo
		{
			mass=42;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\m1_cover";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=2;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_helmet_M1_erdl: rhsgref_helmet_M1_mit
	{
		class ItemInfo: ItemInfo
		{
			mass=42;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\m1_cover";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=2;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_helmet_M1_des: rhsgref_helmet_M1_mit
	{
		class ItemInfo: ItemInfo
		{
			mass=42;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\m1_cover";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=2;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_hat_M1951: H_HelmetB
	{
		class ItemInfo: ItemInfo
		{
			mass=5;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\M1951cap";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=0;
					passThrough=0;
				};
			};
		};
	};
	class rhsgref_hat_m1941cap: rhsgref_hat_M1951
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\m1941cap";
		};
	};
	class rhsgref_helmet_M1_painted: rhsgref_helmet_M1_bare
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\M1_strapdown";
		};
	};
	class rhsgref_helmet_M1_painted_alt01: rhsgref_helmet_M1_bare
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\M1_strapup";
		};
	};
	class rhsgref_helmet_pasgt_erdl: H_HelmetB
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet";
			class HitpointsProtectionInfo
			{
				class Head
				{
					hitpointName="HitHead";
					armor=6;
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_helmet_pasgt_erdl_rhino: H_HelmetB
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet_rhino";
		};
	};
	class rhsgref_helmet_pasgt_woodland: rhsgref_helmet_pasgt_erdl
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet";
		};
	};
	class rhsgref_helmet_pasgt_woodland_rhino: rhsgref_helmet_pasgt_erdl_rhino
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet_rhino";
		};
	};
	class rhsgref_helmet_pasgt_flecktarn: rhsgref_helmet_pasgt_erdl_rhino
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet_rhino";
		};
	};
	class rhsgref_helmet_pasgt_un: rhsgref_helmet_pasgt_erdl
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet";
		};
	};
	class rhsgref_helmet_pasgt_press: rhsgref_helmet_pasgt_erdl
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet";
		};
	};
	class rhsgref_helmet_pasgt_3color_desert: rhsgref_helmet_pasgt_erdl
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet";
		};
	};
	class rhsgref_helmet_pasgt_3color_desert_rhino: rhsgref_helmet_pasgt_erdl_rhino
	{
		class ItemInfo: ItemInfo
		{
			mass=40;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet_rhino";
		};
	};
	class rhsgref_helmet_pasgt_altis_lizard: rhsgref_helmet_pasgt_erdl
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet";
		};
	};
	class rhsgref_helmet_pasgt_olive: rhsgref_helmet_pasgt_erdl
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\head\pasgt_helmet";
		};
	};
	class rhsgref_hat_m43cap_heer: rhsgref_hat_m1941cap
	{
		class ItemInfo: ItemInfo
		{
			mass=5;
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\m43cap";
		};
	};
	class rhsgref_hat_m43cap_heer1_tilted: rhsgref_hat_m43cap_heer
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\m43cap_tilted";
		};
	};
	class rhsgref_helmet_m1940: rhsgref_helmet_M1_bare
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1940_strapdown";
		};
	};
	class rhsgref_helmet_m1940_alt1: rhsgref_helmet_m1940
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1940_strapup";
		};
	};
	class rhsgref_helmet_m1940_h: rhsgref_helmet_m1940
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1940_strapdown";
		};
	};
	class rhsgref_helmet_m1940_camo01: rhsgref_helmet_m1940
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1940_strapdown";
		};
	};
	class rhsgref_helmet_m1940_camo01_alt1: rhsgref_helmet_m1940_camo01
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1940_strapup";
		};
	};
	class rhsgref_helmet_m1940_winter: rhsgref_helmet_m1940
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1940_strapdown";
		};
	};
	class rhsgref_helmet_m1940_winter_alt1: rhsgref_helmet_m1940_winter
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1940_strapup";
		};
	};
	class rhsgref_helmet_m1942: rhsgref_helmet_m1940
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_strapdown";
		};
	};
	class rhsgref_helmet_m1942_alt1: rhsgref_helmet_m1942
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_strapup";
		};
	};
	class rhsgref_helmet_m1942_h: rhsgref_helmet_m1942
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_strapdown";
		};
	};
	class rhsgref_helmet_m1942_camo01: rhsgref_helmet_m1942
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_strapdown";
		};
	};
	class rhsgref_helmet_m1942_camo01_alt1: rhsgref_helmet_m1942_camo01
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_strapup";
		};
	};
	class rhsgref_helmet_m1942_winter: rhsgref_helmet_m1942
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_strapdown";
		};
	};
	class rhsgref_helmet_m1942_winter_alt1: rhsgref_helmet_m1942_winter
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_strapup";
		};
	};
	class rhsgref_helmet_m1942_heermarshcover: rhsgref_helmet_m1942
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_heercoversummer";
		};
	};
	class rhsgref_helmet_m1942_heersplintercover: rhsgref_helmet_m1942_heermarshcover
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_heercoversummer";
		};
	};
	class rhsgref_helmet_m1942_heergreycover: rhsgref_helmet_m1942_heermarshcover
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_heercoversummer";
		};
	};
	class rhsgref_helmet_m1942_heerwintercover: rhsgref_helmet_m1942_heermarshcover
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear\Head\M1942_heercoverwinter";
		};
	};
	class Vest_Camo_Base;
	class rhs_6b23: Vest_Camo_Base
	{
		class ItemInfo;
	};
	class rhsgref_6b23_khaki: rhs_6b23
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_engineer";
			containerClass="Supply80";
			mass=65;
		};
	};
	class rhsgref_6b23_khaki_rifleman: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_vydra_3m";
			containerClass="Supply100";
			mass=75;
		};
	};
	class rhsgref_6b23_khaki_sniper: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_sniper";
			containerClass="Supply80";
			mass=65;
		};
	};
	class rhsgref_6b23_khaki_officer: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_crewofficer";
			containerClass="Supply60";
			mass=65;
		};
	};
	class rhsgref_6b23_khaki_nco: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_6sh46";
			containerClass="Supply80";
			mass=65;
		};
	};
	class rhsgref_6b23_khaki_medic: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_medic";
			containerClass="Supply130";
			mass=80;
		};
	};
	class rhsgref_6b23_ttsko_forest: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_engineer";
		};
	};
	class rhsgref_6b23_ttsko_forest_rifleman: rhsgref_6b23_khaki_rifleman
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_vydra_3m";
		};
	};
	class rhsgref_6b23_ttsko_mountain: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_engineer";
		};
	};
	class rhsgref_6b23_ttsko_mountain_rifleman: rhsgref_6b23_khaki_rifleman
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_vydra_3m";
		};
	};
	class rhsgref_6b23_ttsko_mountain_sniper: rhsgref_6b23_khaki_sniper
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_sniper";
		};
	};
	class rhsgref_6b23_ttsko_mountain_officer: rhsgref_6b23_khaki_officer
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_crewofficer";
		};
	};
	class rhsgref_6b23_ttsko_mountain_nco: rhsgref_6b23_khaki_nco
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_6sh46";
		};
	};
	class rhsgref_6b23_ttsko_mountain_medic: rhsgref_6b23_khaki_medic
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_medic";
		};
	};
	class rhsgref_6b23_ttsko_digi: rhsgref_6b23_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_engineer";
		};
	};
	class rhsgref_6b23_ttsko_digi_rifleman: rhsgref_6b23_khaki_rifleman
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_vydra_3m";
		};
	};
	class rhsgref_6b23_ttsko_digi_sniper: rhsgref_6b23_khaki_sniper
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_sniper";
		};
	};
	class rhsgref_6b23_ttsko_digi_officer: rhsgref_6b23_khaki_officer
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_crewofficer";
		};
	};
	class rhsgref_6b23_ttsko_digi_nco: rhsgref_6b23_khaki_nco
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_6sh46";
		};
	};
	class rhsgref_6b23_ttsko_digi_medic: rhsgref_6b23_khaki_medic
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsafrf\addons\rhs_infantry\gear\vests\rhs_6b23_medic";
		};
	};
	class rhs_6b5: rhs_6b23
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_cdf\vests\rhs_6b5_rifleman";
			containerClass="Supply80";
			mass=132;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=8;
					PassThrough=0.5;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=25;
					PassThrough=0.2;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=25;
					PassThrough=0.2;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=25;
					passThrough=0.2;
				};
				class Body
				{
					hitpointName="HitBody";
					armor=8;
					passThrough=0.5;
				};
			};
		};
	};
	class rhs_6b5_officer: rhs_6b5
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_cdf\vests\rhs_6b5_officer";
			containerClass="Supply120";
			mass=142;
		};
	};
	class rhs_6b5_medic: rhs_6b5
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_cdf\vests\rhs_6b5_medic";
			containerClass="Supply140";
			mass=152;
		};
	};
	class rhs_6b5_sniper: rhs_6b5
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_cdf\vests\rhs_6b5_sniper";
			containerClass="Supply90";
			mass=137;
		};
	};
	class rhs_6b5_rifleman: rhs_6b5
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_cdf\vests\rhs_6b5_6sh46_rifleman";
			containerClass="Supply130";
			mass=147;
		};
	};
	class rhs_6b5_khaki: rhs_6b5
	{
		Scope=2;
	};
	class rhs_6b5_vsr: rhs_6b5
	{
		Scope=2;
	};
	class rhs_6b5_ttsko: rhs_6b5
	{
		Scope=2;
	};
	class rhsgref_otv_khaki: rhsgref_6b23_khaki_rifleman
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="A3\Characters_F_Beta\INDEP\equip_ia_vest01";
			containerClass="Supply100";
			mass=70;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=0;
					PassThrough=0.89999998;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=22;
					PassThrough=0.2;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=22;
					PassThrough=0.2;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_otv_digi: rhsgref_otv_khaki
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="A3\Characters_F_Beta\INDEP\equip_ia_vest01";
		};
	};
	class rhsgref_TacVest_ERDL: rhsgref_6b23_khaki_rifleman
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="A3\Characters_F\Common\equip_tacticalvest";
			containerClass="Supply100";
			mass=40;
			class HitpointsProtectionInfo
			{
				class Neck
				{
					HitpointName="HitNeck";
					armor=0;
					PassThrough=0.89999998;
				};
				class Chest
				{
					HitpointName="HitChest";
					armor=8;
					PassThrough=0.5;
				};
				class Diaphragm
				{
					HitpointName="HitDiaphragm";
					armor=8;
					PassThrough=0.5;
				};
				class Abdomen
				{
					hitpointName="HitAbdomen";
					armor=3;
					passThrough=0.60000002;
				};
				class Body
				{
					hitpointName="HitBody";
					passThrough=0.5;
				};
			};
		};
	};
	class rhsgref_alice_webbing: rhs_6b23
	{
		Scope=2;
		hiddenSelections[]={};
		hiddenSelectionsTextures[]={};
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_cdf\vests\rhs_alice_webbing";
			containerClass="Supply120";
			mass=15;
			class HitpointsProtectionInfo
			{
				class Body
				{
					hitpointName="HitBody";
					armor=0;
					passThrough=1;
				};
			};
		};
	};
	class rhsgref_chestrig: rhs_6b23
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="a3\Characters_F\Common\equip_chestrig_light";
			containerClass="Supply100";
			mass=15;
			class HitpointsProtectionInfo
			{
				class Body
				{
					hitpointName="HitBody";
					armor=0;
					passThrough=1;
				};
			};
		};
	};
	class rhsgref_chicom: rhsgref_chestrig
	{
		Scope=2;
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\vest\rhs_vest_chicom.p3d";
			containerClass="Supply100";
			mass=15;
			class HitpointsProtectionInfo
			{
				class Body
				{
					hitpointName="HitBody";
					armor=0;
					passThrough=1;
				};
			};
		};
	};
	class rhsgref_chicom_m88: rhsgref_chicom
	{
		class ItemInfo: ItemInfo
		{
			uniformModel="rhsgref\addons\rhsgref_infantry\gear_tanoa\vest\rhs_vest_chicom2.p3d";
		};
	};
	class rhs_weap_ak103;
	class rhs_weap_svds;
	class rhs_weap_savz58v_rail_black;
	class rhs_weap_savz58v_ris_grip3;
	class rhs_weap_akm_zenitco01_b33_grip1;
};
